---
# SPDX-FileCopyrightText: Copyright (c) 2025-2026 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
# SPDX-License-Identifier: Apache-2.0

description: Safe Synthesizer Tasks is part of the NeMo Helix
---

## NeMo Safe Synthesizer Tasks Container

This Docker container image provides the NeMo Helix GPU task runtime for NeMo Safe Synthesizer.

### Resources

[Documentation](https://docs.nvidia.com/nemo-helix)

### License

This container is licensed under the [Apache License 2.0](https://github.com/NVIDIA-NeMo/nemo-helix/blob/main/LICENSE).
