...
"""Job compiler - transforms CustomizationJobOutput into HelixJobSpec."""

import logging

from nemo_helix_plugin.client.errors import NotFoundError
from nemo_helix_plugin.deployment import LORA_ENABLED_REQUIRED_MESSAGE
...
from nhx.automodel.app.constants import (
    DEFAULT_DATASET_PATH,
    DEFAULT_MODEL_PATH,
    DEFAULT_OUTPUT_MODEL_PATH,
    DEFAULT_TEACHER_MODEL_PATH,
)
from nhx.automodel.app.jobs.training.compiler import (
    _extract_model_name,
    _resolve_training_recipe,
    compile_training_step,
)
from nhx.automodel.config import config
...
from nhx.customization_common.service.platform_client import AsyncCustomizationHelixClients, fetch_model_entity
...
async def platform_job_config_compiler(
    ...
    """Compile canonical job spec into a four-step HelixJobSpec."""
    transformed_spec = job_spec
    logger.info("Compiling Automodel job to HelixJobSpec: %s", transformed_spec.model_dump_json(indent=2))
    ...
    if training_recipe.value in ("bi_encoder", "cross_encoder") and (
        ...
        raise HelixJobCompilationError(
            "NeMo Helix does not support unmerged LoRA for embedding or cross-encoder models. "
