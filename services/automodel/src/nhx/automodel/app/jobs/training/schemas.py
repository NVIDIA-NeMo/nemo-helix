...
class LoRAConfig(BaseModel):
    ...
    use_memory_efficient_lora: Optional[bool] = Field(
        default=None, description="Use Automodel's lower-memory LoRA path. None keeps Automodel's default (on)."
    )
...
class MTPConfig(BaseModel):
    ...
    num_nextn_predict_layers: Optional[int] = Field(default=None, description="How many tokens ahead to predict")
    use_repeated_layer: Optional[bool] = Field(default=None, description="Share one weight-tied layer across depths")
    loss_scaling_factor: Optional[float] = Field(default=None, description="Weight of the MTP loss term")
