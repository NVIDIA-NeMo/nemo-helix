...
def s3_credentials(sdk: NeMoHelix) -> Iterator[tuple[str, str]]:
    ...
    secrets.create_secret(
        body=HelixSecretCreateRequest(name=secret_key_secret, value=SecretStr(S3_TEST_SECRET_KEY)),
        workspace=DEFAULT_WORKSPACE,
    )
...
class TestS3StorageBackend:
    ...
    def test_fileset_create_with_s3_storage(
        self, sdk: NeMoHelix, s3_test_bucket: str, s3_credentials: tuple[str, str]
    ):
    ...
    def test_invalid_credentials(self, sdk: NeMoHelix, s3_test_bucket: str):
        ...
        secrets.create_secret(
            body=HelixSecretCreateRequest(name=bad_secret_secret, value=SecretStr("invalid-secret")),
            workspace=DEFAULT_WORKSPACE,
        )
