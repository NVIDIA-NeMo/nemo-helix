...
:class:`~nemo_helix_plugin.inference_middleware.NemoInferenceMiddleware` plugins
...
class InferenceMiddlewareCacheAccessorImpl:
    """Concrete implementation of :class:`~nemo_helix_plugin.inference_middleware.InferenceMiddlewareCacheAccessor`.

    Wraps the IGW's in-memory :class:`~nhx.core.inference_gateway.api.model_cache.ModelCache`
    and :class:`~nhx.core.inference_gateway.api.virtual_model_cache.VirtualModelCache` to
    provide read-only platform state to inference middleware plugins.

    Injected into each :class:`~nemo_helix_plugin.inference_middleware.NemoInferenceMiddleware`
    instance before :meth:`~nemo_helix_plugin.inference_middleware.NemoInferenceMiddleware.on_startup`
    is called.
    """
    ...
    def get_model_entity(self, model_entity_id: str) -> Any | None:
        """Return the :class:`~nhx.core.inference_gateway.api.model_cache.ModelEntityInfo`
        for ``model_entity_id``, or ``None`` if not cached.

        :class:`ModelEntityInfo` structurally satisfies the ``ModelEntity`` Protocol from
        ``nemo_helix_plugin.inference_middleware`` — it exposes ``workspace``, ``name``,
...
class ResolvedMiddlewareCall:
    """A :class:`~nemo_helix_plugin.inference_middleware.MiddlewareCall` with its config
...
class PrefetchResult:
    ...
    - :attr:`missing` — the plugin signalled a definitive deletion by raising
      :class:`~nemo_helix_plugin.inference_middleware.MiddlewareConfigNotFoundError`.
    ...
    - :attr:`transient` — any other failure mode (unknown plugin, plugin not
      ...
      cached :attr:`~nhx.core.inference_gateway.api.virtual_model_cache.VirtualModelCache.config_ref_versions`
...
class MiddlewareRegistry:
    """Runtime registry of loaded :class:`~nemo_helix_plugin.inference_middleware.NemoInferenceMiddleware` plugins.
    ...
    async def resolve_configs_for_virtual_model(
        ...
        *,
        ...
        :func:`~nhx.core.inference_gateway.api.virtual_model_cache.refresh_virtual_model_cache`
    ...
    def evict(self, key: tuple[str, str]) -> None:
        ...
        (in :func:`~nhx.core.inference_gateway.api.virtual_model_cache.refresh_virtual_model_cache`)
...
async def execute_request_middleware(
    ...
    The chain short-circuits on the first :class:`~nemo_helix_plugin.inference_middleware.ImmediateResponse`
    ...
    Args:
        ...
        request: The initial :class:`~nemo_helix_plugin.inference_middleware.InferenceRequest`.

    Returns:
        Either the (possibly mutated) :class:`~nemo_helix_plugin.inference_middleware.InferenceRequest`,
        or an :class:`~nemo_helix_plugin.inference_middleware.ImmediateResponse` if a
        plugin short-circuited the chain.

    Raises:
        :class:`~nemo_helix_plugin.inference_middleware.InferenceMiddlewareError`:
...
async def execute_response_middleware(
    ...
    """Run the response middleware chain and return the final :class:`~nemo_helix_plugin.inference_middleware.InferenceResponse`.
    ...
    Args:
        ...
        ctx: Per-request context passed to each plugin.
        response: The initial :class:`~nemo_helix_plugin.inference_middleware.InferenceResponse`
            wrapping the backend result and response headers.

    Raises:
        :class:`~nemo_helix_plugin.inference_middleware.InferenceMiddlewareError`:
            Re-raised with the plugin's ``status_code`` intact.
        :class:`~nemo_helix_plugin.inference_middleware.InferenceMiddlewareError` (500):
            If a plugin raises an unexpected exception.
    """
