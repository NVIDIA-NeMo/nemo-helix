...
from nhx.core.jobs.controllers.backends.exceptions import FailedToScheduleError, JobStorageError
...
def map_pod_status_to_platform_status(pod_status: PodStatus) -> HelixJobStatus:
...
def aggregate_pod_statuses_for_job_step(pods: list[PodStatus]) -> tuple[HelixJobStatus, dict[str, Any]]:
    ...
    if HelixJobStatus.ERROR in per_pod:
        return HelixJobStatus.ERROR, {"message": "Job has pods in error state, check tasks for error details"}
    if HelixJobStatus.ACTIVE in per_pod:
        n_active = sum(1 for s in per_pod if s == HelixJobStatus.ACTIVE)
        return HelixJobStatus.ACTIVE, {
            "message": f"Job has active containers on {n_active}/{len(pods)} pod(s) (phase not Running)"
        }
    if all(s == HelixJobStatus.COMPLETED for s in per_pod):
        return HelixJobStatus.COMPLETED, {
...
def name_for_step(step: HelixJobStepWithContext) -> str:
...
def common_labels_for_step(step: HelixJobStepWithContext) -> dict[str, str]:
...
def create_configmap(core_v1: client.CoreV1Api, namespace: str, step: HelixJobStepWithContext) -> str:
...
def create_pod_template_spec(
    ...
    env.extend(
        [
            ...
            client.V1EnvVar(
                name=NHX_JOB_LAUNCHER_OTLP_LOGS_ENDPOINT_ENVVAR,
