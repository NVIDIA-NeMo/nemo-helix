...
"""
    ...
    def cancel_scheduling(
        self, step: HelixJobStepWithContext, *, created_container: Container | None = None
    ) -> bool:
...
class GPUDockerJobBackend(DockerJobBackend[GPUExecutionProvider]):
    ...
    def sync(
        ...
        status_value = (
            job_update.status.value if isinstance(job_update.status, HelixJobStatus) else job_update.status
        )
