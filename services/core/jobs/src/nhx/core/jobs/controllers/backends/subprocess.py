...
class SubprocessJobBackend(JobBackend[SubprocessExecutionProvider, SubprocessJobExecutionProfileConfig]):
    ...
    def schedule(self, executor_config: SubprocessExecutionProvider, step: HelixJobStepWithContext) -> JobUpdate:
        ...
        return JobUpdate(status=HelixJobStatus.PENDING, status_details=status_details)
    ...
    def sync(self, step: HelixJobStepWithContext) -> JobUpdate:
        ...
        if metadata is None:
            ...
            if step.status == HelixJobStatus.PENDING and not self._pending_step_missing_metadata_is_stale(step):
    ...
    def _get_task_fallback_update(self, step: HelixJobStepWithContext) -> JobUpdate | None:
        try:
    ...
    @staticmethod
    def _pending_step_missing_metadata_is_stale(step: HelixJobStepWithContext) -> bool:
    ...
    def _prepare_runtime(self, step: HelixJobStepWithContext) -> tuple[dict[str, str], str, Path, Path, Path]:
        ...
        otlp_logs_endpoint = get_logs_endpoint_config_from_fileset(
            ...
            artifact_base_path=step.artifact_base_path,
        )
    ...
    def _create_step_update(self, step: HelixJobStepWithContext, metadata: SubprocessProcessMetadata) -> JobUpdate:
    ...
    def _map_process_status(
        self, step: HelixJobStepWithContext, metadata: SubprocessProcessMetadata
    ) -> tuple[HelixJobStatus, dict, dict, str]:
        ...
        if exit_code is None:
            if step.status == HelixJobStatus.CANCELLING:
                return HelixJobStatus.CANCELLING, {"message": "Job is cancelling", **status_details}, {}, ""
            if step.status == HelixJobStatus.PAUSING:
                return HelixJobStatus.PAUSING, {"message": "Job is pausing", **status_details}, {}, ""
            return HelixJobStatus.ACTIVE, {"message": "Job is running", **status_details}, {}, ""
