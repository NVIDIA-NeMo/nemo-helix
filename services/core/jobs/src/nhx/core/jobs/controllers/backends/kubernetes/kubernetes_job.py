...
from nemo_helix_plugin.jobs.types import HelixJobStepWithContext, HelixJobTaskUpdate
from nhx.common.auth import get_workload_delegation_audience
...
class KubernetesJobBackend(JobBackend[ProviderT, KubernetesJobExecutionProfileConfig], Generic[ProviderT]):
    ...
    def create_step_update(self, step: HelixJobStepWithContext, job: V1Job) -> JobUpdate:
        ...
        if self._workload_delegations.should_manage_step(step):
            ...
            if target is not None:
                if status in HelixJobStatus.terminals():
    ...
    def sync_pending(self, step: HelixJobStepWithContext, job: V1Job | None) -> JobUpdate:
        if job is None:
    ...
    def _cancel_active_step_tasks(self, step: HelixJobStepWithContext) -> None:
        ...
        for task in tasks.data:
            if task.status != HelixJobStatus.ACTIVE:
                continue
    ...
    def sync_terminate_job(self, step: HelixJobStepWithContext, job: V1Job | None) -> JobUpdate:
        if job is None:
            ...
            if update.status == HelixJobStatus.CANCELLED:
                self._cancel_active_step_tasks(step)
            return update
...
def map_kubernetes_job_status_to_step_status(
    job: V1Job, core_v1: client.CoreV1Api, step: HelixJobStepWithContext
) -> tuple[HelixJobStatus, KubernetesStatusDetails]:
    ...
    if status.completion_time is not None:
        # Job is completed
        return HelixJobStatus.COMPLETED, {"message": f"Job has completed successfully at {status.completion_time}"}
        ...
        if status.conditions is not None:
            for condition in status.conditions:
                if condition.type == "Failed" and condition.status == "True":
                    return HelixJobStatus.ERROR, {"message": f"Job failed: {condition.message}"}
            cond_summary = [
    ...
    if is_cancelling:
        if is_active:
            return HelixJobStatus.CANCELLING, {"message": "Job is being cancelled"}
        else:
            return HelixJobStatus.CANCELLED, {"message": "Job is cancelled"}
    if is_suspended:
        if is_active:
            return HelixJobStatus.PAUSING, {"message": "Job is being paused"}
        else:
            return HelixJobStatus.PAUSED, {"message": "Job is paused"}

    if errored_pods:
        return HelixJobStatus.ERROR, {"message": "Job has errored pods, check tasks for error details"}
    if is_active:
        return HelixJobStatus.ACTIVE, {"message": f"Job is active with {len(running_pods)}/{len(pods)} running pods"}
    ...
    if pending_pods:
        return HelixJobStatus.PENDING, {"message": f"Job is pending with {len(pending_pods)}/{len(pods)} pending pods"}
    ...
    if not pods:
        ...
        return HelixJobStatus.PENDING, {"message": "Waiting for pods to be created for this job"}

    mapped_status, details = aggregate_pod_statuses_for_job_step(pods)
    if mapped_status == HelixJobStatus.PENDING:
        logger.warning(
    ...
    elif mapped_status == HelixJobStatus.COMPLETED:
        logger.info(
