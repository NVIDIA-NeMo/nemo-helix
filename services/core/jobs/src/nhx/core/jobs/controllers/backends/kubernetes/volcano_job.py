...
from nhx.core.jobs.controllers.backends.workload_tokens import create_authenticated_workload_delegation_store
...
class VolcanoJobBackend(
    ...
    def schedule(
        ...
        try:
            ...
            raise
        return JobUpdate(
            status=HelixJobStatus.PENDING, status_details={"message": "Job scheduled with Volcano backend"}
        )
    ...
    def cleanup_steps(self):
        ...
        for job in jobs:
            ...
            if self._execution_profile_config.cleanup_completed_jobs_immediately and status == HelixJobStatus.COMPLETED:
            ...
            if (
                ...
                and status == HelixJobStatus.COMPLETED
            ) or status == HelixJobStatus.ERROR:
    ...
    def sync_active(self, step: HelixJobStepWithContext, job: dict | None) -> JobUpdate:
        job_name = name_for_step(step)
        if job is None:
            logger.error("Job not found: %s", job_name)
            self._workload_delegations.revoke_by_key(namespace=self.namespace, name=job_name)
            # Job was deleted
            return JobUpdate(status=HelixJobStatus.ERROR, error_details={"message": "Job not found"})
        else:
            return self.create_step_update(step, job)

    def sync_pending(self, step: HelixJobStepWithContext, job: dict | None) -> JobUpdate:
        if job is None:
            # Job doesn't exist yet
            return JobUpdate(status=HelixJobStatus.PENDING, status_details={"message": "Job not yet created"})
        else:
            return self.create_step_update(step, job)
...
def map_volcano_job_status_to_platform_status(status: dict) -> tuple[HelixJobStatus, dict]:
    """Map Kubernetes job status to HelixJobStatus."""
    if status["state"]["phase"] == "Completed":
        return HelixJobStatus.COMPLETED, {"message": "Job completed successfully"}
    elif status["state"]["phase"] == "Failed":
        return HelixJobStatus.ERROR, {"message": "Job failed"}
    elif status["state"]["phase"] == "Running":
        return HelixJobStatus.ACTIVE, {"message": "Job is currently running"}
    else:
        return HelixJobStatus.PENDING, {"message": "Job is pending (not yet started)"}
