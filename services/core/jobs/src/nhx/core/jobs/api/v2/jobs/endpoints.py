...
from nhx.common.api.utils import generate_openapi_extra_params, parse_deep_object
...
from nhx.core.jobs.api.v2.jobs.schemas import (
    ...
    HelixJobsListFilter,
    HelixJobSortField,
    HelixJobStatusDetailsUpdateRequest,
...
from nhx.core.jobs.app.providers import CPUExecutionProvider, SubprocessExecutionProvider
from nhx.core.jobs.app.schemas import (
    HelixJobSpec,
)
from nhx.core.jobs.config import config, profiles
from nhx.core.jobs.entities import HelixJobStep, HelixJobTask
...
_JOB_STATUS_VALUES = ", ".join(job_status.value for job_status in HelixJobStatus)
...
def _format_job_compilation_error(exc: HelixJobCompilationError) -> str:
...
def get_platform_jobs_steps_list_filter(request: Request) -> HelixJobStepsListFilter:
    """Extract HelixJobStepsListFilter from request query parameters."""
    try:
        filters = parse_deep_object(name="filter", params=request.query_params) or {}
        return HelixJobStepsListFilter(**filters)
...
def validate_job_spec(
    ...
    except HelixJobCompilationError as exc:
...
@router.get(
    "/v2/workspaces/{workspace}/jobs",
    response_model=Page[HelixJobResponse],
    response_model_exclude_none=True,
    openapi_extra=generate_openapi_extra_params(
        filter_schema=HelixJobsListFilter,
...
async def list_jobs(
    ...
    sort: HelixJobListSortField = Query(
        default=HelixJobListSortField.CREATED_AT_DESC,
        description="The field to sort by. To sort in decreasing order, use `-` in front of the field name.",
    ),
    parsed: ParsedFilter = Depends(make_filter_dep(HelixJobsListFilter)),
    dispatcher: JobDispatcher = Depends(dep_dispatcher),
) -> Page[HelixJobResponse]:
...
async def update_job_status_details(
    name: str,
    request: HelixJobStatusDetailsUpdateRequest,
    workspace: str,
    dispatcher: JobDispatcher = Depends(dep_dispatcher),
) -> None:
...
async def create_job_result(
    ...
    name: str,
    workspace: str,
    request: HelixJobResultCreateRequest,
    dispatcher: JobDispatcher = Depends(dep_dispatcher),
) -> HelixJobResultResponse:
...
async def list_job_results(
    workspace: str,
    name: str,
    sort: HelixJobSortField = Query(
        default=HelixJobSortField.CREATED_AT_DESC,
        description="The field to sort by. To sort in decreasing order, use `-` in front of the field name.",
    ),
    dispatcher: JobDispatcher = Depends(dep_dispatcher),
) -> HelixJobListResultResponse:
    ...
    with scoped_app_ctx(JobContext(id=name)):
        ...
        return HelixJobListResultResponse(data=[r.to_response() for r in results])
...
@router.get(
    ...
    response_model=Page[HelixJobStepWithContext],
    response_model_exclude_none=True,
    openapi_extra=generate_openapi_extra_params(
        filter_schema=HelixJobStepsListFilter,
...
async def list_steps(
    ...
    sort: HelixJobSortField = Query(
        default=HelixJobSortField.CREATED_AT_ASC,
        description="The field to sort by. To sort in decreasing order, use `-` in front of the field name.",
    ),
    filter: HelixJobStepsListFilter = Depends(get_platform_jobs_steps_list_filter),
    dispatcher: JobDispatcher = Depends(dep_dispatcher),
) -> Page[HelixJobStepWithContext]:
...
async def update_job_step_status(
    job: str,
    name: str,
    request: HelixJobStatusUpdateRequest,
    workspace: str,
    dispatcher: JobDispatcher = Depends(dep_dispatcher),
) -> HelixJobStep:
...
async def list_job_step_tasks(
    ...
    with scoped_app_ctx(JobContext(id=job, step_name=name)):
        ...
        return HelixJobListTaskResponse(data=await dispatcher.list_tasks(step_entity.id, workspace=workspace))
