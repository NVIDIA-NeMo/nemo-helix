...
test_platform_admin_bypasses_scope_check if {
    ...
    with data.authz.principals as {"admin": {"workspaces": {"system": ["HelixAdmin"]}}}
...
# Test that a HelixAdmin's own Scoped Access Key scope restriction is honored.
...
test_platform_admin_scoped_access_key_denied_outside_its_scope if {
    ...
    with data.authz.principals as {"admin1": {"workspaces": {"system": ["HelixAdmin"]}}}
