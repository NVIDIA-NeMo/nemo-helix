...
from nhx.common.auth.authz_format import validate_static_authz_data
...
def merge_plugin_authz_contributions(static_data: dict) -> dict:
    """Overlay authorization rules from installed NeMo Helix plugins.
    ...
    try:
        from nemo_helix_plugin.authz_discovery import discover_plugin_authz
        from nhx.common.auth.authz_merge import merge_authz_contributions
    except ImportError:
...
async def _build_authorization_data_internal(entities_client: Optional[EntityClient] = None) -> dict:
    """Build authorization data for NeMo Helix.
    ...
    entities by the platform-seed task. See nhx.core.auth.app.seeding.
    ...
    # The assets directory is at the package level (nhx/auth/assets)
...
async def _build_opa_bundle_internal(entities_client: Optional[EntityClient] = None) -> bytes:
    """Build an OPA bundle for NeMo Helix authorization.
