...
# Main entry point - returns result with X-NHX-Authorized header
#
# Example input:
# {
...
#   "allowed": true,
#   "headers": {
#     "X-NHX-Authorized": "true"
#   }
# }
allow := result if {
	allow_request
	not deny_request
	result := {"allowed": true, "headers": {"X-NHX-Authorized": "true"}}
} else := result if {
	result := {"allowed": false, "headers": {"X-NHX-Authorized": "false"}}
}
...
# first so a HelixAdmin's own scoped Access Key stays scoped down.
allow_request if {
	...
	"HelixAdmin" in data.authz.principals[principal].workspaces.system
}
...
# the ServiceSystem "*" wildcard and the HelixAdmin bypass, so an un-annotated plugin route
...
# Deny direct secret value access for non-service principals (including HelixAdmin).
...
# principals and HelixAdmin (same as previous middleware: IAM paths stayed service-only).
...
# unaffected and stay allowed. A human HelixAdmin keeps its global bypass here: an admin
...
# True when any applicable principal has HelixAdmin in the system workspace (see allow_request).
