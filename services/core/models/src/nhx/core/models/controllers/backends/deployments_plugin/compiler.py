...
from nhx.common.auth import get_workload_identity_token_audience, is_workload_identity_token_exchange_enabled
...
from nhx.core.models.controllers.backends.deployments_plugin.naming import EntityNames, entity_names
...
from nhx.core.models.controllers.backends.engine import (
    ENGINE_GENERIC,
    ENGINE_NIM,
    ENGINE_VLLM,
    config_engine,
...
from nhx.core.models.controllers.backends.generic_compiler import (
    compile_generic_args,
    compile_generic_env_vars,
    resolve_generic_image,
)
from nhx.core.models.controllers.backends.vllm_compiler import (
    MODEL_STORE_PATH,
    VLLM_SERVER_PORT,
    compile_vllm_args,
...
# ~/.local/state), $XDG_DATA_HOME (default ~/.local/share, via nhx_user_data_dir()),
# and $XDG_CONFIG_HOME (default ~/.config). The pod runs it as the vLLM uid (2000),
# which does not own the nhx-api image's $HOME (/home/nvs, uid 1000), so those defaults
...
def _labels(resolved: ResolvedPluginDeployment, engine: str, role: str) -> dict[str, str]:
    return {
        ...
        "nhx.nvidia.com/deployment-workspace": resolved.deployment.workspace,
        "nhx.nvidia.com/deployment-name": resolved.deployment.name,
        "nhx.nvidia.com/models-role": role,
        "nhx.nvidia.com/engine": engine,
    }
...
def _lora_sidecar(
    ...
    *,
    ...
    ``NHX_BASE_URL`` must reach the host platform API from inside the container
    ...
    sidecar_env = {
        ...
        "NHX_MODEL_ENTITY_WORKSPACE": entity_workspace,
        "NHX_MODEL_ENTITY_NAME": entity_name,
        "NHX_BASE_URL": _container_reachable_platform_base_url(runtime=resolved.runtime),
