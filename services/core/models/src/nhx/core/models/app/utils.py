...
from nemo_helix_plugin.k8s_naming import (
...
from nemo_helix_plugin.models.types import ModelDeployment, ModelDeploymentConfig, ModelEntity, ModelProvider
from nhx.common.api.common import PaginationData
from nhx.common.entities.constants import NAME_PATTERN as ENTITY_NAME_PATTERN
...
class ModelWeightsType(str, Enum):
    ...
    FILES_SERVICE = "files_service"  # Weights from NeMo Helix Files service
...
def parse_model_name_revision(
    ...
    Args:
        model_namespace: Explicit namespace (optional) - refers to HuggingFace/NeMo Helix model source
...
def get_model_weights_type(
    ...
    # uses HF_ENDPOINT to talk to NeMo Helix Files, which resolves the fileset (e.g. to HF Hub).
    ...
    if is_multi_llm_image(image_name) and model_name:
        logger.debug("Detected Files service weights via multi-LLM: downloading from NeMo Helix Files service")
        return ModelWeightsType.FILES_SERVICE
