...
from nmp.core.models.parallelism.utils import default_chat_template, detect_reasoning_toggle
...
def test_template_branching_on_the_kwarg() -> None:
    assert detect_reasoning_toggle(OVERRIDABLE) is True


def test_template_that_hardcodes_the_kwarg_is_not_a_toggle() -> None:
    assert detect_reasoning_toggle(HARDCODED) is False


def test_template_without_the_kwarg() -> None:
    assert detect_reasoning_toggle(PLAIN) is False


def test_alternate_kwarg_spelling() -> None:
    assert detect_reasoning_toggle(DEEPSEEK_STYLE) is True
...
def test_named_templates_consult_only_the_default() -> None:
    ...
    assert detect_reasoning_toggle(templates) is False
...
def test_named_templates_without_a_default_are_undetermined() -> None:
    assert detect_reasoning_toggle([{"name": "tool_use", "template": PLAIN}]) is None


def test_mapping_of_named_templates() -> None:
    assert detect_reasoning_toggle({"default": OVERRIDABLE, "tool_use": PLAIN}) is True


def test_unrenderable_template_is_undetermined() -> None:
    assert detect_reasoning_toggle("{% if enable_thinking %}{% endfor %}") is None
