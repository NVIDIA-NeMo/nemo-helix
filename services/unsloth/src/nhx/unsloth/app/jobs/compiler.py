...
"""Job compiler — transforms ``UnslothJobOutput`` into a 4-step ``HelixJobSpec``.

Invoked from :meth:`UnslothJob.compile` via :mod:`nhx.unsloth.compile`.
...
from nhx.unsloth.app.constants import (
    DEFAULT_DATASET_PATH,
    DEFAULT_MODEL_PATH,
    DEFAULT_OUTPUT_MODEL_PATH,
    DEFAULT_VALIDATION_DATASET_PATH,
)
from nhx.unsloth.app.jobs.training.compiler import compile_training_step
...
async def platform_job_config_compiler(
    ...
    *,
    ...
    """Compile a canonical unsloth job spec into a 4-step ``HelixJobSpec``."""
    del job_name  # reserved for future scheduling decisions (e.g. naming jobs)

    logger.info(f"Compiling Unsloth job to HelixJobSpec: {job_spec.model_dump_json(indent=2)}")
