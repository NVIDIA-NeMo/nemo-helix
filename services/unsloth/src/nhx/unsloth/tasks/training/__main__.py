...
Reads a :class:`~nhx.unsloth.app.jobs.training.schemas.TrainingStepConfig`
from the platform Jobs envelope (``NEMO_JOB_STEP_CONFIG_FILE_PATH``)
and runs :func:`~nhx.unsloth.tasks.training.backends.unsloth_sft.train_sft`
against the paths the file_io download step populated.

The container ENTRYPOINT bakes::

    ENTRYPOINT ["/opt/venv/bin/python"]
    CMD ["-m", "nhx.unsloth.tasks.training"]
...
def main() -> int:
    ...
    if raw is None:
        sys.stderr.write(
            "nhx.unsloth.tasks.training requires NEMO_JOB_STEP_CONFIG_FILE_PATH. "
    ...
    from nhx.customization_common.service.path_utils import get_job_storage_path_from_env, remap_job_storage_path
    from nhx.unsloth.app.jobs.training.schemas import TrainingStepConfig
    from nhx.unsloth.tasks.training.backends.unsloth_sft import train_sft
