...
from nemo_helix_plugin.deployment import DEPLOYMENT_CONFIG_DESCRIPTION, DeploymentParams
...
class GRPOTraining(_TrainingBase):
    ...
    @model_validator(mode="after")
    def _router_is_not_recomputed_under_checkpointing(self) -> Self:
        # The platform sets ignore_router_for_ac=True whenever expert parallelism and
        # activation checkpointing are both on, because the recomputed router can route
        # differently and backward dies in torch.utils.checkpoint. The Automodel that
        # NeMo-RL ships defaults it to False without a warning, so an explicit False here
        # would bring that crash back.
        if (
            self.parallelism.expert_parallel_size > 1
            and self.activation_checkpointing
            and (self.moe_parallelizer or {}).get("ignore_router_for_ac") is False
        ):
            raise ValueError(
                "moe_parallelizer.ignore_router_for_ac=false cannot be combined with "
                "activation_checkpointing and expert_parallel_size > 1: recomputing the router "
                "can route tokens differently and crash the backward pass. Drop the override."
            )
        return self
