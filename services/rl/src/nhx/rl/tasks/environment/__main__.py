...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--upload",
        action="store_true",
        help="Upload env + dataset FileSets to the platform Files API (requires NHX_BASE_URL)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.upload:
        from nhx.rl.tasks.environment.upload import upload_converted_packages

        base_url = os.environ.get("NHX_BASE_URL") or os.environ.get("NHX_FILES_URL")
        if not base_url:
            logger.error("--upload requires NHX_BASE_URL or NHX_FILES_URL")
            return 2
        ...
        try:
            refs = upload_converted_packages(
                ...
                base_url=base_url,
                api_key=os.environ.get("NHX_API_KEY"),
            )
        except Exception as exc:
