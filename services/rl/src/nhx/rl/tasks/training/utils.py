...
from nhx.common.jobs.constants import NEMO_JOB_ID_ENVVAR
from nhx.rl.app.jobs.training.schemas import GPUInfo
...
def generate_torchrun_flags_from_env() -> list[str]:
    ...
    if gpus_per_node is None:
        try:
            ...
            # device_count() returns 0 when CUDA is unavailable; torchrun's
            # --nproc_per_node must be >= 1, so treat <= 0 as "unknown" and fall
            # back to 1 rather than emitting "0".
            device_count = torch.cuda.device_count()
            gpus_per_node = str(device_count) if device_count > 0 else "1"
        except Exception as e:
