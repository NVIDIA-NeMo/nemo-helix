...
from nhx.rl.app.jobs.training.schemas import BatchingStrategy, GRPOConfig, PolicyBackend, TrainingStepConfig
...
from nhx.rl.tasks.training.chat_templates import resolve_chat_template
from nhx.rl.tasks.training.datasets.preparation import compute_val_check_interval, prepare_dataset
from nhx.rl.tasks.training.datasets.validation import DatasetValidator
    ...
    if sandboxed:
        ...
        if not runtime_image:
            raise ValueError(
                ...
                "nhx-rl-training image."
            )
        pvc_claim = resolve_job_storage_pvc_claim()
        if not pvc_claim:
            raise ValueError(
                "Sandboxed GRPO requires the job-storage PVC claim name so the Gym sandbox "
                f"can mount the environment and dataset; {NHX_JOB_STORAGE_PVC_ENVVAR} is unset. "
                "Set NHX_RL_JOB_STORAGE_PVC_CLAIM on the rl service."
            )
