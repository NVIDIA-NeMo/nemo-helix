...
from nhx.rl.tasks.training.datasets.preparation import (
    PreparedDataset,
    compute_val_check_interval,
    prepare_dataset,
)
from nhx.rl.tasks.training.datasets.validation import DatasetValidator, detect_dpo_schema_name
from nhx.rl.tasks.training.integrations import (
    build_mlflow_config,
    build_wandb_config,
)
...
def _build_scheduler_config(
    ...
    if optimizer_type in (OptimizerType.ADAM_WITH_FLAT_LR, OptimizerType.ADAMW_WITH_FLAT_LR):
        # Flat LR. List form is required by NeMo-RL; empty milestones because
        # SequentialLR needs one fewer milestone than schedulers.
        return [
            {
