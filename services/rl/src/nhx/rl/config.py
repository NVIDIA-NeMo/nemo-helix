...
"""Configuration for the nhx-rl compiler and container tasks.

Modeled after :mod:`nhx.unsloth.config`. Environment variables use the
``NHX_RL_`` prefix and drive image resolution and the multi-node shared-storage
gate for the ``HelixJobSpec`` the plugin's ``RlJob.compile`` builds.
"""

from nhx.common.config import create_service_config_class, get_platform_config, get_service_config
from pydantic import Field


class RlConfig(create_service_config_class("rl")):  # type: ignore[misc]  # ty: ignore[unsupported-base]
    """Environment variables use the ``NHX_RL_`` prefix."""

    image_registry: str | None = Field(
        default=None,
        description=(
            "Registry host/path prefix for nhx-customizer-tasks and nhx-rl-training. "
            "Override via NHX_RL_IMAGE_REGISTRY; defaults to the platform's image registry."
        ),
    )
    ...
    gym_runtime_image: str | None = Field(
        ...
        description=(
            ...
            "Defaults to the nhx-rl-training image, which already carries NeMo-Gym and the "
    ...
    sandbox_resources: dict[str, str] | None = Field(
        ...
        description=(
            ...
            "Defaults to 2 CPU / 4Gi. The OpenSandbox SDK default (1 CPU / 2Gi) OOMKills a "
            "Gym server process per config entry plus its own Ray instance mid-rollout, "
            "surfacing as a proxy 502 rather than as a memory error. Raise further if the "
            "environment is larger. Operator-scoped: jobs cannot set it, since it spends "
            "cluster capacity."
        ),
    )
