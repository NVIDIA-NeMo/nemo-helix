...
from nemo_helix_sdk_tools.sdk.cli_generator.config import CLIConfig, get_cli_generator_root, get_templates_dir
from nemo_helix_sdk_tools.sdk.cli_generator.context_collectors.base import (
    build_path_params,
    promote_name_to_positional,
)
from nemo_helix_sdk_tools.sdk.cli_generator.context_collectors.create_collector import CreateContextCollector
from nemo_helix_sdk_tools.sdk.cli_generator.context_collectors.delete_collector import DeleteContextCollector
from nemo_helix_sdk_tools.sdk.cli_generator.context_collectors.get_collector import GetContextCollector
from nemo_helix_sdk_tools.sdk.cli_generator.context_collectors.update_collector import UpdateContextCollector
from nemo_helix_sdk_tools.sdk.cli_generator.docstring_parser import transform_query_to_cli
from nemo_helix_sdk_tools.sdk.cli_generator.generator import SimpleGenerator
from nemo_helix_sdk_tools.sdk.cli_generator.models import (
    Parameter,
...
from nemo_helix_sdk_tools.sdk.cli_generator.sdk_introspector import SDKMethod, SDKParameter
from nemo_helix_sdk_tools.sdk.cli_generator.type_formatter import format_type_for_help, get_type_schema
...
"""
    ...
    def test_multiline_imports_with_parentheses(self):
        ...
        template_output = """from nemo_helix_ext.cli.core.types import (
...
"""
        imports, code = self._extract_imports(template_output)
        assert len([i for i in imports if "nemo_helix_ext.cli.core.types" in i]) >= 1
...
class TestInitFileGeneration:
    def test_imports_subresources_without_annotations_package_attribute_collision(self, tmp_path: Path):
        ...
        target_dir = package_root / "nemo_helix_ext" / "cli" / "commands" / "api"
        for package_dir in [
            package_root / "nemo_helix_ext",
            package_root / "nemo_helix_ext" / "cli",
            package_root / "nemo_helix_ext" / "cli" / "commands",
            target_dir,
        ]:
            package_dir.mkdir(parents=True, exist_ok=True)
            (package_dir / "__init__.py").write_text("")

        core_dir = package_root / "nemo_helix_ext" / "cli" / "core"
        ...
        assert (
            '_cli_child_annotations = _importlib_import_module("nemo_helix_ext.cli.commands.api.intake.annotations")'
            in content
        )
        assert (
            '_cli_child_traces = _importlib_import_module("nemo_helix_ext.cli.commands.api.intake.traces")' in content
        )
        ...
        assert "from nemo_helix_ext.cli.commands.api.intake import annotations" not in content
        assert "app.add_typer(annotations.app" not in content

        saved_modules = {
            name: module
            for name, module in sys.modules.items()
            if name == "nemo_helix_ext" or name.startswith("nemo_helix_ext.")
        }
        sys.path.insert(0, str(package_root))
        try:
            for name in saved_modules:
                sys.modules.pop(name, None)
            module = importlib.import_module("nemo_helix_ext.cli.commands.api.intake")
...
class TestGetApiModules:
    def test_collects_top_level_metadata_by_importing_generated_modules(self, tmp_path: Path):
        generator = SimpleGenerator.__new__(SimpleGenerator)
        target_dir = tmp_path / "nemo_helix_ext" / "cli" / "commands" / "api"
...
""")
        ...
        for package_dir in [
            tmp_path / "nemo_helix_ext",
            tmp_path / "nemo_helix_ext" / "cli",
            tmp_path / "nemo_helix_ext" / "cli" / "commands",
            target_dir,
        ]:
            (package_dir / "__init__.py").parent.mkdir(parents=True, exist_ok=True)
            (package_dir / "__init__.py").write_text("")

        core_dir = tmp_path / "nemo_helix_ext" / "cli" / "core"
        core_dir.mkdir(parents=True, exist_ok=True)
        (core_dir / "__init__.py").write_text("")
        (core_dir / "help_formatter.py").write_text(
            "\n".join(
...
""")
        ...
        assert "from nemo_helix_ext.jobs.watch import watch_job" not in rendered
        assert (
            "from nemo_helix_ext.cli.core.job_watch_renderer import JobWatchRenderResult, render_job_watch_events"
            in rendered
        )
        assert "from nemo_helix_ext.cli.core.waiters import wait_for_platform_job" not in rendered
        assert "from nemo_helix_plugin.client.adapter import client_from_platform" in rendered
        assert "from nemo_helix_plugin.jobs.client import JobsClient" in rendered
...
""")
        ...
        assert "from nemo_helix_ext.jobs.watch import watch_job" not in rendered
        assert "from nemo_helix_ext.cli.core.waiters import wait_for_platform_job" in rendered
        assert (
            "from nemo_helix_ext.cli.core.job_watch_renderer import JobWatchRenderResult, render_job_watch_events"
            in rendered
        )
...
""")
        ...
        assert "from nemo_helix_ext.cli.core.waiters import wait_for_inference_deployment" in rendered
        assert "from nemo_helix_ext.jobs.watch import watch_job" not in rendered
        assert (
