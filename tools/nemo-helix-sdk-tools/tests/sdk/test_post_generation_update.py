...
import pytest
import tomlkit
from nemo_helix_sdk_tools.sdk import post_generation_update
from nemo_helix_sdk_tools.sdk.core.common import SdkInfo


def _sdk_info(tmp_path: Path) -> SdkInfo:
    return SdkInfo(
        sdks_root_dir=tmp_path / "sdk",
        package_name="nemo-helix-sdk",
        directory_name="nemo-helix",
        module_name="nemo_helix",
        sdk_dir=tmp_path / "sdk/python/nemo-helix",
        overrides_dir=tmp_path / "sdk/python/overrides/nemo-helix",
        readme_dir=tmp_path / "sdk/python/overrides/nemo-helix/README",
...
# [Auth](src/nemo_helix/resources/auth/api.md)

# [Unrelated](src/nemo_helix/resources/unrelated/api.md)
""".lstrip(),
        encoding="utf-8",
    )
    ...
    assert "src/nemo_helix/resources/files/api.md" in api_index
    assert "src/nemo_helix/resources/auth/api.md" not in api_index
    assert "src/nemo_helix/resources/unrelated/api.md" in api_index
...
def test_ensure_source_owned_type_aliases_writes_compatibility_modules(tmp_path: Path) -> None:
    ...
    assert "from .shared import HelixJobStatusResponse as HelixJobStatusResponse" in types_init
    assert "from .platform_job_status_response import HelixJobStatusResponse as HelixJobStatusResponse" in shared_init
    assert "HelixJobStatusResponse as HelixJobStatusResponse" in (
        shared_dir / "platform_job_status_response.py"
    ).read_text(encoding="utf-8")
    assert "HelixJobStepResponse as HelixJobStep" in (types_dir / "jobs" / "__init__.py").read_text(encoding="utf-8")
    assert "HelixSecretResponse as HelixSecretResponse" in (
