...
def test_ensure_source_owned_type_aliases_writes_compatibility_modules(tmp_path: Path) -> None:
    ...
    assert (
        "from .platform_job_status_response import HelixJobStatusResponse as HelixJobStatusResponse"
        in shared_init
    )
    assert "HelixJobStatusResponse as HelixJobStatusResponse" in (
        shared_dir / "platform_job_status_response.py"
    ).read_text(encoding="utf-8")
    assert "HelixJobStepResponse as HelixJobStep" in (types_dir / "jobs" / "__init__.py").read_text(
        encoding="utf-8"
    )
