...
"""The published ``nemo-helix`` wheel must expose every ``nemo.*`` entry point its bundled packages declare.
...
def test_bundled_nemo_entry_points_are_exposed_by_the_wrapper(bundle_name: str, pyproject: Path) -> None:
    ...
    assert not missing, (
        f"{bundle_name} declares nemo.* entry points the nemo-helix wheel does not expose: {missing}. "
