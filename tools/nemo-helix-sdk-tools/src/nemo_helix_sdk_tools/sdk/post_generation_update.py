...
"""
Post-generation update tool for NeMo Helix SDK.
...
import tomlkit
import typer
from nemo_helix_sdk_tools.sdk.core.common import WRAPPER_DISTRIBUTION_NAME, SdkInfo, get_sdk_info
from nemo_helix_sdk_tools.sdk.post_generation_exist_ok import inject_exist_ok
from nemo_helix_sdk_tools.sdk.source_owned_resources import SOURCE_OWNED_RESOURCE_EXCLUSIONS
from tomlkit.items import AoT, Table

app = typer.Typer(
    name="post-generation", help="Post-generation update tool for NeMo Helix SDK.", no_args_is_help=True
)

SDK_BUILD_SOURCE_PACKAGES: tuple[dict[str, str | list[str]], ...] = (
    {
        "source": "packages/nemo_helix_ext/src/nemo_helix_ext",
        "target": "nemo_helix_ext",
        "include": [
            "**/*.py",
            "skills/**/*.md",
            "skills/**/*.yaml",
...
SOURCE_OWNED_JOB_STATUS_TYPE_EXPORTS: tuple[tuple[str, str], ...] = (
    ("platform_job_status_response", "HelixJobStatusResponse"),
    ("platform_job_step_status_response", "HelixJobStepStatusResponse"),
    ("platform_job_task_status_response", "HelixJobTaskStatusResponse"),
)
SOURCE_OWNED_SECRET_TYPE_EXPORTS: tuple[tuple[str, str], ...] = (
    ("platform_secret_access_response", "HelixSecretAccessResponse"),
    ("platform_secret_admin_rotation_response", "HelixSecretAdminRotationResponse"),
    ("platform_secret_create_request", "HelixSecretCreateRequest"),
    ("platform_secret_response", "HelixSecretResponse"),
    ("platform_secret_responses_page", "HelixSecretResponsesPage"),
    ("platform_secret_update_request", "HelixSecretUpdateRequest"),
...
from nemo_helix_plugin.secrets.types import (
    ...
    HelixSecretResponse as HelixSecretResponse,
    HelixSecretCreateRequest as HelixSecretCreateRequest,
    HelixSecretResponsesPage as HelixSecretResponsesPage,
    HelixSecretUpdateRequest as HelixSecretUpdateRequest,
    HelixSecretAccessResponse as HelixSecretAccessResponse,
    HelixSecretAdminRotationResponse as HelixSecretAdminRotationResponse,
)
"""
...
from nemo_helix_plugin.secrets.compat import (
...
def sdk_version_file_content(distribution_name: str) -> str:
    ...
    ``nemo-helix`` wheel, so ``nemo-helix-sdk`` metadata only exists in
...
def get_string_replacements() -> List[Tuple[str, str]]:
    ...
    stainless_sdk_url = "https://www.github.com/stainless-sdks/nemo-helix-python"
...
def ensure_source_owned_type_aliases(sdk_info: SdkInfo) -> bool:
    ...
    for module_name, type_name in SOURCE_OWNED_JOB_STATUS_TYPE_EXPORTS:
        ...
        module_content = _source_owned_type_module_content("nemo_helix_plugin.jobs.schemas", type_name)
    ...
    for module_name, type_name in SOURCE_OWNED_SECRET_TYPE_EXPORTS:
        ...
        module_content = _source_owned_type_module_content("nemo_helix_plugin.secrets.types", type_name)
        if not module_path.exists() or module_path.read_text(encoding="utf-8") != module_content:
            module_path.write_text(module_content, encoding="utf-8")
            changed = True
...
def update_pyproject_toml(sdk_info: SdkInfo) -> bool:
    ...
    # Match the supported range of the nemo-helix wheel this SDK ships in.
    ...
    hatch_version.clear()
    hatch_version["source"] = "nhx-dynamic-versioning"
    ...
    try:
        ...
        pass
    if "nhx-build-tools" not in build_requires:
        build_requires.append("nhx-build-tools")
    ...
    nhx_build_tools_source = tomlkit.inline_table()
    nhx_build_tools_source["workspace"] = True
    uv_config.setdefault("sources", {})["nhx-build-tools"] = nhx_build_tools_source
    ...
    wheel_target["packages"] = ["src/nemo_helix"]
...
def save_nhx_context() -> None:
    ...
    nhxcontext_dir = sdk_info.sdk_dir / ".nhxcontext"
    nhxcontext_dir.mkdir(parents=True, exist_ok=True)

    shutil.copy(sdk_info.openapi_spec_file, nhxcontext_dir / "openapi.yaml")
    shutil.copy(sdk_info.stainless_config_file, nhxcontext_dir / "stainless.yaml")

    typer.echo(f"  - Copied to {nhxcontext_dir}")
