...
GENERATED_ALIAS_INIT_TEMPLATE = (
    ...
    + """
from nemo_helix._alias import alias_package as _alias_package
...
_CLIENT_CLASS_NAMES = ("NeMoHelix", "AsyncNeMoHelix")
...
_CLIENT_INIT_REQUIRED_IMPORTS: dict[str, tuple[str, ...]] = {
    "nemo_helix._base_client": ("DefaultAsyncHttpxClient", "DefaultHttpxClient"),
    "nemo_helix_plugin.client.constants": ("WORKLOAD_IDENTITY_TOKEN_FILE_ENVVAR",),
    "nemo_helix_plugin.client.tls": ("client_verify_from_env",),
    "nemo_helix_plugin.jobs.client": ("AsyncJobsClient", "JobsClient"),
    "nemo_helix_plugin.secrets.compat": ("AsyncSecretsResource", "SecretsResource"),
    "pathlib": ("Path",),
}
_STALE_CLIENT_INIT_IMPORTS: dict[str, tuple[str, ...]] = {
    "nemo_helix.client.tls": ("client_verify_from_env",),
    "nemo_helix_ext.client.tls": ("client_verify_from_env",),
}
...
SDK_PATH = NHX_ROOT_PATH / "sdk/python/nemo-helix"
...
def _load_package_config(package: str) -> dict:
    ...
    search_roots = [NHX_ROOT_PATH / "packages", NHX_ROOT_PATH / "services"]
    ...
    if len(matching_configs) > 1:
        matches = ", ".join(str(path.relative_to(NHX_ROOT_PATH)) for path in matching_paths)
...
def _vendor_package_files(config: dict) -> None:
    ...
    all_module_rewrites = []
    for module in modules_to_vendor:
        target_module = module["module_name"] if top_level else f"nemo_helix.{module['module_name']}"
...
def _vendor_package_metadata(config: dict) -> None:
    ...
    for package_name in ("nemo-helix", "nemo-helix-sdk"):
...
def _build_and_validate_package_path(
    ...
    package_root_path = NHX_ROOT_PATH / Path(package_root)
...
def _build_and_validate_target_paths(sdk_path: Path, target_sdk_module: str, *, top_level: bool = False) -> Path:
    ...
    if top_level:
        ...
        target_root = sdk_path / "src/nemo_helix"
...
def _vendor_transitive_dependencies(
    ...
    for dep_config in included_transitive_dependencies:
        ...
        target_module_full = target_sdk_module if top_level else f"nemo_helix.{target_sdk_module}"
...
def _replace_client_methods(
    *,
    ...
    target_path = sdk_path / "src" / "nemo_helix" / "_client.py"
    ...
    # ``nemo_helix``. Keep bootstrap imports pointed at that real package so
    ...
    if remove_vendored_source_relative_path:
        vendored_source_path = sdk_path / "src" / "nemo_helix" / Path(remove_vendored_source_relative_path)
...
def _vendor_scripts(
    ...
    Example: nhx = "nemo_helix.cli.app:cli"
    """
    for target_path in _pyproject_target_paths(sdk_path):
        pyproject_path = target_path / "pyproject.toml"
        if not pyproject_path.exists():
