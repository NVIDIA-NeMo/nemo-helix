...
| Kubernetes clusters | minikube, kind, EKS, AKS, GKE, OKE, OpenShift, and on-prem Kubernetes | See [Install NeMo Platform with Helm](/documentation/kubernetes-deployment/setup/helm) for prerequisites and cluster-specific notes. |

## Local Runtime

| Area | Supported | Notes |
|------|-----------|-------|
| Package installation | `uv>=0.10.10` recommended for source checkouts; `pip` supported for published packages | The one-line installer sets up `uv` automatically. Published packages can also be installed with `pip install "nemo-platform[all]"`. |
