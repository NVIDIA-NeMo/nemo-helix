...
This matrix defines the supported OSS targets for NeMo Helix. It covers the
Python package, CLI, SDK, local services started by `nemo setup`, and
self-managed Kubernetes deployments installed with the NeMo Helix Helm chart.
...
| Python | 3.12, 3.13, 3.14 | Use an isolated virtual environment. Python 3.12 is the lowest supported Python version and Python 3.14 is the highest supported Python version. |
...
| Kubernetes clusters | minikube, kind, EKS, AKS, GKE, OKE, OpenShift, and on-prem Kubernetes | See [Install NeMo Helix with Helm](/documentation/kubernetes-deployment/setup/helm) for prerequisites and cluster-specific notes. |

## Local Runtime

| Area | Supported | Notes |
|------|-----------|-------|
| Package installation | `uv>=0.10.10` recommended for source checkouts; `pip` supported for published packages | The one-line installer sets up `uv` automatically. Published packages can also be installed with `pip install "nemo-helix[all]"`. |
...
- Python earlier than 3.12 or Python 3.15 and later.
