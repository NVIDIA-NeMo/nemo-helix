...
uses `nvidia/nemotron-3-ultra-550b-a55b` for artifact extraction, Q&A
...
The examples below use `nvidia/nemotron-3-nano-30b-a3b` because it is the
model most Platform deployments already serve and it is cheaper and faster.
Copying them as-is produces shallower queries and lower judge scores than the
recipe, and a weaker fine-tune than NVDocs. Use the 550B model when the
provider serves it. Downgrade only when median quality scores stay below 7/10,
queries are shallow, or cost and latency dominate; then run a 50–100 document
pilot and compare Stage 4 nDCG on a frozen `eval_beir` split before adopting
the smaller model.
...
On **release 0.7 and later**, pass that fileset to both downstream consumers
without repackaging it. Automodel and `retrieve-eval` resolve
`workspace/fileset#path`, so the Stage 1 `artifacts` result can be the
`dataset.training` and retrieve-eval `dataset` as-is. Current Stage 1 also
unique-keys `eval_beir/qrels/test.tsv` before writing it.
...
On **release 0.6**, Automodel `dataset.training` and retrieve-eval `dataset`
take a fileset name with no path fragment, and Stage 1 can emit duplicate
qrels. Copy `training.jsonl` and `eval_beir/` to the root of a dataset
fileset, and drop duplicate `(query-id, corpus-id)` rows from
`eval_beir/qrels/test.tsv` before upload. The copy-and-dedup cell in
[Embedding Model Customization](/documentation/customizer-reference/tutorials/embedding-customization-job)
is that 0.6 procedure.
