...
Pass that fileset to both downstream consumers without repackaging it. Automodel and `retrieve-eval` resolve `workspace/fileset#path`, so the Stage 1 `artifacts` result can be the `dataset.training` and retrieve-eval `dataset` as-is. Stage 1 unique-keys `eval_beir/qrels/test.tsv` before writing it.

Copy `training.jsonl` and `eval_beir/` to a fileset root only if you want that layout, or if you are uploading an older dump: drop duplicate `(query-id, corpus-id)` rows from `eval_beir/qrels/test.tsv` because the BEIR loader still rejects them. The copy-and-dedup cell in
[Embedding Model Customization](/documentation/customizer-reference/tutorials/embedding-customization-job)
is that optional procedure.
