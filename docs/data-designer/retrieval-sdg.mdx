...
a platform model entity whose fileset is downloaded by `nhx-customizer-tasks` into
the shared job storage. `nhx-automodel-training` then loads the encoder and tokenizer
from that local directory with Hugging Face networking disabled. Conversion-only
prepare stays on `nhx-cpu-tasks`.
