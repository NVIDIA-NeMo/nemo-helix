...
The [Nemotron embed recipe](https://github.com/NVIDIA-NeMo/Nemotron/blob/main/docs/nemotron/embed/README.md) generates and judges Q&A with `nvidia/nemotron-3-ultra-550b-a55b`. That model is a first-order driver of query quality and of the nDCG you get after fine-tuning. Platform has no default: Stage 0 calls whichever chat model Inference Gateway serves. The cells below default to `nvidia/nemotron-3-nano-30b-a3b` because it is commonly already deployed and cheaper to run. Expect shallower queries and lower quality scores than NVDocs, which was generated with the 550B pipeline. Point `RETRIEVAL_CHAT_MODEL` at the 550B served name when the provider has it. Downgrade only after a 50–100 document pilot if median quality scores stay below 7/10 or cost and latency dominate. See the [Retrieval SDG reference](/documentation/design-synthetic-data/retrieval-sdg) for corpus requirements, runtime estimates, and controls for chunk size, pair count, artifact coverage, multi-document bundling, concurrency, and the Stage 1 quality threshold. This notebook stops after Stage 0; mining and fine-tuning continue in the embedding tutorial.
