...
nemo chat -m nvidia/llama-3.3-nemotron-super-49b-v1 'Hello!' \
...
nemo chat -m meta/llama-3.2-1b-instruct 'Hello!' \
...
nemo chat -m default/qwen-2-5-1-5b 'Hello!' \
...
nemo files filesets create "qwen3-1-7b" \
  --storage '{
    "type": "huggingface",
...
# Register a model entity referencing the fileset
nemo models create "qwen3-1-7b" \
  --fileset "default/qwen3-1-7b"

# Create deployment config with engine "vllm"
nemo inference deployment-configs create "qwen3-vllm-config" \
  --engine "vllm" \
  --model-spec '{
    "model_namespace": "default",
...
nemo inference deployments create "qwen3-vllm-deployment" \
  --config "qwen3-vllm-config"

nemo wait inference deployment qwen3-vllm-deployment

nemo chat -m default/qwen3-1-7b 'Hello!' \
  --provider qwen3-vllm-deployment \
  --max-tokens 100
```
...
# Register a model entity referencing the fileset
client.models.create(name="qwen3-1-7b", fileset="default/qwen3-1-7b")

# Create deployment config with engine "vllm"
config = client.inference.deployment_configs.create(
    name="qwen3-vllm-config",
    engine="vllm",
    model_spec={
        "model_namespace": "default",
        "model_name": "qwen3-1-7b",
...
deployment = client.inference.deployments.create(
    name="qwen3-vllm-deployment", config="qwen3-vllm-config"
)

client.models.wait_for_status(
    deployment_name="qwen3-vllm-deployment", desired_status="READY"
)

response = client.inference.gateway.provider.post(
    "v1/chat/completions",
    name="qwen3-vllm-deployment",
    body={
...
```bash
# Base model fileset + entity (skip if already created above)
nemo files filesets create "qwen3-1-7b" \
  --storage '{
    "type": "huggingface",
    "repo_id": "Qwen/Qwen3-1.7B",
...
# Fileset for the LoRA adapter weights
nemo files filesets create "linear-algebra-lora" \
  --storage '{
    "type": "huggingface",
    "repo_id": "premjatin/qwen-linear-algebra-coder",
    "repo_type": "model"
  }'

# Deployment config with LoRA enabled.
# Set --max-lora-rank to the highest adapter rank you intend to serve.
nemo inference deployment-configs create "qwen3-lora-config" \
  --engine "vllm" \
  --model-spec '{
    "model_namespace": "default",
    "model_name": "qwen3-1-7b",
  ...
  --executor-config '{
    "gpu": 1,
    "additional_args": ["--max-lora-rank", "16"]
  }'

nemo inference deployments create "qwen3-lora-deployment" \
  --config "qwen3-lora-config"

nemo wait inference deployment qwen3-lora-deployment

# Register the adapter against the base model entity.
# The adapter is served under the name "<workspace>--<adapter-name>".
nemo models adapters create "qwen3-1-7b" "linear-algebra" \
  --fileset "default/linear-algebra-lora" \
  --finetuning-type lora \
  --enabled

# Target the adapter by its served name in the request body
nemo inference gateway provider post v1/chat/completions qwen3-lora-deployment \
  --body '{
    "model": "default--linear-algebra",
    "messages": [{"role": "user", "content": "Find the determinant of [[2, 1], [1, 3]]."}],
    "max_tokens": 256
  }'
```
...
deployment = client.inference.deployments.create(
    name="qwen3-lora-deployment", config="qwen3-lora-config"
)

client.models.wait_for_status(
    deployment_name="qwen3-lora-deployment", desired_status="READY"
)
...
response = client.inference.gateway.provider.post(
    "v1/chat/completions",
    name="qwen3-lora-deployment",
    body={
        "model": "default--linear-algebra",
        "messages": [
            {"role": "user", "content": "Find the determinant of [[2, 1], [1, 3]]."}
        ],
        "max_tokens": 256,
    },
)
...
nemo chat -m default/qwen-2-5-14b 'Hello!' \
--max-tokens 100
```
...
config = client.inference.deployment_configs.create(
    name="qwen-14b-config",
    engine="nim",
    model_spec={"model_name": "default/qwen-2-5-14b"},
...
| **Deployment config (`model_spec`)** | Override | Per-deployment overrides — useful for A/B testing or deployment-specific behavior |
...
flowchart LR
 ...
 DC["Deployment Config<br/>model_spec"] -->|override| ENV
```
...
config = client.inference.deployment_configs.create(
    name="llama-tool-config-v2",
    engine="nim",
    model_spec={"model_name": "default/llama-3-2-1b-tool-v2"},
...
To enable it, set `models.tool_call_plugin.enabled: true` in the platform configuration and ensure the user has the `models.tool-call-plugin.set` permission (granted to Admin and HelixAdmin roles by default).
