...
title: "NeMo Helix Skills Spec"
description: ""
---

This document defines the conventions for skills shipped with NeMo Helix: how they're structured, what their frontmatter must contain, how they get tested, and where they live in the repo.

---

## TL;DR: what are we actually solving for?

NeMo Helix is positioned as "the toolkit for safer, more accurate, cheaper agents." Two distinct populations of users hit that pitch with very different starting points:

1. **The user who doesn't have an agent yet.** They have a *problem* and a *use case*. They want NeMo Helix to take them from idea → deployed working agent.
2. **The user who already has a deployed agent.** They have *metrics that aren't good enough*. They want NeMo Helix to make their agent measurably better.
...
Entry point: a coding agent (Claude Code, Cursor, Codex, OpenCode) opened inside a freshly installed NeMo Helix repo. The user has just run `make bootstrap` and `nemo setup` in their shell.
...
| 2 | `nemo-build-agent` | Builds a LangChain Deep Agent through Fabric and deploys |
| 3 | `nemo-try-agent` | Sends queries to the deployed agent |
| 4 | `nemo-status` | Read-only platform health dashboard |
| 5 | `nemo-teardown` | Guided shutdown |
...
Entry point: a coding agent opened inside a NeMo Helix repo where they have a deployed agent. Metrics aren't where they want them.
...
| Step | Skill | What it does |
|---|---|---|
| 1 | `nemo-helix-overview` | Map of surfaces, plugins, and which skill owns each capability |
...
| `allowed-tools` | Yes | Comma-separated string of tool names this skill is permitted to use (e.g. `Bash, Read, Write, Edit`). Real safety signal: the agent harness honors this. |
| `metadata.author` | Yes | Who to contact about this skill (e.g. `NeMo Helix Team <nemo-helix@nvidia.com>`). Enforced by `.github/scripts/check_skill_frontmatter.py` in CI. |
...
**Canonical location:** `packages/nemo_helix_ext/src/nemo_helix_ext/skills/<name>/`. Skills there ship with `pip install "nemo-helix[all]"`.
...
description: >
  Build and deploy a LangChain Deep Agent through Fabric from confirmed requirements.
  Generates the agent YAML, creates the agent, deploys it, optionally
  wires guardrails and an eval suite. Use when the user says "build me an
  agent," "implement these requirements," or "deploy my agent."
triggers:
  - build the agent
...
compatibility: nemo-helix >= 0.1.0; macOS or Linux
maturity: active
license: Apache-2.0
user-invocable: true
...
- User-invocable: `packages/nemo_helix_ext/src/nemo_helix_ext/skills/<name>/` (ships with the platform package).
...
- `allowed-tools` (comma-separated string of tools the skill expects to invoke)
- `metadata.author` (who to contact about this skill)
...
- **Contextual:** user describes the intent with surrounding situation. `"I have an Ethos at agents/calculator-ethos/ETHOS.md and a working dev cluster. Take it from here."`
...
```bash
.venv/bin/python scripts/skill-cli-lint.py --root packages/nemo_helix_ext/src/nemo_helix_ext/skills/<your-skill>
