...
description: "Anonymous telemetry collected by NeMo Helix and bundled NeMo libraries, and how to opt out."
---

NeMo Helix and its bundled library plugins include optional anonymous telemetry
...
You can opt out at any time. When you run components through NeMo Helix, the
...
| --- | --- | --- |
| NeMo Helix | Random session identifier stored in local platform state and rotated every 30 days. If the state file has no creation date, an invalid creation date, or an expired creation date, NeMo Helix creates a new identifier. | Session identifier; onboarding step name, outcome, and invocation method; CLI command name, duration, and outcome; job type, duration, plugin references, model-data present/absent bucket, and token counts when available; CI flag; coding-agent flag; package version; CPU architecture. |
...
Disable NeMo Helix telemetry with any one of these controls:
...
`telemetry_enabled: false` goes in `~/.config/nhx/config.yaml`. The
`--no-telemetry` flag disables telemetry for one CLI invocation.

For CLI-specific behavior, see [CLI configuration](/documentation/reference/cli-reference/configuration#telemetry).

### Data Designer

When you use Data Designer through NeMo Helix, use the platform controls
...
When you use Anonymizer through NeMo Helix, use the platform controls above.
...
When you use Safe Synthesizer through NeMo Helix, use the platform controls
...
NeMo Helix and bundled library plugins can use third-party inference endpoints
