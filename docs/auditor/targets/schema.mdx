...
This page lists every field on `AuditTarget`. Defaults match the pydantic definition in `nemo_auditor.entities`; the NeMo Helix entity store validates writes against this schema.
...
| `model` | `str` | Yes | Provider model identifier (for example, `nvidia/nemotron-3.5-lightning-30b-a3b`). Some generator types (`test.Blank`) ignore this field but the schema still requires it. |
...
The plugin recognizes one sentinel inside `options.<generator>`: an `nhx_uri_spec` block is resolved at run time to a concrete `uri` value via the NeMo Helix Inference Gateway. See [Inference Gateway](/documentation/vulnerability-scanning/targets/inference-gateway) for the resolution rules and conflict semantics.
