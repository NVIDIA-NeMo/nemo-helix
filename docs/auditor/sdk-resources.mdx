...
title: "NeMo Auditor NeMo Helix SDK Resources"
description: ""
---
<a id="auditor-nhx-sdk-resources"></a>

The NeMo Auditor plugin mounts a Python SDK surface on the `nemo_helix` client at `client.auditor`.
...
| `get_job_status()` | Fetches only the current platform status string. | `HelixJobStatus \| None` |
...
The `AsyncAuditorPluginResource` provides the same surface for `AsyncNeMoHelix`.
