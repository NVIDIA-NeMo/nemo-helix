...
- Set the `NHX_BASE_URL` environment variable to your NeMo Helix endpoint

```bash
export NHX_BASE_URL="https://your-nhx-base-url"
```
...
# Get job status (use the job name returned at submission)
...
```bash
curl -X GET \
  "${NHX_BASE_URL}/apis/jobs/v2/workspaces/default/jobs/automodel-a1b2c3d4e5f6/status" \
