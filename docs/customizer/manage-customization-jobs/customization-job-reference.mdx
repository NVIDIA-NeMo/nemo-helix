...
- The job registers a **new model entity** (`all_weights`, or a LoRA job with a merged save method) and the referenced config names a different model. Retraining is fine, because the config already names that entity.
...
To reuse engine and executor settings across jobs that register a new model entity, use an unbound template — a config with neither `model_entity_id` nor `model_spec.model_name`. It is accepted by any job and bound to the trained model at deploy time.
