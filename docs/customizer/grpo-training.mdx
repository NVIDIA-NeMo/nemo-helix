...
GRPO runs on the `rl` backend, the same one that runs DPO, and requires a NeMo Helix configured with `platform.runtime: kubernetes` — it provisions a Ray cluster and has no local Docker fallback.
...
`adapter-wheels-v1` can run without network access at job start. Its converter builds
source-only dependencies such as `verifiers` into wheels, and the image-bundled harness
uses a version requirement that can resolve from that wheelhouse. Conversion still
requires an internet-connected host.

Egress is an operator setting, not a job field: `NHX_RL_SANDBOX_ALLOW_INTERNET`, plus `NHX_RL_SANDBOX_PUBLIC_DNS_ALLOW` for hosts outside the built-in `*.com` / `*.org` allowance. Ask your platform operator which is configured before choosing a format.
...
Add `--upload` (with `NHX_BASE_URL` set) to create both FileSets and upload them in the same command.
...
## Deploy automatically when the job finishes

Set `deployment_config` on the job spec and the platform deploys once training completes, instead of leaving you to create the deployment by hand. Omit it and nothing is deployed.

Pass a string to reuse an existing `ModelDeploymentConfig`:
...
```json
{
  ...
  "deployment_config": "default/qwen3-base-cfg"
}
```

Or an object to have one created inline:
...
| Field | Default | Description |
| --- | --- | --- |
| `gpu` | `1` | GPUs the deployment requests. |
| `lora_enabled` | `true` | Whether the deployment can load LoRA adapters. |
| `image_name` | multi-llm NIM | Container image from NGC. |
| `image_tag` | image default | Container image tag. |
| `disk_size` | unset | Disk size for the deployment. |
| `additional_envs` | unset | Extra environment variables. |
| `tool_call_config` | unset | Tool-calling overrides for the NIM. Setting `tool_call_plugin` requires the `models.tool-call-plugin.set` permission. |

A LoRA job deploys the **base** model with `lora_enabled: true` and serves the adapter from it, so `lora_enabled: false` on a LoRA job is rejected at submit — that deployment could never load the adapter. A full-weight job deploys the new model entity it registers.

Bad configurations fail before training starts, not after the run. These are rejected at submit:

- A string reference that does not resolve to a deployment config.
- On a LoRA job, a referenced config with `lora_enabled: false`.
- On a full-weight job, a referenced config that names a *different* model. The output entity does not have to exist yet — a config may point forward at the model the run will produce — but a config built for some other model cannot serve it.

A deployment config that names no model — neither `model_entity_id` nor `model_spec.model_name` — is an **unbound template**. It carries only engine and executor settings, so it is accepted by any job: the trained model is stamped onto a copy of it at deploy time, leaving the template reusable for the next run. This is the only way to share engine/executor settings across full-weight jobs, whose output model does not exist when the job is submitted.
...
| `policy_backend` | Follows `finetuning_type` | The NeMo-RL worker that trains the model. Set it on `training`, not under `training.parallelism`. **`lora` requires `automodel`**. **`all_weights` defaults to `dtensor`** and may be set to `automodel` for a consolidated HuggingFace export. MoE models that use expert parallelism also require `automodel`, because `dtensor` does not support `expert_parallel_size` above 1. Leave it unset to get the default. `automodel` needs Transformer Engine (Hopper or newer) and supports `automodel_kwargs`; `dtensor` runs stock HuggingFace on PyTorch FSDP2 and is the pre-Hopper option. `dtensor` plus LoRA, expert parallelism, or `automodel_kwargs` is rejected at submit, every conflict at once. |
| `v4_compatible` | `true` | `GRPOTraining.v4_compatible`. On Automodel full-weight export, keep the base transformers-v4 `config.json` so the platform's vLLM can load the published model; the v5 config is written beside it as `config.v5.json`. Set `false` to export the v5 `config.json` instead. If the base checkpoint already uses transformers v5, the job logs warn you to set this to `false`. No effect on `dtensor` or LoRA jobs. |
...
| Setting | Why |
| --- | --- |
| `NHX_SANDBOX_CLUSTER_CAPABLE=true` (Helm: `sandboxClusterCapable`) | Declares that OpenSandbox is installed. The platform chart does **not** install it. GRPO defaults to sandboxed Gym (`NHX_RL_SANDBOXED_GYM_DEFAULT`) and fails closed without this |
| `NHX_RL_JOB_STORAGE_PVC_CLAIM` | The Gym sandbox mounts the job-storage claim itself to read the downloaded environment and dataset, and only learns the claim by name |
| `NHX_RL_SANDBOX_ALLOW_INTERNET` | Needed whenever an environment's venv build has to reach a package index — which is every `native-v1` package, and every `adapter-wheels-v1` one |
| `NHX_RL_SANDBOX_PUBLIC_DNS_ALLOW` | Extra hosts, consulted only when the above is on. The built-in allowance covers `*.com` and `*.org`, so a host on another TLD (for example `hub.primeintellect.ai`) must be named |
