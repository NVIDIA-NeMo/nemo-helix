...
- On a full-weight job, a string reference at all — the job registers a new model entity, so a pre-existing config cannot target it. The exception is retraining, where the config already targets that entity. Use inline parameters otherwise.
