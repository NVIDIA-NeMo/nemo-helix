...
- On a full-weight job, a referenced config that names a *different* model. The job registers a new model entity, so a config built for some other model cannot serve it. Retraining is fine, because the config already names that entity.
