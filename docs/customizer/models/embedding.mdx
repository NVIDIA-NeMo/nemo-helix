...
| Epochs | `schedule.epochs` | `1` to start, raised while `retrieve-eval` metrics keep improving; the published NVDocs reference run used `3` |
...
Validation data is optional. Automodel first looks in the `dataset.training`
fileset for files named `val*.jsonl`, `validation*.jsonl`, or `dev*.jsonl`. If
none exists, `schedule.validation_split` deterministically holds out part of
the discovered training rows; the default is `0.1` (10%). This split drives
validation loss and checkpoint selection. It is separate from the frozen
`eval_beir` split used by `retrieve-eval` to compare retrieval quality.
