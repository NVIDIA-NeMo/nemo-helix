...
A **Model Entity** represents a model registered in the NeMo Helix. It contains:
...
| --- | --- | --- |
| **Embedding Models** | ✅ Supported | **Defaults**: Nemotron 3 Embed 1B (`nvidia/Nemotron-3-Embed-1B-BF16`, `training.recipe: bi_encoder`, Retriever NIM `nemotron-3-embed-1b:2.2.0`) and Llama Nemotron Embed 1B v2. Use cases: semantic search, RAG first-stage retrieval. |
| **Reranking Models** | ✅ Supported | **Model**: Llama Nemotron Rerank 1B v2 with `training.recipe: cross_encoder`. Automodel merges to a Hugging Face sequence-classification checkpoint, then exports ONNX (`logits`) with HF weights under `alternates/hf/` for Ranking NIM `llama-nemotron-rerank-1b-v2:1.10.0`. |
