...
```json
{
  "messages": [
    ...
    {
      "role": "assistant",
      "content": "<think>To solve 2 + 2, add 2 and 2 together.</think> The answer is 4."
    }
  ]
}
```
...
# Create an Automodel job (include tools in each JSONL training example)
spec = AutomodelJobInput(
    model="default/llama-3-2-1b",
    dataset={"training": "default/my-tool-dataset"},
...
- [Start a LoRA Model Customization Job](/documentation/customizer-reference/tutorials/lora-customization-job) - For parameter-efficient fine-tuning
- [Start a Full SFT Customization Job](/documentation/customizer-reference/tutorials/sft-customization-job) - For full model fine-tuning
