...
[Run in Google Colab](https://colab.research.google.com/github/NVIDIA-NeMo/nemo-helix/blob/main/docs/customizer/tutorials/lora-customization-job.ipynb)

Learn how to use the NeMo Helix to create a LoRA (Low-Rank Adaptation) customization job using a custom dataset. In this tutorial we use LoRA to fine-tune a **question-answering model** from the SQuAD dataset.
...
The SDK needs to know your NeMo Helix server URL. By default, `http://localhost:8080` is used. If NeMo Helix is running elsewhere, set the `NHX_BASE_URL` environment variable:
...
def create_or_get_secret(name: str, value: str | None, label: str) -> HelixSecretResponse | None:
    if not value:
