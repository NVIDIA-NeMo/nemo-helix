...
1. **A NeMo Helix configured with `platform.runtime: kubernetes`** — GRPO provisions a Ray cluster and has no local Docker fallback
...
3. **The `nemo` CLI on your `PATH`**, or a source checkout where `uv run --package nhx-rl` resolves
...
The converter emits a complete wheel closure, including `verifiers_agent` dependencies
that are source-only upstream, so `adapter-wheels-v1` does not require network access
when the job starts. Conversion itself still runs on an internet-connected host.
...
| `--upload` | Create both FileSets (`NHX_BASE_URL` required). Default names: `<slug>-env` and `<slug>-env-dataset` |
...
```bash
uv run --package nhx-rl pi-to-gym-conversion --validate-only ./ascii-tree-pkg
```
...
uv run --package nhx-rl pi-to-gym-conversion --validate-only ./weather-env
```
...
Job JSON: [GRPO and Reward Environments](/documentation/customizer-reference/grpo-and-reward-environments#submit-the-job). Sandboxed Gym is configured on the platform (`NHX_RL_SANDBOXED_GYM_DEFAULT`), not in the job payload. Cluster setup: [OpenSandbox](/documentation/kubernetes-deployment/setup/helm/open-sandbox).
