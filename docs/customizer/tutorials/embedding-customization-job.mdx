...
Automodel `dataset.training` and `retrieve-eval` `dataset` resolve that path fragment, so you can pass the Stage 1 `artifacts` URL through unchanged. Stage 1 unique-keys `eval_beir/qrels/test.tsv` before writing it.

Freeze that fileset and reuse it for every run. Scoring two runs on different splits measures data variance rather than model quality.

Equivalent CLI: take `artifact_url` from the `artifacts` result.

```bash
nemo jobs results list <prepare-job-name> --workspace default --output json
```

The next cell stores that URL as `TRAINING_REF`.
...
TRAINING_REF = artifacts.artifact_url
...
`model` and `dataset.training` are independent inputs. The entity supplies the encoder weights; the fileset supplies the contrastive rows. Automodel discovers `training.jsonl` in that fileset, including under a `#path` fragment. To train on an existing triplet dataset instead, build that fileset in [Train on SPECTER or triplet JSONL](/documentation/customizer-reference/tutorials/embedding-triplet-dataset) and point `dataset.training` at it — SPECTER carries no `eval_beir`, so keep the NVDocs Stage 1 artifacts URL for Stage 4.
...
nemo customization automodel.jobs submit --workspace default --name embed-nvdocs --spec '{
  "model": "default/nemotron-3-embed-1b",
  "dataset": {"training": "<stage-1-artifacts-url>"},
  "training": {
    "training_type": "sft",
    "recipe": "bi_encoder",
