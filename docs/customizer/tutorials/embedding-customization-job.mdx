...
[Run in Google Colab](https://colab.research.google.com/github/NVIDIA-NeMo/nemo-helix/blob/main/docs/customizer/tutorials/embedding-customization-job.ipynb)

Fine-tune Nemotron 3 Embed 1B on domain retrieval data and score it against the base checkpoint with BEIR nDCG and Recall. This is the NeMo Helix mapping of the [Nemotron embed recipe](https://github.com/NVIDIA-NeMo/Nemotron/blob/main/docs/nemotron/embed/README.md).
...
- A running NeMo Helix at `NHX_BASE_URL`, default `http://localhost:8080`. See the [Quickstart](/documentation/get-started).
- The Python SDK: `pip install "nemo-helix[all]"`, or `make bootstrap` from the repo root.
...
The `nemo` CLI reads `NHX_BASE_URL` too, and `--base-url` overrides it per command. The NGC key becomes a Platform secret so the cluster can pull NIM images:
...
```python
from nemo_helix.types.files import HuggingfaceStorageConfigParam
...
Automodel `dataset.training` and `retrieve-eval` `dataset` take a fileset name with no path fragment, so copy `training.jsonl` and `eval_beir/` to the root of a dataset fileset. Stage 1 unique-keys `eval_beir/qrels/test.tsv` before writing it; still drop duplicate `(query-id, corpus-id)` rows if you are uploading an older dump, because the BEIR loader rejects them. The next cell does the copy and unique-key.
...
```bash
nemo evaluator retrieve-eval --workspace default --spec '{
...
if proc.returncode != 0:
    raise RuntimeError(f"retrieve-eval failed:\n{proc.stderr}")
