...
Pre-trained embeddings handle general retrieval well but underperform on specialized vocabulary, document structure, and query phrasing. Fine-tuning adapts the model to your corpus. On the NVDocs split used here, three epochs on mined hard negatives lift nDCG@10 from 0.567 to 0.613 and Recall@10 from 0.638 to 0.696 against the base checkpoint. Expect larger gains where the base model has the least prior exposure, such as legal, biomedical, and internal engineering documents, and smaller ones on corpora close to its pre-training mix.
...
Mining embeds every training query and the whole corpus on GPU, then keeps the highest-scoring passages that fall below `min_positive_score * hard_neg_margin`. Raising the margin toward 1.0 produces harder negatives and a higher chance of labeling a relevant passage as negative; lowering it to 0.85–0.90 is safer but a weaker training signal. Start at the default and change it only if Stage 4 metrics plateau.
...
Reference scores on the 20,909-query NVDocs `eval_beir` split, from full three-epoch runs of this recipe on 2 GPUs:

| Model | nDCG@10 | Recall@10 | Recall@100 | MAP@10 |
| --- | --- | --- | --- | --- |
| Base `nemotron-3-embed-1b` | 0.5671 | 0.6375 | 0.8238 | 0.4787 |
| Fine-tuned, 3 epochs | 0.6131 | 0.6960 | 0.8794 | 0.5219 |

Judge a run by the gap between target and baseline in the same job, not by the absolute number: the eval split, corpus size, and mining depth all move the scale. Runs shortened for a smoke test land within a few thousandths of the base model, which confirms the plumbing but says nothing about the recipe.
...
| Epochs | `schedule.epochs` | `1` for 10,000 rows or more, `2` for a few thousand |
