...
On **release 0.7 and later**, Automodel `dataset.training` and `retrieve-eval` `dataset` resolve that path fragment, so you can pass the Stage 1 `artifacts` URL through unchanged. Current Stage 1 unique-keys `eval_beir/qrels/test.tsv` before writing it. The BEIR loader still rejects duplicate `(query-id, corpus-id)` rows if an older dump still has them.

On **release 0.6**, those fields take a fileset name with no path fragment, and Stage 1 can emit duplicate qrels. Copy `training.jsonl` and `eval_beir/` to the root of a dataset fileset, and drop duplicate qrels before upload. The next cell is that 0.6 procedure; it also works on 0.7 if you want a fileset whose root already has both artifacts.
...
On 0.6, unique-key `./artifacts/eval_beir/qrels/test.tsv` before the upload; the cell below does it inline.
