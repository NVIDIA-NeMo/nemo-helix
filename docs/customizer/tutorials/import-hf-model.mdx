...
title: "Import and Fine-Tune Private Hugging Face Models"
description: ""
---
...
- Completed the [Quickstart](/documentation/get-started) to install and deploy NeMo Helix locally.
...
This tutorial uses the current NeMo Helix SDK resources: `client.files.filesets`, `client.models`, `client.customization.automodel.jobs`, and `client.inference.gateway`.
...
The SDK needs your NeMo Helix server URL. By default, this tutorial uses `http://localhost:8080`.
...
Private and gated Hugging Face repositories require a token. Store it as a NeMo Helix secret and reference that secret from the Hugging Face fileset.

```python
def create_or_get_secret(name: str, value: str | None, label: str) -> HelixSecretResponse:
    if not value:
...
export INFERENCE_MODEL_NAME="default--${OUTPUT_NAME}"
