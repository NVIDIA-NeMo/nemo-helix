...
- Obtained the base URL of your NeMo Helix.
- For gated or private Hugging Face models: Created a secret with your HF token. Refer to [Manage Secrets](/documentation/get-started/core-concepts/manage-secrets).
- Set the `NHX_BASE_URL` environment variable.

```bash
export NHX_BASE_URL="https://your-nemo-helix-url"
```
...
The most common method is downloading directly from Hugging Face. The example below uses [Qwen/Qwen3-1.7B](https://huggingface.co/Qwen/Qwen3-1.7B), a public model that requires no token:
...
import os
from nemo_helix import ConflictError, NeMoHelix
from nemo_helix.types.files import NGCStorageConfigParam
