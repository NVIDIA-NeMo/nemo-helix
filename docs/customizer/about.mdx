...
This page provides an overview of the customization concepts for the NeMo Helix.
...
SFT and LoRA both learn from examples of the right answer. Reinforcement learning instead learns from a *comparison* — some signal that one response was better than another. NeMo Customizer supports two methods, both on the `rl` backend.
...
GRPO needs no labeled responses at all. For each prompt it samples a **group** of responses, hands them to a **reward environment** that scores each one, and updates the policy toward the responses that scored better than the others in their group.

That framing has a consequence worth understanding before you start: the learning signal is the *spread* of rewards inside a group. A prompt whose responses all earn the same score contributes nothing to the update. This is why GRPO runs are read on reward rather than loss, and why sampling settings such as `temperature` and `num_generations_per_prompt` matter more here than in any other method.

Use GRPO when correctness is *verifiable* — math with a checkable answer, code that either passes tests or does not, structured output that either parses or does not — rather than merely preferred.

GRPO requires a [NeMo Gym](https://docs.nvidia.com/nemo/gym/about/) environment packaged as a FileSet, alongside the prompt dataset. See [GRPO and Reward Environments](/documentation/customizer-reference/grpo-and-reward-environments).

<Note>

Both RL methods run on a Ray cluster and require a platform configured with `platform.runtime: kubernetes`. They have no local Docker fallback, unlike the SFT and LoRA backends.
