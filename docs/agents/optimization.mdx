...
| `cpu` (docker or `kubernetes_job`) | The `nhx-tasks` image, same entry point | The task image must carry those dependencies; `nemo-optimization-plugin` is in the `cpu-tasks` dependency group for this reason |
