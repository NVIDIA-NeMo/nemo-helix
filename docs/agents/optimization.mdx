...
|--------------------|----------|----------------------------------|
| `subprocess` | Host subprocess running `python -m nemo_agent_optimization_plugin.tasks.optimize` | The jobs host venv must carry the optimization and Fabric harness dependencies (and the Docker socket, if trials need it) |
