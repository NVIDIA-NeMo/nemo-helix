...
| Prompt optimization | The agent has an optimization config and baseline dataset                                                     | Suggests staging with `nemo agents optimize prepare-fileset`, then running `nemo agents optimize run-strategy` for Fabric-backed tuning |
...
The `nemo agents optimize run-strategy` command submits Fabric-backed numeric
optimization. `nemo agents optimize` is a shared command group, one per
installed optimization strategy; `--strategy nat` selects the Fabric-backed
numeric study implemented in `nemo-optimization`. Run
`nemo agents optimize list-strategies` to see what this environment has.
Input must be a Fabric-native agent package
...
The `agents-optimize` skill suggests `nemo agents optimize run-strategy` when the agent
...
- Stages the optimize bundle with `nemo agents optimize prepare-fileset`,
  then submits it with `nemo agents optimize run-strategy --strategy nat`.
...
The agent calls the email-phishing-analyzer MCP server from the
`email-phishing-analyzer-harnesses` checkout, declared statically like any
production MCP server: its console script as `url` and its key in `env`
(`PHISHING_MCP_BIN`, `PHISHING_ANALYZER_API_KEY`; the analyzer calls
`integrate.api.nvidia.com` while the coordinator uses `inference-api.nvidia.com`). Trials are scored on the judged
classification, on the analyzer being called exactly once, and on the email
reaching the tool verbatim; the last two are read from the ATIF trajectory.
`optimize-mcp-mock.yaml` runs the same study against a fixture shipped in the
bundle that replays recorded analyzer results, so it needs no checkout or
analyzer credential. Full steps are in `plugins/nemo-optimization/examples/hermes-optimize/README.md`.
...
`nemo agents optimize run-strategy` hands the study to the platform's Jobs service, which
...
`nemo agents optimize run-strategy` command on success. It checks that:
...
- every path the config references (dataset, `eval.fabric.base_dir`, stdio MCP
  server scripts in `mcp.servers.*.args`) is relative and present under
  `--source`;
...
`run-strategy` does no optimization itself: it resolves `--strategy` to the
installed strategy job and delegates `compile` to it, so the executor and entry
point below are the `nat` strategy's own. That job picks its executor from the
execution profiles the platform actually registered, for the requested profile
...
| `cpu` (docker or `kubernetes_job`) | The `nhx-tasks` image, same entry point | The task image must carry those dependencies; `nemo-agent-optimization-plugin` and `nemo-optimization-plugin` are in the `cpu-tasks` dependency group for this reason |
