...
every other strategy ships its own `tasks/agent_optimize.py` under its own
package — `prompt-master` does today. Each strategy job picks
...
| `cpu` (docker or `kubernetes_job`) | The `nmp-cpu-tasks` image, same entry point | The task image must carry those dependencies; `nemo-agent-optimization-plugin`, `nemo-optimization-plugin`, and `prompt-master-plugin` are in the `cpu-tasks` dependency group for this reason |
