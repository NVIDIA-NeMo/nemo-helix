...
The agent calls a mock phishing-analyzer MCP server that ships in the example
bundle (`phishing_analyzer_mcp/server.py`, spawned with `python3`; the optimizer
makes bundle-relative `mcp.servers.*.args` paths absolute per trial). It replays
the `analysis` stored on each `dataset-mcp.json` row, so new tasks only need a
real analyzer's result recorded once. Trials are scored on the judged
...
`optimize-mcp-live.yaml` runs the same study against the real analyzer from the
`email-phishing-analyzer-harnesses` checkout, declared statically with its
console script as `url` and its key in `env` (`PHISHING_MCP_BIN`,
`NVIDIA_API_KEY`). Full steps are in `plugins/nemo-optimization/examples/hermes-optimize/README.md`.
