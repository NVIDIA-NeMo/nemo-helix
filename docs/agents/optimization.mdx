...
bundle and is installed into the platform venv as `phishing-analyzer-mcp`
(`command -v phishing-analyzer-mcp` to confirm). It replays the `analysis` stored
on each `dataset-mcp.json` row, so new tasks only need a real analyzer's result
recorded once. Trials are scored on the judged
...
- every path the config references (dataset, `eval.fabric.base_dir`) is
  relative and present under `--source`;
