...
| `cpu` (docker or `kubernetes_job`) | The `nmp-cpu-tasks` image, same entry point | The task image must carry those dependencies; `nemo-agent-optimization-plugin` and the strategy plugins are in the `cpu-tasks` dependency group for this reason |
