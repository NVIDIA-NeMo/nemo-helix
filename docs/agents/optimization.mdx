...
`nemo-optimization`). The input is a platform agent entity
(`config_format: nemo-agents-spec-v1`), named with `--agent`; the job
translates it into the Fabric package the study runs and registers the
result as a new agent entity. The golden-path harness is
...
Keep the config and everything it references in one directory — an *optimize
bundle* — stage that bundle with `prepare-fileset`, and pass `--optimize-config` as a
path relative to the staged fileset root. `--agent` (the platform agent under
test) and `--output-agent` (the name for the new, optimized agent entity the
run creates) are both required — the job always resolves `--agent` against
the platform and always creates `--output-agent`, whether you submit through
the CLI or run the job in-process with the Python SDK. Paths *inside* the YAML
(`dataset`, `eval.fabric.base_dir`, hook and MCP configs) resolve against the
staged bundle root.

Every `optimizer.search_space` path is Fabric-shaped and is validated before the
study starts. A path must address a value the study can really change in the
Fabric package it runs — `models.default.<declared model field>`, or
`harness.settings.<field the agent already sets>` — *and* one the job can write
back onto the stored agent afterwards. Paths that fail either half are refused
in seconds, all of them named in one error, instead of after a full study of
paid model calls: a spec-shaped `harnesses.<name>.settings.<field>` (the Fabric
harness block is singular, `harness`) tunes nothing, and a model leaf the agent
spec cannot carry, such as `top_p`, cannot round-trip back to the agent.
...
Register the platform agent the study will optimize (once):
...
os.chdir(bundle)  # dataset / base_dir are relative to the bundle, and the
                   # fileset must already be staged (see the CLI tab)
...
# Register a platform agent for the phishing-analyzer harness first (this
# example does not ship a standalone agent.yaml for it the way the
# chat-only example's agents/chatonly/agent.yaml does) -- substitute your
# own --agent-config, then use that agent's name below.
...
The job always resolves `--agent` against the platform: it fetches the stored
...
`default/hermes-optimize-chatonly`) and is required on every submission along
with `--output-agent`, the name for the new, optimized agent entity the run
creates. Endpoint URLs and other URI forms (`http://...`, `https://...`,
`file://...`) are rejected -- raw HTTP endpoint optimize mode was removed, and
there is no longer an inline-agent-only path that skips `--agent`.
...
compile time with a pointer back to `prepare-fileset`. `--agent` and
`--output-agent` are required too — there is no separate artifacts fileset or
directory to publish to; the study's result *is* the new `--output-agent`
entity it creates.

### Where the study runs

`nemo agents optimize` (`OptimizeJob`) does no work itself: it resolves
`--strategy` to the installed `AgentOptimizeJob` subclass and delegates
`compile` to it, so the executor and entry point are the *strategy's*, not a
shared one. For the `nat` strategy, that is `nemo_optimization.tasks.agent_optimize`;
other strategies (`prompt-master`, `switchyard`, `experimentalist`) ship their
own `tasks/agent_optimize.py` under their own package. Each strategy job picks
its executor from the execution profiles the platform actually registered, for
the requested profile (currently always `default`):

| Registered profile | Executor | What the deployment must provide |
|--------------------|----------|----------------------------------|
| `subprocess` | Host subprocess running `python -m <strategy's task_module>` (e.g. `nemo_optimization.tasks.agent_optimize` for `nat`) | The jobs host venv must carry the optimization and Fabric harness dependencies (and the Docker socket, if trials need it) |
| `cpu` (docker or `kubernetes_job`) | The `nmp-cpu-tasks` image, same entry point | The task image must carry those dependencies; `nemo-agent-optimization-plugin`, `nemo-optimization-plugin`, `nemo-switchyard`, and `prompt-master-plugin` are in the `cpu-tasks` dependency group for this reason |
