...
Observability on NeMo Helix is provided by **NeMo Intake**, the trace ingestion and query service
...
- A NeMo Helix environment with the `intake` service running.
...
You do not need to deploy your agents on NeMo Helix. To start, run the platform, post one
...
The examples assume a running NeMo Helix reachable at `$NHX_BASE_URL`. Point at whatever you have,
...
`NHX_ACCESS_TOKEN` command. Also omit `OTEL_EXPORTER_OTLP_HEADERS` from the OTLP example.
...
local ClickHouse container for the resolved NeMo data directory unless `NHX_INTAKE_CLICKHOUSE_URL`
...
curl --fail-with-body -i \
  ...
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/spans?page=1&page_size=1"
```
...
curl --fail-with-body -X POST \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/ingest/chat-completions" \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
...
curl -g --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/spans?filter[session_id]=$SESSION_ID&page=1&page_size=10"
```

Then open `$NHX_STUDIO_URL/workspaces/$WORKSPACE/intake/traces`.
...
export OTEL_EXPORTER_OTLP_HEADERS="Authorization=Bearer%20$NHX_ACCESS_TOKEN"
```
...
```bash
curl --fail-with-body -X POST \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/ingest/atif" \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl --fail-with-body -X POST \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/annotations" \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl --fail-with-body -X POST \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluator-results" \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
        "span_id": "<span-id>",
...
curl --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/spans/<span-id>/evaluator-results"
curl -g --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluator-results?filter[name]=faithfulness/v1&filter[value][\$gte]=0.8"
```
...
curl -g --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/spans?filter[status]=error&page=1&page_size=20"
curl -g --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/spans/groups?by=session_id&filter[status]=error"
```
...
Raise the OTLP body cap with `NHX_INTAKE_OTLP_MAX_BODY_BYTES` when larger batches are required. For
