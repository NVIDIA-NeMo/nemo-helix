...
- The optimization plugins installed in the same Python environment as the NeMo command-line
  ...
  through the default `enabled-plugins` group. Experimentalist includes the internal
  Eval Author agent and requires Python earlier than 3.14.
...
uv sync --only-group experimentalist  # Experimenter, including its internal Eval Author
```
