...
- A NeMo Helix deployment (local or remote) with Intake enabled, so the
...
The optimization agents build on the following NeMo Helix service
dependencies:
...
`NHX_BASE_URL` is the base-URL environment variable for this workflow, and
...
export NHX_ACCESS_TOKEN="$(nemo auth token)"
...
Set `NHX_STUDIO_URL` explicitly if Studio uses a different origin or path.
...
the `NHX_ACCESS_TOKEN` command and the `Authorization` header in the API example.
...
```bash
curl --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/insights/v2/workspaces/$WORKSPACE/insights?agent=$AGENT&page=1&page_size=20"
```

API-stored Insights also appear in Studio at
`$NHX_STUDIO_URL/workspaces/$WORKSPACE/optimizer`. Profile-based Insights do
...
`$NHX_STUDIO_URL/workspaces/$WORKSPACE/experiment`. There you can see
...
| `--ethos` | no | profile `ethos`, else `ETHOS.md` or `README.md` beside the profile | Path to [Ethos](/documentation/agents/optimize-agents/ethos) Markdown for the AUT. |
| `--workspace` | no | profile `workspace`, else `default` | Workspace to operate in. |
| `--base-url` | no | `NHX_BASE_URL`, else `http://localhost:8080` | Running platform instance the Analyst's tools call. |
...
| `--base-url` | no | `NHX_BASE_URL`, else `http://localhost:8080` | Running platform instance. |
