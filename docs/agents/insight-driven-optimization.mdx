...
- The optimization plugins installed in the same Python environment as the NeMo command-line
  interface (CLI). From a source checkout, `uv sync` installs Insights and Experimentalist
  through the default `enabled-plugins` group. Experimentalist requires Python 3.12
  or 3.13.
...
- Train and validation datasets in a Harbor-compatible layout, plus a task
  template and an output directory for artifacts. The Experimenter uses these to
  build evaluation suites and validate changes.

To install only one side of the loop, use the convenience groups:

```bash
uv sync --only-group insights         # Analyst only
uv sync --only-group experimentalist  # Experimenter only
```
...
The Experimenter reads `optimizer.yaml`, discovered by walking up from the
current directory, and validates its schema. For Analyst submissions, pass
`--agent`, `--workspace`, and optional `--ethos` explicitly.
...
The optional `ethos` path is the agent's [ETHOS.md](/documentation/agents/optimize-agents/ethos) contract.
For the Experimenter, relative paths resolve against the profile. An adjacent
`.env` is loaded when a profile is found, without replacing variables already
set in the shell. Analyst submissions use `--ethos` when supplied; they do not
automatically select an Ethos file.
...
AnalysisRuns execute on NeMo Platform. The Insights plugin stores new insights
and appends evidence to existing insights through the Insights API.
Pass analysis settings explicitly; results are stored in NeMo Platform.
...
[Ethos](/documentation/agents/optimize-agents/ethos) (`--ethos ETHOS.md`) so it can flag divergence from
intended behavior.
...
`--agent` overrides it. The Experimenter builds an Insight-specific
...
prompt to reuse the default. The Analyst and Experimentalist both
...
```bash
nemo insights analysis-runs --help
...
```bash
nemo insights analysis-runs create --wait \
  --agent "$AGENT" \
  --workspace "$WORKSPACE" \
...
- `--ethos ETHOS.md`: Append [Ethos](/documentation/agents/optimize-agents/ethos) so the Analyst can flag
  divergence from intended behavior.
- `--evaluation-id <id>`: Restrict analysis to one evaluation.
- `--since <ISO-8601 timestamp>`: Set the lower time bound.

### Confirm It Worked

Inspect the AnalysisRun and its backing job with
`nemo insights analysis-runs get <run-name> --workspace "$WORKSPACE"`.
Confirm the job completed and inspect its analysis report. A successful run
may create no insights if it finds no new recurring problems.

Insights are stored in NeMo Platform and appear in Studio at
`$NMP_STUDIO_URL/workspaces/$WORKSPACE/optimizer`. Each finding should have a
clear `title`, an actionable `description`, and supporting `trace_refs`.
...
Instead of running `nemo insights analysis-runs create --wait` manually, opt an agent in and let
...
Submit analysis through `nemo insights analysis-runs`.

### `nemo insights analysis-runs create`

Submit one analysis of an agent's traces. `--agent` is required. Optional flags
include `--workspace`, `--base-url`, `--ethos`, `--since`, `--evaluation-id`,
`--default-model`, and `--fast-model`. Use `--wait` to wait for the backing job.
Provide analysis settings through command-line flags. Results are stored in NeMo Platform.
...
| `--ethos` | no | profile `ethos` | Path to [Ethos](/documentation/agents/optimize-agents/ethos) Markdown for the AUT. |
...
|----------|---------|-------------------|
| stored `default_model` | Analyst, Experimenter | Required workspace-qualified Model Entity selected by `nemo setup`. Used for quality-critical analysis, authoring, proposing, and coding. |
| stored `fast_model` | Analyst, Experimenter | Selected by `nemo setup`; defaults to the default model. Used for high-volume scoring, summarization, and control steps. |
| `NEMO_DEFAULT_MODEL` | Analyst, Experimenter | Optional non-interactive override for `default_model`, in `workspace/model-name` form. |
| `NEMO_FAST_MODEL` | Analyst, Experimenter | Optional non-interactive override for `fast_model`, in `workspace/model-name` form. Falls back to the effective default model. |
...
Check the AnalysisRun's backing job and analysis report, then confirm that
Studio is showing the workspace and agent supplied when the run was submitted.
A completed run may produce no new insights.
