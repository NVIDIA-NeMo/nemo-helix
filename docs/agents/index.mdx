...
Legacy NAT compatibility does not include the former experimental Claude Code,
Codex, Cursor, Hermes, or OpenClaw workflow adapters. Configure Claude, Codex,
DeepAgents, or Hermes through the Fabric-backed `nemo-agents-spec-v1` format
instead. Cursor and OpenClaw do not currently have supported Fabric adapters.
