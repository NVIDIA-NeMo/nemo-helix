...
| Optimize  | `nemo agents optimize --strategy nat --agent <name> --config-fileset <fileset> --config <path> --output-agent <new-name>` | Run Fabric-backed numeric HPO (Optuna). Stage the bundle first with `nemo agents optimize prepare-fileset`. See [Optimize Agents](/documentation/agents/optimize-agents). |
