...
An agent on NeMo Helix calls tools, accesses models through shared Platform
...
1. **The Agents gateway.** The gateway resolves the agent to its current running deployment and proxies client requests to it, including streaming responses. From a client's perspective, the agent is an OpenAI-compatible endpoint owned by NeMo Helix.
...
When you instead want to update an agent **in place** — iterating on your own
agent's config and pushing the new version to the same name — use
`nemo agents redeploy`. It wraps the four manual steps (undeploy → delete →
create → deploy) into one command. Because the agent entity is immutable by
name, redeploy validates your new config first (a bad config is a safe no-op),
then tears the old agent down and rebuilds it; the durable `{agent}-ethos`
fileset is preserved and re-uploaded. Once teardown starts the operation is not
atomic, but each step is check-first, so it is safe to re-run.
...
| Redeploy  | `nemo agents redeploy --agent <name> --agent-config <path>`                                                   | Push a changed config to a deployed agent in one step (undeploy → delete → create → deploy). |
...
| Optimize  | `nemo agents optimize run-strategy --strategy ...                                                             | Run various plugin-provided agent optimization strategies. See [Optimize Agents](/documentation/agents/optimize-agents). |
...
NeMo Helix uses Fabric as the runtime wrapper around your agent so the
platform can deploy it and route its model traffic
through shared infrastructure.

### Install Harness Support
...
| Harness selection | `nemo-helix` package expression | Source checkout plugin expression | Harness packages installed |
|---|---|---|---|
...
From a source checkout, use the standalone plugin extra instead:

```bash
uv sync --package nemo-agents-plugin --extra deepagents
```

Hermes Agent 0.20 and later is not distributed through PyPI and has dependency
conflicts with the Platform environment. Follow the
[Hermes Agent installation guide](https://hermes-agent.nousresearch.com/docs/installation)
to create a separate environment, then install the matching Fabric runtime and
Hermes adapter into that environment:
...
PLATFORM_PYTHON="$(sed -n '1s/^#!//p' "$(command -v nemo)")"
FABRIC_VERSION="$("$PLATFORM_PYTHON" -c \
  'import importlib.metadata as m; print(m.version("nemo-fabric"))')"

uv pip install --python "$ADAPTER_PYTHON" \
  "nemo-fabric[relay]==$FABRIC_VERSION" \
  "nemo-fabric-adapters-hermes==$FABRIC_VERSION"
```

Keep `ADAPTER_PYTHON` set when starting NeMo Helix. Source-checkout users
can instead run `script/dev-install-hermes.sh`, which performs these steps and
installs the supported Hermes version in `.venv-hermes`.
...
NeMo Helix uses NAT as the runtime wrapper around your agent so the
...
- **Leave `base_url` and `api_key` unset on `openai` and `nim` LLMs.** NeMo Helix injects an Inference Gateway URL when it deploys the agent, and the gateway retrieves upstream credentials from the secrets service. Setting `base_url` explicitly bypasses both.
...
- [Optimize Agents](/documentation/agents/optimize-agents): Try various strategies for agent optimization
  (`nemo agents optimize run-strategy --strategy ...`), including model-routing, skill or prompt improvement, and more.
- [Add Guardrails to an Agent](/documentation/agents/add-guardrails): route an agent's model traffic
  through input and output rails.
- [Scan Trace Data](/documentation/agents/scan-trace-data): check recent telemetry for PII and leaked
  credentials.
