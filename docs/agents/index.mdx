...
| Harness selection | `nemo-platform` package expression | Source checkout plugin expression | Harness packages installed |
|---|---|---|---|
...
```bash
export ADAPTER_PYTHON=/path/to/hermes-environment/bin/python
...
Keep `ADAPTER_PYTHON` set when starting NeMo Platform. Source-checkout users
