...
- Access to a deployment of NeMo Helix with NeMo Safe Synthesizer available (see [getting-started](/documentation/synthesize-safe-data))
- **An NVIDIA GPU with 80 GB+ VRAM** — Safe Synthesizer requires GPU access for model training.
- Python environment with `nemo-helix` SDK installed
...
Install the PyPI `nemo-helix` wrapper package with Safe Synthesizer support.
If you are working from a source checkout, run `make bootstrap` from the repository root instead, then install the tutorial-only packages (`kagglehub matplotlib`) in that environment.

For a PyPI install, run the following command in a **terminal (shell)**:

```shell
if command -v uv &> /dev/null; then
 uv pip install "nemo-helix[all]" 'kagglehub==0.3.13' matplotlib
else
 pip install "nemo-helix[all]" 'kagglehub==0.3.13' matplotlib
fi
```
...
Column classification uses an LLM to automatically detect column types and improve PII detection accuracy.
Without this setup, you may see classification errors and reduced detection quality.

</Tip>

```python
# Use the pre-configured NVIDIA Build model provider
# This provider is set up during platform deployment
...
If you prefer not to send column data to `build.nvidia.com`, you can deploy your own LLM and create a custom model provider.
Pass the fully-qualified provider name (`workspace/provider-name`) to `.with_classify_model_provider()` instead.
...
`wait_for_completion()` raises `RuntimeError` if the job ends in an `error` or `cancelled` state.
Check the printed status output and logs above for the cause.

If the job fails with **"No GPUs available on this system"**, ensure your system is configured with GPU access:

```bash
nemo setup --start-services
```

Verify GPU access with `nvidia-smi` on the host, if deployed via quickstart.
...
The evaluation report contains two high-level scores: Synthetic Quality Score (SQS) and Data Privacy Score (DPS).
Both are measured out of 10, and higher is better.
...
- Verify `NHX_BASE_URL` is correct
