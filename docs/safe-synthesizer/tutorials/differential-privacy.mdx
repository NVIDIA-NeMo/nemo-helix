...
if command -v uv &> /dev/null; then
 uv pip install "nemo-helix[all]" kagglehub matplotlib
else
 pip install "nemo-helix[all]" kagglehub matplotlib
fi
```
...
def check_data_requirements(dataset_size):
    ...
    if dataset_size <= 0:
        print("❌ Empty dataset - collect records before applying DP")
        return
