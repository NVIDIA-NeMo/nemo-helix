...
The Anonymizer service detects personally identifiable information (PII) in text data on the NeMo Helix and replaces or rewrites it.

## Overview

The service wraps the open-source [NVIDIA NeMo Anonymizer library](https://github.com/NVIDIA-NeMo/Anonymizer) and exposes it through the NeMo Helix's Python SDK and CLI. The library still owns PII detection, replacement, rewrite, and config validation. The platform adds inference routing through the Inference Gateway, fileset-backed inputs, SDK/service execution for streaming preview, and a Jobs-worker path for full anonymization runs.
...
Submit the config to the Anonymizer service with the NeMo Helix SDK:

```python
from nemo_anonymizer_plugin.app.task_config import PreviewRequest
from nemo_helix import NeMoHelix

sdk = NeMoHelix(base_url="...", workspace="default")
...
| Feature           | Standalone Library                                  | NeMo Helix Service                                                                                       |
|-------------------|-----------------------------------------------------|-----------------------------------------------------------------------------------------------------------------|
...
| **Input sources** | Local file, `http(s)` URL                           | `http(s)` URL or NeMo Helix Fileset                                        |
| **Artifacts**     | Local filesystem                                    | NeMo Helix job artifact storage for `run` |
| **Authentication**| Direct API keys                                     | NeMo Helix Secrets service                                                                               |
...
- An `anonymizer.run` job that writes `dataset.parquet`, `trace.parquet`, `metadata.json`, and optional `failed_records.json`. The job executes on the NeMo Helix Jobs worker (`nemo anonymizer run` / `sdk.anonymizer.run`).
...
<Card title="Studio UI" href="/documentation/studio/anonymizer">

Configure a job, preview it on a few records, and track results from **Datasets > Anonymizer** in NeMo Studio.
