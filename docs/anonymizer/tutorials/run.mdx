...
This tutorial walks through the `anonymizer.run` job: defining a run spec, submitting it to the NeMo Helix Jobs worker, and loading the parquet artifacts it produces.
...
- A running NeMo Helix cluster with the `nemo anonymizer` CLI available (see [Setup](/documentation/get-started)).
...
|-------------------------|-------------------------------|-------------|--------------------------|--------------------------------------------------------|
| `nemo anonymizer run`   | NeMo Helix Jobs worker     | Rejected    | Required                 | Stored in NeMo Helix job artifact storage; pull with `download_artifacts()` |
...
Submit the spec to the NeMo Helix Jobs worker:

```bash
nemo anonymizer run \
  --spec-file /tmp/anonymizer-run.yaml \
  --workspace "${NHX_WORKSPACE:-default}" \
...
nemo jobs get-status <job-name> --workspace "${NHX_WORKSPACE:-default}"
nemo jobs get-logs <job-name> --workspace "${NHX_WORKSPACE:-default}"
```
