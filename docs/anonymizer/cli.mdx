...
| `nemo anonymizer run`            | Submit an `anonymizer.run` job to the NeMo Helix Jobs worker. |
...
```bash
nemo jobs get-status anonymizer-run-2026-05-12-abc123 --workspace "${NHX_WORKSPACE:-default}"
nemo jobs get-logs anonymizer-run-2026-05-12-abc123 --workspace "${NHX_WORKSPACE:-default}"
```
...
| `model_configs`   | list of Data Designer `ModelConfig`             | yes      | Required so execution routes through NeMo Helix Inference Gateway.  |
