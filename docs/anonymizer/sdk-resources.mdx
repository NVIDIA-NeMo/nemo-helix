...
title: "Anonymizer NeMo Helix SDK Resources"
description: ""
---
<a id="anonymizer-nhx-sdk-resources"></a>

The `anonymizer.config` module (from the [NVIDIA NeMo Anonymizer library](https://github.com/NVIDIA-NeMo/Anonymizer)) builds `AnonymizerConfig` objects in a context-agnostic way. Once you are ready to execute that config against the NeMo Helix Anonymizer service, you use objects from the `nemo_helix` SDK. This page describes the NeMo Helix-specific objects.

## AnonymizerResource

The `AnonymizerResource` is the entry point for working with Anonymizer on NeMo Helix. It wraps the streaming preview endpoint and job submission for the plugin service.

A `AnonymizerResource` is accessed directly from a `NeMoHelix` instance:
...
An `AsyncAnonymizerResource` with the same surface is available via `AsyncNeMoHelix.anonymizer`.
...
| `run(request, *, workspace=None, wait_until_done=False)` | Submits an `anonymizer.run` job to the NeMo Helix Jobs worker. Returns an `AnonymizerJobResource`. When `wait_until_done=True`, blocks until the job reaches a terminal state. |
...
| `get_job_status()`                                  | Returns the current `HelixJobStatus`.                                                                          |
