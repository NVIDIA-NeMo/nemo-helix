...
title: "Hardware and Software Requirements for NeMo Helix"
description: ""
---
...
clusters, use the NeMo Helix Helm chart on Kubernetes.

## Local Setup Requirements

| Component | Requirement | Notes |
|-----------|-------------|-------|
| Python | 3.12, 3.13, or 3.14 | Use an isolated virtual environment. |
| Package installer | `uv>=0.10.10` recommended for source checkouts; `pip` supported for published packages | The one-line installer sets up `uv` automatically when needed. |
...
| Helm | Helm 3 | Required to install and upgrade the NeMo Helix chart. |
| Storage | ReadWriteMany-capable persistent storage | Required for shared files and job storage in multi-pod deployments. |
| Image access | NGC credentials and Kubernetes image pull secrets | Required to pull NeMo Helix container images. |
