...
The NeMo Helix Helm chart can expose the API service externally using **Kubernetes Ingress**, the **Gateway API HTTPRoute**, or on OpenShift an **OpenShift Route**. Choose one based on your cluster.
...
Install or upgrade with your values file so the Ingress resource is created. After installation, use the URL shown in the Helm notes (`helm status nemo-helix`) or the host you configured.

**Advanced:** For multiple hostnames or different paths per host, leave `defaultHost` unset and use `ingress.hosts` (each entry has `name` and `paths`). See the [NeMo Helix Helm Chart reference](/documentation/kubernetes-deployment/helm/helm-reference) for the full structure.
...
1. **Enable HTTPRoute** and set `parentRefs` to the Gateway(s) this route should attach to:
 ...
 hostnames:
 - "nhx.example.com" # optional
 ```

2. Install or upgrade with this values file. The HTTPRoute will be created and bound to the specified Gateway(s).

For full option details (e.g. `filters`, `labels`, `annotations`), see the [NeMo Helix Helm Chart reference](/documentation/kubernetes-deployment/helm/helm-reference).
...
1. **Enable the OpenShift Route** and set the hostname (optional):
 ...
 openshiftRoute:
 enabled: true
 host: "nhx.example.com" # optional; if empty, the router may assign a default
 ```
...
3. Install or upgrade with your values file. After installation, use the URL from the Helm notes (`helm status nemo-helix`) or the host you configured.

For all options (`targetPort`, `annotations`, `labels`), see the [NeMo Helix Helm Chart reference](/documentation/kubernetes-deployment/helm/helm-reference).
...
2. **Set the IngressClass and ALB annotations in your values file:**
 ...
 enabled: true
 className: alb
 defaultHost: "nhx.example.com" # optional
 annotations:
