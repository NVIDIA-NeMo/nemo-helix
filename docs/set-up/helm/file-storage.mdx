...
The NeMo Helix Files service stores uploaded files and datasets. By default, it uses filesystem storage backed by a Kubernetes ReadWriteMany PersistentVolumeClaim (PVC). Alternatively, you can configure S3-compatible object storage.
...
platformConfig:
 ...
 type: s3
 bucket: my-nemo-bucket
 prefix: nemo-helix/files
 region: us-east-1
 use_sdk_auth: true
```

All files will be stored under `s3://my-nemo-bucket/nemo-helix/files/`.
