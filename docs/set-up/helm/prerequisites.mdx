...
Before installing the NeMo Helix Helm chart, prepare Kubernetes access, storage, and NGC credentials.
...
Review the [system requirements](/documentation/reference/system-requirements) before choosing node types. For storage and persistent volume configuration, including ReadWriteMany and StorageClass requirements, see [Persistent Volumes](/documentation/kubernetes-deployment/setup/helm/persistent-volumes).
...
| OpenShift | Apply the OpenShift security context overrides in [OpenShift](/documentation/kubernetes-deployment/setup/helm/openshift). |

## Create NGC API key and secrets

You need an NGC account and an NGC API key that can access the NeMo Helix chart and container images in the `nvidia/nemo-helix` org. NeMo Helix images are hosted under paths such as `nvcr.io/nvidia/nemo-helix/nhx-api` and `nvcr.io/nvidia/nemo-helix/nhx-safe-synthesizer-tasks`.
...
1. Create an NGC API key following [Generating NGC API Keys](https://docs.nvidia.com/ngc/latest/ngc-private-registry-user-guide.html#generating-ngc-api-keys).
   ...
   Include permissions that allow access to NGC Catalog artifacts and the NVIDIA private registry. At minimum, include the NGC Catalog and Public API Endpoints permissions shown in the NGC API key UI. If your organization uses scoped service keys, ensure the key can read the `nvidia/nemo-helix` org.
...
4. Create the NGC API secret.
   ...
   This secret is mounted into Helix services that need to authenticate to NGC APIs for model and artifact access.
...
5. Confirm both secrets exist.
   ...
   ```sh
   kubectl get secret nvcrimagepullsecret ngc-api -n "$NHX_NAMESPACE"
   ```
