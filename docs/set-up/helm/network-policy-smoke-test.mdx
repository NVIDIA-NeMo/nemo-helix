...
- `NGC_API_KEY` with access to the NeMo Helix chart and container images
...
export KIND_CLUSTER_NAME=nhx-network-policy
export KUBE_NAMESPACE=nemo-helix
...
```sh
kubectl -n "$KUBE_NAMESPACE" port-forward svc/nemo-helix-api 8080:8080
```

```python
from nemo_helix import NeMoHelix

client = NeMoHelix(base_url="http://localhost:8080")
...
When testing source changes, set `NHX_E2E_REGISTRY` and `NHX_E2E_TAG` to a
branch-built `nhx-api` image. CI supplies these from the CPU smoke image build.
