...
title: "Volcano"
description: "Install Volcano as a cluster scheduler for multi-node NeMo Helix jobs."
---
<a id="install-volcano"></a>

The NeMo Helix Helm chart does **not** install [Volcano](https://volcano.sh/en/docs/). Volcano is **optional**. It is needed for multi-node jobs on the `volcano_job` backend (Customizer multi-node). Skip it and set `rbac.volcanoEnabled: false` if you are not running those jobs.
