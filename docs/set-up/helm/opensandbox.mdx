...
title: "OpenSandbox"
description: "Install OpenSandbox as a cluster engine and point NeMo Helix jobs at the in-cluster server."
---
<a id="install-opensandbox"></a>

The NeMo Helix Helm chart does **not** install [OpenSandbox](https://github.com/opensandbox-group/OpenSandbox). Install the upstream charts, then point the platform at the running server.
...
Gym sample YAML uses `OPENSANDBOX_DOMAIN` / `OPENSANDBOX_API_KEY`. NeMo Helix and the SDK use `OPEN_SANDBOX_*`. Do not mix the names.

## Install the shared-kernel server

Replace `NHX_NAMESPACE` with the platform release namespace.
...
# That Secret must exist in $NHX_NAMESPACE. If yours has a different name, edit
