...
description: ""
---
NeMo Helix v0.6.0 expands the 0.5 agent, evaluator, customization, and
...
- NeMo Gym evaluations can run in a sandboxed Gym host instead of sharing the
  evaluator process. The sandboxed path supports a locked `nhx-gym-host` image,
  the `nhx-cpu-tasks` task image, broker placement controls, rollout streaming,
...
- Platform API and `nemo-helix` Python SDK `0.6.0`
