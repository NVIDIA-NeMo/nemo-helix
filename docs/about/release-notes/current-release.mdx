...
If you have pre-0.5 platform-owned agent intent packages, use the `nemo-ethos`
coding-agent skill to write `ETHOS.md`, then remove the legacy spec package
before using newer optimization workflows.
...
- **GRPO environment packaging.** Converter-built `adapter-wheels-v1` packages
  include a complete wheel closure and do not require network access at job
  start. Conversion still requires an internet-connected host. Offline
  `wheels-v1` jobs likewise need a complete closure for the training image's
...
- **Agent intent migration.** `AGENT-SPEC.md` is legacy. Use `ETHOS.md` for
  new work and the `nemo-ethos` coding-agent skill for platform-owned legacy
