...
description: ""
---
NeMo Platform v0.4.0 introduces research-preview analysis of agent telemetry.
Agent owners can send traces to Intake and use Analyst to diagnose recurring
issues and record evidence-backed Insights.
...
- **Research-preview analysis.** Analyst produces evidence-backed Insights
  from recurring problems in agent telemetry.
...
- `optimizer.yaml` configures Analyst with the agent name, workspace, and
  optional `ETHOS.md` path.
- `ETHOS.md` is the durable Markdown contract for intended agent behavior. It
  can be consumed by the Analyst when present. It records
...
- Coding agents that find `agents/<name>-spec/AGENT-SPEC.md` read it as
  prior answers, provide `ETHOS.md`, and delete the spec
...
- **Research-preview optimizer.** Analyst is an early agentic
  workflow. It is useful for guided analysis, but quality and autonomy
