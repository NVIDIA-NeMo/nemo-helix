...
If you installed `nemo-helix` with `pip` or `uv pip`, it was installed in a virtual environment. The `nemo` command is only available when that environment is activated.
...
**Symptoms:**
- `uv tool install "nemo-helix[all]"` fails while building a dependency, often on a missing C header such as `omp.h`

**Cause:**

NeMo Helix supports Python 3.12 through 3.14, but uv [picks the interpreter itself](https://docs.astral.sh/uv/concepts/tools/#python-versions), so an install can land on a newer interpreter that NeMo Helix does not support yet. `nemo setup` exits immediately on an unsupported interpreter instead of starting services.
...
```bash
uv tool install --python 3.14 --reinstall "nemo-helix[all]"
```
