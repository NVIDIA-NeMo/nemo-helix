...
NeMo Helix supports Python 3.12 and 3.13, but uv [picks the interpreter itself](https://docs.astral.sh/uv/concepts/tools/#python-versions), so an install can land on Python 3.14. `nemo setup` exits immediately on an unsupported interpreter instead of starting services.
...
```bash
uv tool install --python 3.13 --reinstall "nemo-helix[all]"
```
