...
NeMo Platform supports Python 3.12 through 3.14, but uv [picks the interpreter itself](https://docs.astral.sh/uv/concepts/tools/#python-versions), so an install can land on a newer interpreter that NeMo Platform does not support yet. `nemo setup` exits immediately on an unsupported interpreter instead of starting services.
...
```bash
uv tool install --python 3.14 --reinstall "nemo-platform[all]"
```
