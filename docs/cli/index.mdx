...
title: "NeMo Helix CLI"
description: ""
---
<a id="nemo-ms-cli"></a>

The NeMo Helix CLI (`nemo`) is a command-line tool for interacting with NeMo Helix. It provides a unified interface for managing models, running jobs, deploying inference endpoints, and working with a local setup.
...
Install the PyPI `nemo-helix` wrapper package. From a source checkout, run `make bootstrap` from the repository root instead; the generated SDK package under `sdk/python/nemo-helix/pyproject.toml` does not define the wrapper extras.
...
# View service logs
cat ~/.local/state/nhx/instances/<scope>/services.log
