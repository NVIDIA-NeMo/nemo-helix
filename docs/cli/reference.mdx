...
* `--model, -m`: Model entity name (from 'nemo models list') or model ID when using --provider. Defaults to the context's default model set by 'nemo setup'.
* `--fast`: Use the context's fast model instead of its default model.
