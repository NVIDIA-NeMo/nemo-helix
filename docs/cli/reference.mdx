...
- Run local Helix services with **`nemo services run --help`**
...
* `--start-services, --no-start-services`: Start local Helix services
...
* `run`: Run Helix services in the foreground.
* `start`: Start Helix services in the background.
* `stop`: Stop running Helix services.
* `restart`: Restart Helix services.
* `status`: Show status of the Helix services instance for this scope.
