...
* `--strategy`: Installed optimization strategy, e.g. 'nat'.
* `--agent`: Platform agent to optimize.
* `--config-fileset`: Fileset holding the optimization bundle.
* `--output-agent`: Name for the new optimized agent entity.
...
Some spec fields cannot be represented as CLI flags: `config`. Supply them with --spec or --spec-file. Individual flags override values from the supplied spec.
​
...
* `--optimize-config`: Optimize YAML, as a path relative to --source. This is the value to pass to `optimize --config`.
...
* `list`: Print the ``strategy`` name of every installed...
