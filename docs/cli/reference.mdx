...
* `--workspace`: Workspace for the fileset and for agent / model preflight. Target workspace. Resolution order: (1) this --workspace flag; (2) NMP_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
...
* `--workspace, -w`: Target workspace. Defaults to the active CLI context's workspace.
* `--cluster`
* `--base-url`
* `--request-id`
...
* `--cluster`: Configured cluster name to resolve via the NeMo CLI config.
* `--base-url`: Explicit plugin-service base URL (overrides --cluster and all other submit host resolution).
* `--workspace`: Workspace path segment used in the submit URL. Defaults to the active CLI context's workspace.
...
* `--workspace`: Workspace the agent belongs to. Defaults to the active CLI context's workspace.
...
* `--workspace`: Workspace to inspect. Defaults to the active CLI context's workspace.
* `--base-url`: Base URL of the running NMP instance. [default: http://localhost:8080] [env: NMP_BASE_URL=]
...
* `--workspace`: Workspace the run belongs to. Defaults to the active CLI context's workspace.
* `--base-url`: Base URL of the running NMP instance. [default: http://localhost:8080] [env: NMP_BASE_URL=]
