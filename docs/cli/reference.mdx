...
* `--base-url`: Base URL for the NeMo Helix API
...
* `--workspace, -w`: Target workspace. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
...
* `--skills-from`: Comma-separated list of skill sources to install from (e.g. 'nemo-helix,nemo-evaluator-plugin'). Use 'nemo-helix' for the built-in set. Default: all sources. Only applied when --install-skills is set.
* `--skills-path <DIRECTORY>`: Install skills to a custom directory for another coding agent
...
* `login`: Authenticate with the NeMo Helix cluster.
...
* `access-keys`: Manage NeMo Helix Scoped Access Keys.
...
* `--password`: Password for password grant (prefer env NHX_OIDC_PASSWORD)
...
Manage NeMo Helix Scoped Access Keys.
...
* `--service-account`: Bind the key to a non-human service account (HelixAdmin only).
* `--scope`: Comma-separated service names to scope this key to, e.g. 'intake,entities'. Omit for an unscoped key.
* `--rotate`: JTI of a previous Scoped Access Key owned by the caller to revoke once this key is created. Primarily for rotating personal keys, which have no service-account identity to correlate rotations against automatically.
* `--workspace`: Grant the key's principal workspace membership, formatted '\<workspace>' or '\<workspace>:\<role1>,\<role2>' (role defaults to Editor). Repeat for multiple workspaces. Replaces a separate 'nemo workspaces members create' call.
...
* `--source`: Filter to skills from a specific source (distribution / plugin name as shown in the `Source` column, e.g. `nemo-helix`, `nemo-agents-plugin`). Can be repeated to include multiple sources. Matching is case-insensitive.
...
* `--agent, -a`: Agent to install for. Supported: claude, codex, cursor, opencode
...
* `--path <DIRECTORY>`: Install to a custom skills directory (for example ~/.my-agent/skills)
...
* `--workspace`
* `--job`: (required)
* `--output-file, -o <PATH>`: Output file path (required).
...
* `redeploy`: Redeploy an agent from a new config, in one command.
...
* `package`: Package a Fabric or NAT agent -- render -> validate ->...
...
* `--description`: [default: ]
* `--workspace, -w`: Target workspace. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
* `--base-url`: Platform base URL. Resolution order: (1) this --base-url flag or NEMO_BASE_URL; (2) shared CLI config (`nemo config set --base-url`) or NHX_BASE_URL; (3) http://localhost:8080 (default). [env: NEMO_BASE_URL=]
...
```shell
nemo agents redeploy [OPTIONS]
```

**Options:**

* `--agent, -a`: Name of the deployed agent to rebuild.
* `--agent-config, -c <FILE>`: Path to the new agent YAML config file to redeploy from.
* `--description`: Agent description (as on create). [default: ]
* `--name, -n`: Deployment name for the new deployment (auto-generated if omitted).
* `--mode`: Runtime backend: subprocess, docker, or k8s. Auto-detected from the existing deployment when omitted.
* `--image, -i`: Container image for docker/k8s modes. Auto-detected from the existing deployment when omitted.
* `--use-image-entrypoint, --no-use-image-entrypoint`: For docker/k8s modes, preserve the image ENTRYPOINT/CMD instead of injecting the platform-owned agent server command. Auto-detected from the existing deployment when omitted.
* `--environment, -e`: AgentEnvironment to deploy under, as a 'workspace/name' ref. Auto-detected from the existing deployment when omitted; pass an empty string to deploy with no environment.
* `--wait, --no-wait`: Wait for the new deployment to reach a terminal status (running or failed) before returning. Exits 0 only on running; exits 1 on failure or timeout. Pass --no-wait to return the pending deployment immediately as JSON.
* `--timeout, -t <INTEGER>`: Maximum seconds to wait for a terminal status (only with --wait). [default: 300]
* `--yes, -y`: Skip confirmation prompt.
* `--workspace, -w`: Target workspace. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
* `--base-url`: Platform base URL. Resolution order: (1) this --base-url flag or NEMO_BASE_URL; (2) shared CLI config (`nemo config set --base-url`) or NHX_BASE_URL; (3) http://localhost:8080 (default). [env: NEMO_BASE_URL=]
...
* `--workspace, -w`: Workspace scope for the submission. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
...
* `--profile`: Annotate the bundle with this profile.
* `--cluster`: Accepted for forward compatibility.
...
* `--output PATH | FILESET_REF`: Where to write eval outputs — either a local directory (path-shaped: starts with '/', './', '../', '~/') or a NeMo Helix fileset reference ('name' or 'workspace/name').  Filesets are created on demand if missing.  Defaults to \<ctx.storage.persistent>/results (the platform-injected persistent volume) when not provided.
...
* `--image`: Container image to execute the agent in. Mirrors CreateDeploymentRequest.image. Typically the output of `nemo agents package`, which contains both the agent's Fabric adapter and the execute task module. Empty falls back to agents.jobs.default_image, then to the jobs substrate's chain (the execution profile's default_task_image, then the platform CPU tasks image).
...
* `--workspace`: Workspace for the fileset and for agent / model preflight. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
...
* `<<PATH | FILESET_REF>>`: Local path to a result.json / run dir / nat-jobs dir, or a NeMo Helix fileset reference.
...
* `--agent, -c <FILE>`: Path to an agent YAML config file: a Fabric agent spec ('config_format: nemo-agents-spec-v1') or a NAT workflow ('config_format: nat-workflow-v1', the default when omitted).
...
* `--nat-version`: NAT release to install (e.g. '1.7.0').  NAT workflow configs only; rejected for Fabric agent specs.  Strongly recommended: pin explicitly so image tags/labels/deps are reproducible.  When omitted, a baked-in default is used and a warning is printed.
...
* `check-models`: Check that every model referenced by the configuration is...
* `preview`
...
* `create`: Generate a synthetic dataset
...
#### nemo data-designer check-models
...
```shell
nemo data-designer check-models [OPTIONS] CONFIG_SOURCE
```
...
* `--workspace`: Workspace used to resolve provider references and seed sources. Defaults to the SDK's configured workspace, or 'default'.
* `--output <CHOICE>`: Output format. With 'json' the report is the only thing on stdout; engine progress logs go to stderr so stdout stays parseable. [possible values: text, json; default: text]
...
* `--sdg-input`: Stage 0 file or directory: a fileset, fileset#file, local path, or hf:// URI.
* `--generation-file`: Relative Stage 0 filename inside sdg_input when the ref is a directory.
* `--train-input-file`
...
* `--generate.multi-doc`: Override `generate.multi_doc` in the spec.
* `--generate.bundle-size <INTEGER>`: Override `generate.bundle_size` in the spec.
* `--generate.max-docs-per-bundle <INTEGER>`: Override `generate.max_docs_per_bundle` in the spec.
...
* `--generate.max-parallel-requests-for-gen <INTEGER>`: Override `generate.max_parallel_requests_for_gen` in the spec.
...
* `--cluster`: Configured cluster name to resolve via the NeMo CLI config.
* `--base-url`: Explicit plugin-service base URL (overrides --cluster and all other submit host resolution).
* `--workspace, -w`: Workspace path segment used in the submit URL. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
...
Some spec fields cannot be represented as CLI flags: `generate.file_extensions`, `generate.bundle_strategy`, `generate.query_counts`, `generate.reasoning_counts`, `generate.resume`. Supply them with --spec or --spec-file. Individual flags override values from the supplied spec.
​
...
```shell
nemo data-designer create [OPTIONS] [CONFIG_SOURCE] [COMMAND] [ARGS]...
```
...
##### nemo data-designer create explain
...
```shell
nemo data-designer create explain [OPTIONS]
```
...
```shell
nemo data-designer retrieval-generate [OPTIONS] [COMMAND] [ARGS]...
```
...
* `--multi-doc`: Override `multi_doc` in the spec.
* `--bundle-size <INTEGER>`: Override `bundle_size` in the spec.
* `--max-docs-per-bundle <INTEGER>`: Override `max_docs_per_bundle` in the spec.
...
* `--max-parallel-requests-for-gen <INTEGER>`: Override `max_parallel_requests_for_gen` in the spec.
...
Some spec fields cannot be represented as CLI flags: `file_extensions`, `bundle_strategy`, `query_counts`, `reasoning_counts`, `resume`. Supply them with --spec or --spec-file. Individual flags override values from the supplied spec.
​
...
##### nemo data-designer retrieval-generate explain
...
```shell
nemo data-designer retrieval-generate explain [OPTIONS]
```
...
```shell
nemo data-designer retrieval-prepare [OPTIONS] [COMMAND] [ARGS]...
```
...
* `--sdg-input`: Fileset or hf:// URI to Stage 0 output. Name a file with fileset#path or hf://org/dataset[@revision]/path; otherwise generation_file is used inside the directory.
* `--generation-file`: Relative path to the Stage 0 file inside sdg_input when the ref is a directory. Live generate writes generation_result.json. Skip-SDG dumps set this (e.g. nv_pp_dd_sdg.json) or put the filename on sdg_input.
...
##### nemo data-designer retrieval-prepare explain
...
```shell
nemo data-designer retrieval-prepare explain [OPTIONS]
```
...
```shell
nemo data-designer retrieval-run [OPTIONS] [COMMAND] [ARGS]...
```
...
* `--generate.embed-model`: Embedding model for retrieval SDG.
* `--generate.hf-token-secret`: Override `generate.hf_token_secret` in the spec.
* `--prepare.sdg-input`: Fileset or hf:// URI to Stage 0 output. Name a file with fileset#path or hf://org/dataset[@revision]/path; otherwise generation_file is used inside the directory.
* `--prepare.generation-file`: Relative path to the Stage 0 file inside sdg_input when the ref is a directory. Live generate writes generation_result.json. Skip-SDG dumps set this (e.g. nv_pp_dd_sdg.json) or put the filename on sdg_input.
...
##### nemo data-designer retrieval-run explain
...
```shell
nemo data-designer retrieval-run explain [OPTIONS]
```
...
* `--workspace, -w`: Target workspace. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
* `--base-url`: [default: http://localhost:8080] [env: NHX_BASE_URL=]
...
* `--config.rewrite.use-combined-graph`: Run rewrite and conditional repair iterations in one Data Designer graph.
...
Some spec fields cannot be represented as CLI flags: `config.detect.entity_labels`, `config.detect.excluded_entity_labels`, `config.replace`, `model_configs`, `selected_models.detection`, `selected_models.replace`, `selected_models.rewrite`. Supply them with --spec or --spec-file. Individual flags override values from the supplied spec.
​
...
* `retrieve-eval`: Evaluate dense retrieval over a BEIR corpus with nDCG,...
...
#### nemo evaluator retrieve-eval

Evaluate dense retrieval over a BEIR corpus with nDCG, recall, precision, and MAP.

**Usage:**

```shell
nemo evaluator retrieve-eval [OPTIONS] [COMMAND] [ARGS]...
```
...
* `--dataset.root`: Reference to a Fileset (format: workspace/fileset-name).
...
Job Spec flags are generated from the RetrieveEvalInputSpec Pydantic schema. Precedence: --spec-file (base) → --spec JSON (overlay) → per-flag values (top).

Some spec fields cannot be represented as CLI flags: `target`, `baseline`, `k`. Supply them with --spec or --spec-file. Individual flags override values from the supplied spec.
​
...
##### nemo evaluator retrieve-eval explain
...
```shell
nemo evaluator retrieve-eval explain [OPTIONS]
```
...
* `--workspace, -w`: Target workspace. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
* `--base-url`: Override the platform API host.
* `--cluster`: Name of a cluster in the CLI config.
...
* `--upload-model SOURCE`: Local path to model weights, or a HuggingFace repo id. Creates the fileset and model entity.
* `--upload-dataset SOURCE`: Local file or directory to upload as the dataset. Pass a directory to send several files, the same as 'nemo files upload'.
* `--upload-environment SOURCE`: Local directory to upload as a NeMo Gym environment, for GRPO.
* `--exist-ok`: Reuse whichever of the above already exists. Its files are left as they are.
* `--hf-token-secret`: Platform secret holding a HuggingFace token, for a gated or private repo.

**Commands:**

* `automodel`: Fine-tune a model with Automodel: SFT, LoRA, or knowledge...
* `rl`: Align a model with NeMo-RL, using DPO or GRPO.
* `unsloth`: Fine-tune a model with Unsloth on one GPU: SFT with LoRA...
...
Fine-tune a model with Automodel: SFT, LoRA, or knowledge distillation.
...
* `explain`: Show input/output schemas.
* `submit`: Submit an Automodel training job to the platform.
...
Submit an Automodel training job to the platform.
...
* `--workspace, -w`: Target workspace. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
* `--profile`: Execution profile to run the job on. Uses the backend default when omitted.
* `--cluster`: Name of a cluster in the CLI config. Submits to that cluster's base URL.
* `--base-url`: Override platform API host. If omitted: --cluster, then CLI context, then `$NHX_BASE_URL`, then http://localhost:8080.
* `-o`: Backend option override, 'backend.key=value'. [default: ]
* `--options-file <PATH>`: JSON or YAML file of backend option overrides. Any -o flag wins over it.
...
* `--wait`: Wait for the job to finish, showing its status.
* `--watch`: Wait for the job to finish, showing its status and logs.
* `--timeout <INTEGER RANGE>`: Give up waiting after this many seconds. Waits indefinitely when omitted.
* `--poll-interval <INTEGER RANGE>`: Seconds between status checks while waiting. [default: 3]

**Resource Creation Options:**

* `--upload-model SOURCE`: Local path to model weights, or a HuggingFace repo id. Creates the fileset and model entity.
* `--upload-dataset SOURCE`: Local file or directory to upload as the dataset. Pass a directory to send several files, the same as 'nemo files upload'.
* `--upload-environment SOURCE`: Local directory to upload as a NeMo Gym environment. GRPO only.
* `--exist-ok`: Reuse whichever of the above already exists. Its files are left as they are.
* `--hf-token-secret`: Platform secret holding a HuggingFace token, for a gated or private repo.

#### nemo customization rl

Align a model with NeMo-RL, using DPO or GRPO.
...
* `explain`: Show input/output schemas.
* `submit`: Submit a NeMo-RL training job to the platform.
...
Submit a NeMo-RL training job to the platform.
...
Fine-tune a model with Unsloth on one GPU: SFT with LoRA or full weights.
...
* `explain`: Show input/output schemas.
* `submit`: Submit an Unsloth training job to the platform.
...
Submit an Unsloth training job to the platform.
...
* `--dataset.training`: Training fileset as 'name', 'workspace/name', or either form with a '#path/' directory.
...
* `--dataset.shuffle`: Reshuffle training examples each epoch. Disable only for a deliberate curriculum order.
...
* `--training.lora.use-memory-efficient-lora`: Use Automodel's lower-memory LoRA path. Recommended for large MoE checkpoints. Omit to use Automodel's default, which is on.
...
* `--training.retrieval.train-n-passages <INTEGER>`: Override `training.retrieval.train_n_passages` in the spec.
* `--training.retrieval.eval-negative-size <INTEGER>`: Override `training.retrieval.eval_negative_size` in the spec.
* `--training.retrieval.do-gradient-checkpointing`: Override `training.retrieval.do_gradient_checkpointing` in the spec.
* `--training.retrieval.do-distributed-inbatch-negative`: Score each query against every passage in the global batch rather than only its own train_n_passages. Ignored for cross_encoder.
* `--training.retrieval.query-max-length <INTEGER>`: Override `training.retrieval.query_max_length` in the spec.
* `--training.retrieval.passage-max-length <INTEGER>`: Override `training.retrieval.passage_max_length` in the spec.
* `--training.retrieval.query-prefix`: Literal prefix prepended to each query. Empty string disables prefixing.
* `--training.retrieval.passage-prefix`: Literal prefix prepended to each passage. Empty string disables prefixing.
* `--training.retrieval.export.opset <INTEGER>`: Override `training.retrieval.export.opset` in the spec.
* `--training.retrieval.export.normalize`: L2-normalize embeddings. Ignored for cross_encoder.
* `--training.retrieval.export.dimensions`: Add a Matryoshka 'dimensions' input that truncates and renormalizes embeddings.
* `--training.activation-checkpointing`: Recompute intermediate activations in the backward pass instead of storing them. Substantially lowers memory use for a modest slowdown, and is commonly enabled when training large MoE models.
* `--training.mtp.num-nextn-predict-layers <INTEGER>`: How many tokens ahead to predict. Omit to keep the checkpoint's own value.
* `--training.mtp.use-repeated-layer`: Share one weight-tied layer across the prediction depths. Only some Automodel models accept it (Nemotron-H in r0.6.0); omit it for others.
* `--training.mtp.loss-scaling-factor <FLOAT>`: Weight of the MTP loss term relative to the main loss. Omit to keep the model's default (0.1).
* `--training.backend.rope-fusion`: Fuse the rotary embedding into the attention kernel.
* `--training.backend.fake-balanced-gate`: Route tokens evenly across experts instead of using the trained router. Benchmarking aid.
* `--training.backend.enable-hf-state-dict-adapter`: Save and load checkpoints in HuggingFace layout. Needed when the trained model is consumed by HuggingFace tooling.
* `--training.backend.enable-fsdp-optimizations`: Enable Automodel's additional FSDP2 sharding optimizations.
...
* `--schedule.validation-split <FLOAT>`: Validation split to use when a validation dataset is not provided.
...
* `--batch.packed-sequence-size <INTEGER>`: Pin the packed sequence length instead of estimating it. Requires sequence_packing.
...
* `--parallelism.pipeline.pp-schedule`: Pipeline schedule name, e.g. '1f1b', 'interleaved1f1b', 'gpipe'. Defaults to 'interleaved1f1b'.
* `--parallelism.pipeline.pp-microbatch-size <INTEGER>`: Micro-batch size flowing through each pipeline stage.
* `--parallelism.pipeline.scale-grads-in-schedule`: Scale gradients inside the schedule rather than afterwards.
* `--parallelism.pipeline.patch-inner-model`: Apply Automodel's pipeline patch to the inner transformer module.
* `--parallelism.pipeline.patch-causal-lm-model`: Apply Automodel's pipeline patch to the causal-LM wrapper.
...
Some spec fields cannot be represented as CLI flags: `training.training_type`, `training.recipe`, `training.finetuning_type`, `training.lora.target_modules`, `training.lora.exclude_modules`, `training.precision`, `training.attn_implementation`, `training.teacher_precision`, `training.retrieval.export.primary`, `training.retrieval.export.precision`, `training.retrieval.export.attn_implementation`, `training.retrieval.export.pooling`, `training.backend.attn`, `training.backend.linear`, `training.backend.rms_norm`, `training.backend.rope`, `training.backend.experts`, `training.backend.dispatcher`, `schedule.checkpoint_selection`, `schedule.progress_reporting.time_series_metrics`, `optimizer.optimizer`, `optimizer.lr_decay_style`, `parallelism.pipeline.round_virtual_stages_to_pp_multiple`, `integrations.wandb.tags`, `integrations.mlflow.tags`, `deployment_config (other union forms)`. Supply them with --spec or --spec-file. Individual flags override values from the supplied spec.
​
...
Some spec fields cannot be represented as CLI flags: `training`, `integrations.wandb.tags`, `integrations.mlflow.tags`, `deployment_config (other union forms)`. Supply them with --spec or --spec-file. Individual flags override values from the supplied spec.
​
...
* `--workspace, -w`: Workspace the agent belongs to. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
* `--base-url`: Base URL of the running NHX instance. [default: http://localhost:8080] [env: NHX_BASE_URL=]
...
* `--workspace, -w`: Workspace to inspect. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
* `--base-url`: Base URL of the running NHX instance. [default: http://localhost:8080] [env: NHX_BASE_URL=]
...
* `--workspace, -w`: Workspace the run belongs to. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
* `--base-url`: Base URL of the running NHX instance. [default: http://localhost:8080] [env: NHX_BASE_URL=]
...
```shell
nemo safe-synthesizer generate [OPTIONS] [COMMAND] [ARGS]...
```
...
* `--config.training.warmup-steps <FLOAT>`: Linear warmup from 0 to the learning rate. A whole number of 1 or more sets the exact number of warmup steps; a float in (0, 1) is treated as a ratio of total training steps; 0 disables warmup. Must be finite and >= 0, and cannot be fractional at or above 1.
* `--config.training.warmup-ratio <FLOAT>`: Deprecated. Use warmup_steps instead.
...
* `--config.generation.max-tokens-multiplier <FLOAT>`: Multiplier on the longest training example when sizing per-sample max_tokens. Must be a finite value > 0. Default 1.2.
...
* `--config.generation.structured-generation.max-records-per-sequence <INTEGER>`: Max records per grouped sequence under structured generation. None uses the largest training group size. Must be None or >= 1.
...
* `--config.generation.validation.enforce-numeric-range`: When True, reject floats outside the training-observed min/max. Set False to skip float range checks (integers/enums still enforced).
...
##### nemo safe-synthesizer generate explain
...
```shell
nemo safe-synthesizer generate explain [OPTIONS]
```
