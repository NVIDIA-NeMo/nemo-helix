...
* `optimize`: Optimize a platform agent with an installed optimization...
...
Optimize a platform agent with an installed optimization strategy.
...
* `list-strategies`: List the strategies `--strategy` accepts, one name per line.
* `prepare-fileset`: Validate a NAT optimize bundle and upload it to a fileset...

*Jobs:*

* `run-strategy`: Optimize a platform agent with an installed optimization...
...
```shell
nemo agents optimize list-strategies [OPTIONS]
```

**Options:**

* `--base-url`: Platform to ask. Defaults to the same target as every other `nemo` command. [env: NEMO_BASE_URL=]
...
Validate a NAT optimize bundle and upload it to a fileset for `optimize run-strategy`.
...
* `--optimize-config`: Optimize YAML, as a path relative to --source. This is the value to pass to `optimize run-strategy --optimize-config`.
...
```shell
nemo agents optimize run-strategy [OPTIONS] [COMMAND] [ARGS]...
```
...
* `--strategy`: Installed optimization strategy to run, e.g. 'nat'. List what this platform has with `nemo agents optimize list-strategies`.
* `--optimize-config`: Location of the strategy's configuration YAML.  With optimize_config_fileset set — required for remote submission — this is a path relative to the fileset root.  Without it (programmatic local runs only) it is an absolute path on the host running the job.
* `--optimize-config-fileset FILESET_REF`: Fileset holding the optimization bundle: the config named by optimize_config plus every asset it references.  Stage one with `nemo agents optimize prepare-fileset`.  Required for remote submissions, where the job has no access to the client's filesystem.
* `--agent`: Platform agent to optimize ('name' or 'workspace/name'). Strategies that can take the agent under test from the config itself may leave this unset.
* `--output PATH | FILESET_REF`: Where the strategy should publish its artifacts once the run succeeds — either a local directory (path-shaped: starts with '/', './', '../', '~/') or a NeMo Helix fileset reference ('name' or 'workspace/name').  Filesets are created on demand if missing.
...
Job Spec flags are generated from the RunStrategySubmitSpec Pydantic schema. Precedence: --spec-file (base) → --spec JSON (overlay) → per-flag values (top).
​
...
###### nemo agents optimize run-strategy explain
...
```shell
nemo agents optimize run-strategy explain [OPTIONS]
```
