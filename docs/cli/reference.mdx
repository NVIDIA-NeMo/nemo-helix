...
* `run`: Run the analyst agent against a running NHX instance.
...
* `--workspace, -w`: Workspace the analyst should operate in. Resolution order: (1) this --workspace flag; (2) NHX_WORKSPACE; (3) the active CLI context's workspace (`nemo config use-context`); (4) 'default'.
* `--base-url`: Base URL of the running NHX instance the analyst's tools should call.
...
* `--profile <FILE>`: Path to optimizer.yaml. Default: discovered by walking up from cwd.
* `--base-url`: Base URL of the running NHX instance to check.
