...
Learn how to use NeMo Helix to apply content safety checks to user inputs and LLM outputs with the NVIDIA Nemotron Content Safety NIM. Content safety checks detect and block harmful, abusive, or policy-violating content before it reaches users.
...
For the main model, this tutorial uses the [Nemotron 3.5 Lightning 30B-A3B](https://build.nvidia.com/nvidia/nemotron-3.5-lightning-30b-a3b) NIM.

## Prerequisites

Before you begin:

- You have access to a running NeMo Helix.
- `NHX_BASE_URL` is set to the NeMo Helix base URL.
...
- `main` model: `nvidia/nemotron-3.5-lightning-30b-a3b`
...
The `Nemotron-Safety-Guard-8B-v3` and `Nemotron 3.5 Lightning 30B-A3B` NIMs both support multiple languages. For the full list, refer to the model cards:
