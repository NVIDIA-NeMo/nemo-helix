...
Platform-managed agents are the canonical consumer: pointing an agent's `llms` block at a guarded VirtualModel entity is how you secure the agent's model path. See [Add Guardrails to an Agent](/documentation/agents/add-guardrails) for the agent-side wiring. The rest of this page applies to any client calling a guarded VirtualModel, agent or not.
...
```bash
nemo inference virtual-models create guarded-nemotron \
  --default-model-entity default/nvidia-nemotron-3.5-lightning-30b-a3b \
...
client.inference.virtual_models.create(
    name="guarded-nemotron",
    default_model_entity="default/nvidia-nemotron-3.5-lightning-30b-a3b",
    request_middleware=[
        {
...
```bash
nemo chat -m default/guarded-nemotron "What is the capital of France?"
```
...
curl -s $NHX_BASE_URL/apis/inference-gateway/v2/workspaces/default/openai/-/v1/chat/completions \
  ...
  -d '{
    "model": "default/guarded-nemotron",
...
Get a pre-configured OpenAI client from the platform SDK and call it like any other OpenAI-compatible endpoint. The client's base URL points at the workspace-scoped IGW route, so `model="default/guarded-nemotron"` resolves through IGW's VirtualModel cache.
...
```bash
nemo inference virtual-models create guarded-nemotron-inline \
  --default-model-entity default/nvidia-nemotron-3.5-lightning-30b-a3b \
  --request-middleware '[{
    "name": "nemo-guardrails",
    "config_type": "guardrail_config",
...
```bash
curl -N -s $NHX_BASE_URL/apis/inference-gateway/v2/workspaces/default/openai/-/v1/chat/completions \
  -H 'content-type: application/json' \
  -d '{
    "model": "default/guarded-nemotron",
...
```bash
nemo chat -m default/guarded-nemotron "Explain machine learning in simple terms."
```
...
- All headers starting with `x-` or `X-` — NeMo Helix principal headers, `x-otel-*`, and any custom `X-Foo` headers your client sets.
...
- `X-NHX-Principal-Id` / `X-NHX-Principal-On-Behalf-Of` for service-principal authorization.
