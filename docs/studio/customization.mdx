...
title: "NeMo Studio Fine-tuning"
description: "Fine-tune models from NeMo Studio, and track customization jobs."
---

Use **Models > Fine-tune** in the NeMo Studio workspace sidebar to open the fine-tuned models list, start fine-tuning jobs, and track their progress for the selected workspace.

<Note>

Fine-tuning is enabled by default. An administrator can disable it by setting `studio.feature_flags.customizer_enabled: false` in the platform configuration and restarting Studio, which removes the **Fine-tune** navigation entry and hides fine-tuning jobs from the general **Jobs** list.
...
Select **Customize a Model** from the fine-tuned models list to open **Fine-tune a Model**, which first asks how you want to start:
...
The list shows fine-tuned models created in the current workspace. Select a row to open the model details side panel, which includes **Model Details** and **Chat Playground** tabs, a **Customize this Model** action, an **Evaluate this Model** action, and a **View Intake Traces** link when intake is enabled.
