...
NeMo Studio is the web app for AI development with NVIDIA NeMo Helix. It provides a workspace-oriented UI for managing local platform resources, reviewing agents, running agent optimization workflows, monitoring agent telemetry, and working with filesets, jobs, and secrets.

The pages visible in your workspace depend on the installed plugins, enabled services, feature flags, and NeMo Helix version. A page documented here might be hidden in your deployment. Preview pages identify the feature flag that enables them.

---

## Getting Started

NeMo Studio is included with the platform. Follow the [Setup guide](/documentation/get-started) to start NeMo Helix, then access Studio at `/studio` on your running server.
...
| **Data** | **Datasets > Anonymizer** | Detect and protect PII in a dataset. Enabled by default; see [NeMo Studio Anonymizer](/documentation/studio/anonymizer). |
| **Data** | **Datasets > Data Designer** | Build and monitor synthetic-data jobs. Other installed data plugins can appear below **Datasets**. |
| **Data** | **Datasets > Safe Synthesizer** | Create and monitor privacy-preserving synthetic tabular data jobs. Enabled by default. |
...
| **Governance** | **Agent Hardener** | Plugin-provided agent hardening UI. Appears only when the Agent Hardener plugin is installed; see [Studio Plugin UIs](/documentation/studio/plugins). |
...
Additional entries, including Dashboard, Deployments, and Playground, are disabled by default and appear only when an administrator enables their feature flag. Because plugins contribute navigation at runtime, an installed plugin can also add pages to an existing group or add a new group.
...
Select **Models > Fine-tune** to open the fine-tuned models list, start a fine-tuning job against an eligible base model, and track job progress. Enabled by default; see [NeMo Studio Fine-tuning](/documentation/studio/customization).
...
Use **Datasets > Anonymizer** to detect and protect PII in a dataset, choosing a substitute, redact, annotate, hash, or rewrite strategy, previewing the result on a few records, and running a full job. Enabled by default.

For the full workflow, see [NeMo Studio Anonymizer](/documentation/studio/anonymizer).

### Safe Synthesizer

Use **Datasets > Safe Synthesizer** to create and monitor privacy-preserving synthetic tabular data jobs from NeMo Studio. The Studio page provides access to job creation, job status, details, reports, and result downloads for Safe Synthesizer workflows.
