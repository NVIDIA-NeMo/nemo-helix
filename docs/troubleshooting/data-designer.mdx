...
Learn how to troubleshoot common issues with NeMo Data Designer in a local NeMo Helix setup.
...
- Local Helix services are running from `nemo setup` or `nemo services run`.
...
Local setup writes service output to `~/.local/state/nhx/instances/<scope>/services.log` in the directory where services were started.

```bash
tail -n 200 ~/.local/state/nhx/instances/<scope>/services.log
tail -f ~/.local/state/nhx/instances/<scope>/services.log
```
...
- How-to: [Set Up NeMo Helix](/documentation/get-started) covers local setup and service startup.
