...
description: ""
---
The following reference provides detailed documentation for the synchronous and asynchronous clients of the NeMo Helix Python SDK.

## Synchronous Client

Use `nemo_helix.NeMoHelix` from regular Python code when you want blocking API calls.
...
from nemo_helix import DEFAULT_MAX_RETRIES, not_given
...
| `base_url` | Base URL for the NeMo Helix API. If omitted, the client reads it from the active CLI context. |
| `inference_base_url` | Optional override for inference gateway requests. Defaults to `base_url`. |
| `config_path` | Path to the CLI config file. Defaults to `~/.config/nhx/config.yaml`. |
...
| --- | --- | --- |
| CLI context | `NeMoHelix()` | Reads the active CLI context and uses configured authentication with token refresh. |
| Named context | `NeMoHelix(context_name="dev")` | Reads a specific CLI context instead of the active one. |
| Direct API | `NeMoHelix(base_url="http://localhost:8080")` | Uses the given URL without reading config or injecting auth headers. |
| Explicit token | `NeMoHelix(base_url=url, access_token=token)` | Uses the provided bearer token for API calls. |
...
Use `nemo_helix.AsyncNeMoHelix` from `asyncio` applications. It exposes the same
resource tree as `NeMoHelix`, but API methods are awaited.
...
`AsyncNeMoHelix` accepts the same connection and authentication parameters as
`NeMoHelix`. Pass a custom `httpx.AsyncClient` through `http_client` only when
...
API calls raise typed exceptions from `nemo_helix`, including:
...
Catch `APIStatusError` for all non-success HTTP responses, or `NeMoHelixError`
for SDK-level errors.

## Client Attributes

The NeMo Helix clients provide access to API resources through the following attributes.
