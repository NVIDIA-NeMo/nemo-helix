...
description: "Score agents and models in NeMo Helix — dataset-driven metrics over labeled data and task-driven agent evaluation — with the same scoring interface across local and platform execution."
---
...
<Button href="https://github.com/NVIDIA-NeMo/nemo-helix/tree/main/packages/nemo_evaluator_sdk" variant="secondary">**Open Source SDK**</Button>

---

## Three shapes of evaluation

Evaluation comes in three shapes, told apart by what produces the thing you score:
...
- **Retrieval-driven evaluation** — score how well a retriever ranks documents. A `Retrieval` target
  ranks a BEIR corpus against queries and relevance judgments, and retrieval metrics score the
  rankings. Reach for it when tuning embeddings or a reranker. Entry point: the `retrieve-eval` job,
  described in
  [Evaluation Approaches](/documentation/evaluate-models/dataset-driven-vs-task-driven-evaluation).

<Note>

**Metrics are the shared scorer, not a shape of their own.** The same deterministic and
LLM-as-a-judge scorers work in any of the three — what changes is the input being scored, not the
scorer.
...
### 2. Run it — two dataset-driven modes
...
|------|--------|------|----------|
| **Local SDK** | `Evaluator()` (`nemo_evaluator_sdk`) | `await evaluator.run(metrics=[metric], dataset=dataset, config=config)` | Fast in-process iteration with no Helix services. |
...
The platform caller is mounted on a `NeMoHelix` client:
...
that do not require running nemo-helix.
- Use the Evaluator plugin's `uv run nemo evaluator agent-evaluate` job for durable runs with inline
...
| **Inference** | Direct model or agent endpoint calls | The same, and can route through the NeMo Helix [Inference Gateway](/documentation/models-and-inference) and platform-managed endpoints |
| **Datasets** | Inline rows and local files | Inline rows, local paths resolved at submission time, and NeMo Helix [Filesets](/documentation/get-started/core-concepts/manage-files) |
| **Results** | Returned in memory | Platform artifact storage with typed result download |
| **Authentication** | Local environment variables | Local env vars for local runs; NeMo Helix [Secrets](/documentation/get-started/core-concepts/manage-secrets) for remote jobs |
