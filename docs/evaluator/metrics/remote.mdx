...
The Evaluator plugin SDK runs either type as a durable platform job through `evaluator.submit(metric=metric, dataset=dataset)`. To iterate on a metric in-process without Helix services, use the standalone `nemo_evaluator_sdk.Evaluator` instead.
