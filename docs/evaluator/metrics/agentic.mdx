...
# Force JSON-only judge output. RAGAS parses the judge's topic extraction and
# classification responses as structured JSON; verbose free-text replies fail to
# parse and are recorded as NaN scores instead of a valid result.
...
```python
from nemo_evaluator_sdk import InferenceParams, RunConfig, Model, SecretRef
from nemo_evaluator_sdk.metrics.ragas import TopicAdherenceMetric
...
# Force JSON-only judge output so RAGAS can parse the judge responses; verbose
# free-text replies otherwise fail to parse and are recorded as NaN scores.
metric = TopicAdherenceMetric(
    metric_mode="f1",
    judge_model=judge_model,
    inference=InferenceParams.model_validate({"temperature": 0, "response_format": {"type": "json_object"}}),
...
Confirm a real score: a valid result has `count` of at least 1, `nan_count` of 0, and a `mean` in the range `[0, 1]`. A result with `count: 0` and `nan_count: 1` means the judge output could not be parsed into a score — verify the JSON-only judge configuration shown above is applied.
...
```python
from nemo_evaluator_sdk import RunConfig, Model, SecretRef
from nemo_evaluator_sdk.metrics.ragas import AgentGoalAccuracyMetric
...
```python
from nemo_evaluator_sdk import RunConfigOnlineModel, InferenceParams, Model, SecretRef
from nemo_evaluator_sdk.metrics.ragas import AnswerAccuracyMetric
...
For models that support extended reasoning, such as `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning`, add system prompt and reasoning parameters to online model generation:

```python
from nemo_evaluator_sdk import RunConfigOnlineModel, InferenceParams, Model, ReasoningParams, SecretRef
