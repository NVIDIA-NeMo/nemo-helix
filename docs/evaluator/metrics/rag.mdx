...
from nemo_evaluator_sdk import RunConfigOnlineModel, RunConfig, InferenceParams, Model, SecretRef
...
Use `evaluator.submit(metric=metric, dataset=dataset)` for a durable platform job. For local in-process iteration without the platform, use `nemo_evaluator_sdk.Evaluator` directly:

```python
def submit_evaluation(evaluator, metric, dataset):
...
Reference secrets by name in your model configuration. For `nemo_evaluator_sdk.Evaluator` versus submitted-job behavior, see [Model API Authentication](/documentation/evaluate-models/metrics/model-configuration#model-api-authentication).
...
result = LocalEvaluator().run_sync(metrics=[metric], dataset=offline_rows, config=RunConfig(parallelism=8))
...
Noise Sensitivity uses RAGAS mode-qualified outputs internally. The SDK normalizes those results to the declared `noise_sensitivity` output name.
...
Reference secrets by name in your metric configuration. For `nemo_evaluator_sdk.Evaluator` versus submitted-job behavior, see [Model API Authentication](/documentation/evaluate-models/metrics/model-configuration#model-api-authentication).

```python
judge_model = Model(
    url="https://integrate.api.nvidia.com/v1/chat/completions",
    name="nvidia/nemotron-3.5-lightning-30b-a3b",
    api_key_secret=SecretRef(root="judge-api-key"),
)
```
...
```python
def submit_and_download_artifacts(evaluator, metric, dataset):
    job = evaluator.submit(metric=metric, dataset=dataset)
    job.wait_until_done()
