...
description: "Evaluate a simple agent end to end with the NeMo Evaluator SDK — define tasks and a metric, run locally, and read the scores and HTML report. No Helix services or API keys required."
---

In this quickstart you'll evaluate an agent on two tasks, score its answers, and produce a run
bundle with a browsable HTML report — all on your machine, with **no Helix services and no API
...
```bash
pip install nemo-helix[nemo-evaluator-sdk]
```
...
result = await AgentEvaluator().run(
    ...
    config=AgentEvalRunConfig(work_dir=Path("./agent-eval-run"), parallelism=2),
)
result.persist()
...
keyword_match.score.pass@1: 1.0
```

Alongside each metric's mean, the SDK reports a `pass@k` aggregate — here `pass@1`, because each
task ran a single trial. See [Reading Results](/documentation/evaluate-models/agent-eval/reading-results).
