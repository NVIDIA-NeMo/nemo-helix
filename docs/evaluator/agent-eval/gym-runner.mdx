...
The async client takes the same call — `await client.evaluator.submit(tasks=..., target=runner)` on
an `AsyncNeMoHelix`, returning an `AsyncAgentEvaluatorJobResource` with the same status and polling
methods. See [SDK Resources](/documentation/evaluate-models/sdk-resources).

Submitting from the live runner is not the only route. The same evaluation can be described as an
`agent-evaluate` spec with a `GymRunnerTarget` and submitted with the CLI, which is how a job that
stages a custom environment is usually run:

```bash
nemo evaluator agent-evaluate --spec-file gym_agent_eval.json
```

[Run a Custom Gym Environment](/documentation/evaluate-models/agent-eval/gym-custom-environment) has
a worked spec.

On a sandbox-enabled deployment, the Evaluator step runs in `nhx-tasks` and calls a separate
...
- **No retries and no re-run cache.** A failed rollout is not retried, and nothing is reused between
  runs: every run collects from scratch, and refuses to run into a directory that already holds Gym
  rollout output — rollouts, the failures sidecar, or model-call captures. (An existing empty
  directory is fine.) Re-running a large suite pays the full collection cost again. (The
