...
| `env_secrets` | no | `{ENV_NAME: secret-ref}` the service resolves into the job's environment for the harness to read |
...
If the agent is already registered with `nemo agents create`, name it in place of `config`:

```json
{"kind": "fabric", "agent": "calculator-agent"}
```
...
translates the platform `agent.yaml` into the `config` above. The persisted job spec therefore shows a
plain `config`; the job never looks an agent up. It runs the agent fresh for every trial and never
calls an existing deployment, so the evaluation is independent of what is (or is not) deployed.

| Field | Required | Notes |
|---|---|---|
| `agent` | with no `config` | `name` in the submission workspace, or `workspace/name`; exactly one of `agent` or `config` |
| `environment` | no | an environment spec (the same shape `nemo agents deploy` takes) merged onto the agent's config; requires `agent` |
| `timeout_s` | no | per-task timeout for the Fabric run |
| `capture_trajectory` | no | capture the ATIF trajectory and attach it to trial evidence |

`model` cannot be combined with `agent`. If the agent's config refers to skills or prompts by relative
path, its Ethos FileSet is staged into the job before the run so those paths resolve. The same `agent`
(and `environment`) fields exist on the [Harbor target](/documentation/evaluate-models/agent-eval/harbor-fabric-agent#run-a-registered-agent),
...
resolved Fabric target and the service injects them into the job's environment.
