...
- **The harness adapters and their runtime dependencies**, from the `fabric`
  extra:
...
- **Harness authentication**, when required. For example, authenticate Codex
  before running an evaluation. The Fabric packages provide the supported
  harness and `nemo-relay` binaries; no separate host CLI installation is
  required.

The `fabric` extra installs the Codex, Claude, and Hermes adapters and their
supported runtime packages. The deepagents adapter is deliberately excluded
from it, because that adapter does not support the Relay observability
configuration Fabric streaming generates; install its harness separately if
you need it.
