...
A trial contributes to a view only when every signal is present and finite. If a metric omits an
optional signal, the entire view reduction for that trial is unmeasured and contributes to the view's
`nan_count`. Views do not partially reduce the remaining signals or replace the missing signal with
zero.
...
used_expected_tool.tool_use.pass@1: 0.5
```
