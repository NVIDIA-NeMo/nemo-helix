...
- **Result** (`AgentEvaluator` → `AgentEvalResult`) — the evaluator orchestrates the run and returns
  the trials, per-trial scores, and an aggregated `summary`. Call `result.persist()` and it also
  writes a run **bundle**: `run.json`, `tasks.jsonl`, `trials.jsonl`, `scores.jsonl`, `summary.json`,
  `metadata.json`, and a browsable `report.html`.

## A minimal example

<Note>

Conceptual — `OutcomeMetric` below stands in for a metric you supply. For a runnable, end-to-end
version see the [quickstart](/documentation/evaluate-models/agent-eval/quickstart), which defines a
real `KeywordMatchMetric`.
...
- **Runs locally or durably.** A local run can produce the full bundle,
  including `report.html`, without Helix services. Use the plugin's
...
- [Run a Fabric Agent on Harbor](/documentation/evaluate-models/agent-eval/harbor-fabric-agent) —
  drive a Fabric harness as the agent inside a Harbor task container.
...
- [Configure Sandboxed Gym](/documentation/evaluate-models/agent-eval/gym-sandbox-configuration) —
  deployment settings that govern sandboxed Gym execution.
...
<Cards>
  <Card title="Evaluation Approaches" href="/documentation/evaluate-models/dataset-driven-vs-task-driven-evaluation" />
