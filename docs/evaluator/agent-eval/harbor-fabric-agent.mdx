...
- A task image with **CPython and bash**. `NemoFabricAgent` installs Fabric and the harness into the
  ...
  [`FabricInstalledAgent`](#run-on-a-task-image-you-do-not-control) instead, which brings its own
  Python.
...
## Run on a task image you do not control

`NemoFabricAgent` installs Fabric with the task image's own `python3`, which limits it to images that
ship one. Most Harbor tasks do not, and their Dockerfiles are not yours to change.
...
manager the image has, then `uv`, then a uv-managed CPython, and only then the Fabric virtualenv. Every
...
Swap the import path and drop the image requirement:
...
Three differences from `NemoFabricAgent`:
...
- `fabric_max_turns` defaults to `50` instead of Fabric's unbounded `None`. An unbounded harness on a
  task it cannot finish runs until Harbor kills the agent phase, and a killed phase writes **no**
  `RunResult` — no trajectory, no error, just `AgentTimeoutError`. On `terminal-bench-sample` that
  was six of ten trials. Size it for your dataset, or pass `fabric_max_turns=None` for Fabric's
  behaviour.
...
Two image requirements the agent cannot work around:
...
- **A working package manager**, when the image has no `curl`. Archived distributions are the trap:
  on `debian:bullseye-slim`, `apt-get install curl ca-certificates` now exits 100 because the baked-in
  index no longer resolves, and the image ships no curl, wget, python3, or busybox to fall back on.
  This breaks any installed agent, not just this one — terminal-bench's own `qemu-*` verifiers run the
  same apt-then-`uv` sequence and fail identically, so those tasks score `0` even under the oracle.
