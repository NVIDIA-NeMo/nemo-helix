...
reward-only scoring. Do not add another `HarborRewardMetric`; duplicate metric types are rejected
at job submission.

Upload without registration, add scoring to the verified definition, and register it. This example
adds an exact-match check against the agent's final answer; adapt the expected answer for your task.
Use a new task name for the first run, or the task resource's explicit `replace()` operation to
publish a scoring change to an existing task.

```python
from pathlib import Path

from nemo_evaluator.api.schemas import HarborTaskDefinition, TaskInput, TaskRef
...
uploaded = upload_harbor_task(Path("./my_task"), client=client, register=False)
metric = to_inline(bundle_metric(
    ExactMatchMetric(reference="Done", candidate="{{sample.output_text}}"),
    CloudpickleMetricBundlePackager(),
))

definition = HarborTaskDefinition.model_validate({
    **uploaded.definition.model_dump(),
    "metrics": [metric],
    "views": {
        "quality": SemanticView(reducer=SemanticReducer.MEAN, signals=[
...
task = tasks.create("my_task_scored", task=TaskInput(spec=definition))

# This example creates a new task; its first revision is in the returned history page.
revision = next(item for item in tasks.list_revisions(task.name).data if item.revision == task.revision)
task_ref = TaskRef(f"default/{task.name}#{revision.content_hash}")
spec = AgentEvalInputSpec(tasks=[task_ref], target=HarborRunnerTarget(agent_name="oracle"))
job = evaluator_client.submit_agent_eval_job(
    body=SubmitAgentEvalJobRequest(spec=spec.model_dump(mode="json")),
).data()
```

- **Reuse:** Supply existing references in `metrics`; include `task_ref` in a taskset's `tasks`.
- **Scoring resolution:** Submit task or taskset references only. Evaluator loads the
  metrics and views from each selected task revision. The runner always reports the Harbor
  verifier's primary reward, even when the task's `metrics` list is empty. Configure only additional
  metrics. Do not add `HarborRewardMetric` yourself since it's set provided by default.
- **Task publication:** Use `upload_harbor_task()` or `upload_harbor_dataset()` to publish Harbor
  tasks. The helpers create and verify the required archive and register it by default. Uploading an archived
  task directory or Harbor dataset bypassing these functions will lead to a failure.
...
Submit stored task or taskset references with saved SDK trials and omit `target`. The worker
loads pinned definitions and applies `HarborRewardMetric`, additional metrics, and views without
downloading task archives or running the agent or verifier. Metric evidence such as traces and
...
client = NemoClient(base_url="http://localhost:8080", workspace="default")
spec = AgentEvalInputSpec(tasks=TasksetRef("default/suite"), trials=trials)
job = EvaluatorClient.from_client(client).submit_agent_eval_job(
    body=SubmitAgentEvalJobRequest(spec=spec.model_dump(mode="json")),
).data()
```
...
- **Resolution and size:** New submissions resolve metric/model dependencies again; compiled-job
  retries retain resolved scoring. Job size grows with task count and metric bundles.
- **Reward key:** Every trial must contain the same valid `metadata["harbor_primary_reward_key"]`
  (for example, `"grade"`). New Harbor adaptation records it even on failed trials. Rescoring
...
- **Older trials:** Missing reward-key metadata is rejected. Re-import original results with the
  current SDK's `build_trials_from_job_dir(job_dir, tasks, reward_key="grade")` from
  `nemo_evaluator_sdk.agent_eval.runtimes.harbor_runtime`, using SDK tasks with the original native
  IDs and the original reward key (`"grade"` is illustrative). This rebuilds metadata without
