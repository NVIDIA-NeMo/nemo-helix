...
- **`agent_kwargs`** — keyword arguments forwarded to the agent's constructor, the equivalent of
  ...
  (`api_key`, `token`, `secret`, ...) redacted. Name credentials in `agent_env_from_host` instead.
...
them as `runner.agent_kwargs`. Instead of `agent_env_from_host`, the platform target takes `env_secrets`, a mapping
