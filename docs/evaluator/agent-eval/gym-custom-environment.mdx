...
The same submission from Python, with the runner object rather than a spec file. The runner carries
the evaluation and a `GymPlacement` carries the deployment, so the configuration is written once and
