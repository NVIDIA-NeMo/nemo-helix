...
A custom Gym environment runs from a NeMo Helix FileSet rather than from the components bundled
...
- A running NeMo Helix with the Evaluator plugin.
...
target:
  ...
  concurrency: 1
  hydra_params:
    policy_base_url: http://nemo-helix-api.default.svc.cluster.local:8080/apis/inference-gateway/v2/workspaces/default/model/my-model/-/v1
...
Submit the runner object rather than the spec file. The runner carries the evaluation and a
`GymPlacement` carries the deployment, so the configuration is written once and
`client.evaluator.submit` returns a job handle that polls itself:
...
runner = GymAgentTaskRunner(
    config=GymRuntimeConfig(
        ...
        agent_config="responses_api_agents/simple_agent/configs/simple_agent.yaml",
        resources_server="custom_greeting",
        env_secrets={"EXTERNAL_MODEL_API_KEY": SecretRef("default/my-model-api-key")},
    )
)

job = client.evaluator.submit(
    tasks=TasksetRef("my-gym-taskset"),
    target=runner,
    placement=GymPlacement(environment=FilesetRef(root="default/my-gym-environment")),
)
job.wait_until_done()
```

The same runner runs locally under `AgentEvaluator()` without change — `env_secrets` resolves from
your own environment there. Only the placement is specific to the platform, which is why it is not
part of the runner.
...
2. `agent-evaluate` provisions `nhx-gym-host`, collects rollouts, scores them, and destroys the host.

Both platform steps use `nhx-cpu-tasks`; Gym and Ray run in the separate sandbox host.
