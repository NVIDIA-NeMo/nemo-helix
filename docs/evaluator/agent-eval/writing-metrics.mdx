...
Declare an output as optional when the metric cannot (or not expected) measure it on every otherwise scoreable trial.
Return the output when measured and omit it when unmeasured:
...
See [Reading Results](/documentation/evaluate-models/agent-eval/reading-results) for the two-trial `format_ok` example.

#### Not recommended

A metric author is allowed to emit a filler. That puts the trial in the measured set and changes aggregates:

- `0.0` is a finite measurement: it enters `count` and `mean`, and coverage counts it as
  `scored`. The mean is pulled toward zero as if the metric measured failure.
- NaN is observed but non-finite: it is excluded from `count` and `mean`, increments
  `nan_count`, and coverage still counts it as `scored` (not `missing`). The trial looks
...
[`TrialMeasurements`](https://github.com/NVIDIA-NeMo/nemo-helix/blob/main/packages/nemo_evaluator_sdk/src/nemo_evaluator_sdk/agent_eval/trials.py)
...
See [Reading Results](/documentation/evaluate-models/agent-eval/reading-results) for exact examples. To
roll several outputs into one reported score, define a **view** on the task — see
