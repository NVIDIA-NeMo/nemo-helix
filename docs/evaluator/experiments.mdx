...
Experiments are part of the `intake` service, so any running NeMo Helix already serves them. Point
...
container for the resolved NeMo data directory unless `NHX_INTAKE_CLICKHOUSE_URL` explicitly points
...
curl --fail-with-body -i \
  ...
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/experiments?sort=-created_at&page=1&page_size=1"
curl --fail-with-body -i \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluations?sort=-cost_usd.mean&page=1&page_size=1"
```
...
curl --fail-with-body -X POST \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/experiments" \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
...
export EXPERIMENT_ID=$(curl --fail-with-body --silent \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/experiments/reranker-prompt-iteration" \
...
curl -g --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluations?filter[experiment_id]=$EXPERIMENT_ID&page=1&page_size=20"
```

Then open `$NHX_STUDIO_URL/workspaces/$WORKSPACE/experiment`, then open
...
appear on both a per-project board and a cross-project benchmark. To demonstrate adding a second membership,
first create the second Experiment and capture its ID:
...
export OTHER_EXPERIMENT_ID=$(curl --fail-with-body --silent \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/experiments/reranker-cross-project-benchmark" \
  | python3 -c 'import sys,json; print(json.load(sys.stdin)["id"])')
```
...
curl --fail-with-body -X PATCH \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluations/reranker-bge-large" \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
...
curl -g --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluations?filter[experiment_id]=$EXPERIMENT_ID&sort=-evaluators.solved.mean&page=1&page_size=20"
```
...
curl -g --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluations?filter[experiment_id]=$EXPERIMENT_ID&filter[cost_usd.mean][\$lte]=0.5"
```
...
curl --fail-with-body -X POST \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluations/reranker-main-baseline/pin"
...
curl --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluations/reranker-add-cross-encoder/sessions?page=1&page_size=20"
```
...
curl -g --fail-with-body \
  -H "Authorization: Bearer $NHX_ACCESS_TOKEN" \
  "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluations?filter[is_deleted]=true&page=1&page_size=20"
```
