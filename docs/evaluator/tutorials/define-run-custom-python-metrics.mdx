...
Install the PyPI `nemo-helix` wrapper package, which includes the Evaluator SDK. The `all` extra is defined by the wrapper package; if you are working from a source checkout, run `make bootstrap` from the repository root instead.
...
You do not need a running NeMo Helix instance to define the metric or run it locally. Start NeMo Helix and configure platform resources before submitting the durable remote job.
...
`compute_scores(...)` receives a `MetricInput` object as `input`:

- `input.row.data` contains the original dataset row, including any canonical fields produced by field mapping.
- `input.candidate.output_text` contains the candidate output. For offline evaluations, the Evaluator SDK can populate this from a mapped dataset column. For online evaluations, it contains the generated model output.
...
```python
from nemo_evaluator_sdk import Metric, MetricInput, MetricOutputSpec, MetricResult
...
The scoring method uses `input.candidate.output_text`, where the evaluator stores the candidate output. The metric does not need to look for dataset-specific prediction columns or implement a row-level fallback. For offline rows, use field mapping to normalize your prediction column into the evaluator's canonical `output` field before the metric runs. If the evaluator does not provide a candidate output, the metric treats the row as a failure.
...
class ArithmeticExpressionCorrectnessMetric(Metric):
    ...
    async def compute_scores(self, input: MetricInput) -> MetricResult:
        expression = input.candidate.output_text
        if not expression:
            return MetricResult(
                outputs=[
        ...
        expected = float(input.row.data["expected"])
        tolerance = float(input.row.data.get("tolerance", 1e-6))
...
For an offline evaluation, datasets often store predictions under task-specific column names such as `model_expression`, `answer`, or `prediction`. Use `FieldMapping` to map that column to the evaluator's canonical `output` field. The Evaluator SDK then passes it to your metric as `input.candidate.output_text`.
...
With this mapping in place, every row still preserves its original `model_expression` field in `input.row.data`, and the metric receives the normalized candidate output through `input.candidate.output_text`.

## 6. Prepare Platform Resources

To run the same metric remotely, install and start NeMo Helix using the [Setup guide](/documentation/get-started). You also need:

- A model entity that can be referenced as `workspace/model-name`. See [Model Configuration](/documentation/evaluate-models/metrics/model-configuration) for model setup details.
...
MODEL_REF = os.environ.get("NHX_EVAL_MODEL_REF", "default/my-model")
...
```python
from nemo_helix import ConflictError
...
Pass `CloudpickleMetricBundlePackager()` to `client.evaluator.submit(...)` so the SDK serializes the metric object into the evaluator job spec. The job runtime hydrates the metric bundle before scoring rows. Use `ModelRef` to reference the platform model entity you configured with `NHX_EVAL_MODEL_REF`.
...
- Learn about built-in metric types in [Evaluation Metrics](/documentation/evaluate-models/metrics).
- Learn how to configure model targets in [Model Configuration](/documentation/evaluate-models/metrics/model-configuration).
