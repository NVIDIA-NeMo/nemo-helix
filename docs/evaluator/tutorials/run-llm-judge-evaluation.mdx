...
1. Install and start NeMo Helix using the [Setup guide](/documentation/get-started).

For PyPI installs, use the `nemo-helix` wrapper package. If you are working from a source checkout, run `make bootstrap` from the repository root instead.
...
- **Fileset**: A dataset registered with NeMo Helix. The evaluator plugin SDK accepts fileset references directly, so this tutorial passes the registered HelpSteer2 split to evaluations as a `FilesetRef`.
...
- **Evaluation**: The process of scoring model outputs using one or more metrics. Use `evaluator.submit(...)` for durable jobs, or `nemo_evaluator_sdk.Evaluator` for local in-process execution
...
from nemo_helix import ConflictError, NeMoHelix

# Workspace for this tutorial.
WORKSPACE = "llm-as-judge-tutorial"

# Create an SDK instance to interact with the NeMo Helix.
client = NeMoHelix(
...
Local runs resolve `Model(api_key_secret=SecretRef(root="NVIDIA_API_KEY"))` from your local environment. Remote jobs run in the platform job runtime, so they use the platform secret name created above.
...
This tutorial uses `nvidia/nemotron-3.5-lightning-30b-a3b` from NVIDIA Build.
...
JUDGE_MODEL_NAME = "nvidia/nemotron-3.5-lightning-30b-a3b"

# `nemo_evaluator_sdk.Evaluator` resolves this name from your local environment.
LOCAL_JUDGE_MODEL = Model(
    url=JUDGE_MODEL_URL,
    name=JUDGE_MODEL_NAME,
    api_key_secret=SecretRef(root=NVIDIA_BUILD_API_KEY_ENV),
)
...
REMOTE_JUDGE_MODEL = Model(
    url=JUDGE_MODEL_URL,
    name=JUDGE_MODEL_NAME,
    api_key_secret=SecretRef(root=nvidia_api_key_secret.name),
)
```
...
Before running a durable job, test your metric with a few examples using `nemo_evaluator_sdk.Evaluator`. It runs locally in-process and returns results immediately, which is useful for prompt iteration.

```python
from nemo_evaluator_sdk import Evaluator as LocalEvaluator

quick_test_result = LocalEvaluator().run_sync(
    metrics=[metric_v1_local],
    dataset=[
        {
...
# Delete remote evaluation jobs. Local SDK results are in-memory
