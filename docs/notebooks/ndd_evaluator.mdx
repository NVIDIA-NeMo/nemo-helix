...
First, create a client for interacting with NeMo Helix.
...
# Initialize NeMoHelix client (does not trigger any action yet)
...
Note: these resources may already exist if you are running NeMo Helix Quickstart.
...
MODEL_ID_GENERATOR = "nvidia/nemotron-3.5-lightning-30b-a3b"
...
def filter_by_scores(df, min_coherence=3, samples_per_complexity=100):
    ...
    for idx, k in enumerate(df['triage_note_quality']):
        try:
            k_dict = ast.literal_eval(k) if isinstance(k, str) else k
...
from nemo_helix.filesets import build_fileset_ref
...
from nemo_helix.types.evaluation import (
 FilesetRef,
