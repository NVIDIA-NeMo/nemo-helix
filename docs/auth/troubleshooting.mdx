...
- Using email-as-API-key (`X-NHX-Principal-Id`) when OIDC is enabled and native validation rejects it
...
Your IdP doesn't have device flow enabled for the NeMo Helix application.
...
The `client_id` in NeMo Helix config doesn't match the application in your IdP.
...
## "Gateway Bearer-Auth Callout Isn't Working"

**Cause**: Headers are not being set or stripped correctly by the gateway.

**Diagnosis**:

1. Check that the gateway calls `/apis/auth/authenticate` with the original `Authorization` header before forwarding protected requests.

2. Check that the gateway forwards the trusted `X-NHX-Principal-*` and `X-NHX-Scopes` headers returned by the auth service on successful authentication.

3. Check that auth headers are stripped from external requests. Try sending a request with `X-NHX-Principal-Id` or `X-NHX-Scopes` from outside the cluster — those headers should be stripped by the gateway.

4. Verify the PDP is reachable from platform services. If using external OPA, check that the OPA sidecar/service is running and the bundle endpoint is accessible.
...
## "HelixAdmin Can't Access a Workspace"
...
- [Auth Configuration](/documentation/access-control/deployment) — Configuration reference.
- [Security Model](/documentation/access-control/security-model) — Architecture and trust boundaries.
- [OIDC Setup](/documentation/access-control/authentication/oidc-setup) — IdP configuration.
- [Gateway Integration](/documentation/access-control/deployment/gateway-integration) — Gateway auth and headers.
