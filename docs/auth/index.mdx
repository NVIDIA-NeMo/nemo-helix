...
description: ""
---
NeMo Helix includes a built-in security layer that lets you control who can access your platform and what they can do. When multiple teams or users share a NeMo Helix deployment, authentication and authorization ensure that each user sees only the workspaces and resources they are permitted to access, and can only perform actions appropriate to their role.

Access control has two layers:

-   **Authentication** — Prove your identity. NeMo Helix validates a JWT issued by your OpenID Connect (OIDC) identity provider.
...
NeMo Helix authenticates every request using a JWT from your OIDC identity provider. The token is sent in the `Authorization: Bearer <token>` header, and NeMo Helix validates the signature, issuer, audience, and expiry. Refer to [OIDC Setup](/documentation/access-control/authentication/oidc-setup) to connect your identity provider.
...
**Quickstart shortcut** — When running NeMo Helix quickstart without an OIDC provider, you can use an unsigned JWT:
...
Run `nemo quickstart configure` and select **Configure advanced options** → **Yes** for authentication. Provide an admin email — it receives the **HelixAdmin** role with full platform access.
...
```text
NeMo Helix Quickstart Configuration
...
...
Admin email (grants HelixAdmin role): admin@example.com
...
For production or Helm-based deployments, enable auth by setting `platformConfig.auth.enabled: true` in your Helm values and configure the `auth:` section in platform config. Refer to [Auth Configuration](/documentation/access-control/deployment) for the full reference and [OIDC Setup](/documentation/access-control/authentication/oidc-setup) to connect your identity provider.
...
Understand how NeMo Helix authentication and authorization work together — trust boundaries, principal model, and authorization layers.

</Card>
<Card title="Connect an Identity Provider" href="/documentation/access-control/authentication/oidc-setup">

Configure NeMo Helix to authenticate users using your OIDC identity provider.
...
<Card title="Configuration Reference" href="/documentation/access-control/deployment">
