...
description: ""
---
NeMo Helix uses **workspace-scoped RBAC**: resources live in workspaces, users get roles (Viewer, Editor, Admin) per workspace, and a policy engine evaluates every request against role bindings and token scopes.
...
flowchart BT
    ...
    HelixAdmin["HelixAdmin<br/>all operations across all workspaces"]
...
The **HelixAdmin** role is a special highly privileged role that provides full access to all workspaces and all operations across the entire platform. This role is typically reserved for platform operators.
...
The HelixAdmin role is granted via the `admin_email` config setting. See [Auth Configuration](/documentation/access-control/deployment).
...
NeMo Helix automatically provisions role bindings for the wildcard principal `*` on certain workspaces:
...
For technical details on the policy engine, see [Policy Engine](/documentation/access-control/authorization/policy-engine). For configuration, see [Auth Configuration](/documentation/access-control/deployment).
