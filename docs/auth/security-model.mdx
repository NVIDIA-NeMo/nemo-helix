...
description: ""
---
This page describes the security architecture of NeMo Helix authentication and authorization — how requests are authenticated, how access decisions are made, and where trust boundaries lie. It is intended for platform operators and security reviewers evaluating NeMo Helix for production deployment.
...
sequenceDiagram
    ...
    participant Service as NeMo Helix Service
    ...
    Auth-->>Gateway: Trusted X-NHX-Principal-* and X-NHX-Scopes headers
...
3. The validating component derives trusted `X-NHX-Principal-*` and `X-NHX-Scopes` headers.
...
In quickstart deployments without an OIDC provider, the `X-NHX-Principal-*` headers are set directly by the client instead of being derived from a validated JWT. See [Getting Started](/documentation/access-control).

</Note>
## Authentication Modes

NeMo Helix supports two authentication modes: **service-level** bearer-token validation and **gateway bearer-auth callout**.

In both modes, identity arrives in the `Authorization: Bearer <token>` header. The bearer token is validated exactly once — either by the first NeMo Helix service or by the auth service behind the gateway callout. After validation, the authenticated identity (email, subject, groups) is propagated to downstream services via **trusted `X-NHX-Principal-*` headers**. Downstream services accept these headers without re-validating the token, but they still run authorization checks.

This "validate once, propagate via headers" design means that **network perimeter security is critical**: anything inside the trust boundary that receives `X-NHX-Principal-*` headers will trust them unconditionally. The gateway must strip these headers from all incoming external requests to prevent clients from forging an identity. See [Gateway Integration](/documentation/access-control/deployment/gateway-integration).

### Service-Level Authentication

The first NeMo Helix service that receives the request validates the bearer token directly:
...
5. Forwards `X-NHX-Principal-*` headers to downstream services
...
2. Auth service validates the bearer token and returns trusted `X-NHX-Principal-*` and `X-NHX-Scopes` headers
...
This rejects unauthenticated requests before they reach Helix services while keeping authorization decisions in the normal service middleware path.
...
When OIDC is enabled, the principal is resolved from JWT claims: the `sub` claim becomes the principal ID (or `oid` for Azure AD), the `email` claim provides the email (or `upn` for Azure AD), and group memberships come from the `groups` claim. These claim names are configurable. In gateway deployments, the auth service performs this extraction and the gateway forwards the returned `X-NHX-Principal-*` headers.

<Note>

**Quickstart shortcut** — When running without OIDC (`email-as-API-key` mode), the principal is the raw value of the `X-NHX-Principal-Id` header, without any token validation. This is intended for quick testing only.
...
| Header | Description |
|---|---|
| `X-NHX-Principal-Id` | Unique identifier for the principal (required). Resolved from the JWT `sub` claim (or `oid` for Azure AD). |
| `X-NHX-Principal-Email` | The principal's email address. |
| `X-NHX-Principal-Groups` | Comma-separated list of groups the principal belongs to (from JWT group claims). |
| `X-NHX-Scopes` | Space-separated list of token scopes (extracted from the JWT `scp` or `scope` claim). Used by the PDP for scope-based authorization checks. |
...
In these cases, the calling service sets the `X-NHX-Principal-On-Behalf-Of` header to a JSON object containing the original user's identity:
...
Many NeMo Helix operations — customization, evaluation, synthetic data generation — run as asynchronous jobs. When a user submits a job, the platform propagates the submitting user's identity into the job container via the `NHX_PRINCIPAL` environment variable.

<Note>

Job containers need to run inside the trust boundary — they call NeMo Helix APIs using the propagated identity headers and are subject to the same authorization checks as any other caller. The key distinction is that jobs act as the *user*, not as a privileged service principal, so they can only access resources the submitting user is permitted to reach.

</Note>
## Authorization: Workspace-Scoped RBAC

NeMo Helix uses **workspace-scoped Role-Based Access Control (RBAC)**. All resources (models, datasets, jobs, evaluations) belong to exactly one workspace, and access is controlled at the workspace level — not per-resource.
...
On top of RBAC, NeMo Helix supports **API scopes** as a second authorization layer at the token level. Every authorized request passes through two independent checks:
