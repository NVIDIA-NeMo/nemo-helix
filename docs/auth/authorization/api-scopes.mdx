...
The `entities:read` and `entities:write` scopes are for internal service-to-service usage. The generic entities API is not accessible to regular users — only HelixAdmin and service principals can access it. Users interact with entities through feature-specific APIs (Models, Evaluation, etc.) which use their own scopes.
...
If your IdP prefixes scopes (e.g., Azure AD uses `api://client-id/platform:read`), configure `scope_prefix` in the NeMo Helix OIDC settings so the platform strips the prefix before authorization:
...
If you use per-API scopes, each scope (e.g., `api://nhx/models:read`) must be exposed individually in the IdP app registration.
...
- **HelixAdmin**: Bypasses scope checking entirely
