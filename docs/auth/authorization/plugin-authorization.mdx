...
Factory-generated routes admit **both** caller kinds — `PRINCIPAL` and `SERVICE_PRINCIPAL` — and this is not configurable. Plugin controllers and other Helix services call these routes as `service:<name>`, and a principal-only route is an unconditional **deny** for a service principal: it overrides even the `ServiceSystem` wildcard, so no role grant recovers it.
