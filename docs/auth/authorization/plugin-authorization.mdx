...
### Letting a controller call its own generated routes

Factory-generated routes are `PRINCIPAL`-only by default, and a principal-only route is an unconditional **deny** for service principals — it overrides even the `ServiceSystem` wildcard. So if your plugin's controller (or any other service-to-service caller) drives these routes, it will get a 403 unless you widen them.

Name the verbs the caller actually needs, and nothing more:

```python
add_job_routes(
    MyJob,
    authz=scope,
    # This plugin's controller submits jobs and polls for in-flight ones.
    service_principal_verbs={"create", "list"},
)

add_function_routes(SweepFunction, authz=scope, allow_service_principals=True)
```

Valid job verbs are `create`, `list`, `read`, `delete`, `cancel`, `pause`, and `resume`; an unknown verb is a `ValueError` rather than a silent no-op. A verb covers every route bound to that permission, so `read` widens get/status/logs/results/downloads together. Both kwargs require `authz` — without a rule to ride on, the opt-in would be discarded.
