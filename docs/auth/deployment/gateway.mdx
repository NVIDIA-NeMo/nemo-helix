...
description: ""
---
In production, a gateway (reverse proxy, ingress controller, or service mesh) often sits in front of the NeMo Helix. This page explains the recommended enforced gateway path for bearer-token authentication, what headers the gateway must set, and which paths skip authorization.

For the security architecture, see [Security Model](/documentation/access-control/security-model).

## Overview

NeMo Helix services trust identity headers only after a bearer token has been validated. In gateway deployments, the recommended path is:

1. The gateway calls `/apis/auth/authenticate` before forwarding protected API requests.
2. The auth service validates the bearer token and returns trusted `X-NHX-Principal-*` and `X-NHX-Scopes` headers.
3. The gateway forwards those trusted headers to NeMo Helix services.
...
**Security Requirement**: Your ingress/gateway **must** strip the following headers from all incoming external requests before forwarding to NeMo Helix:

- `X-NHX-Principal-Id`, `X-NHX-Principal-Email`, `X-NHX-Principal-Groups`, `X-NHX-Principal-On-Behalf-Of`
- `X-NHX-Scopes`
...
| Header | Description |
|--------|-------------|
| `X-NHX-Principal-Id` | Principal identifier (e.g., user ID or email). Required. |
| `X-NHX-Principal-Email` | User email (optional but recommended). |
| `X-NHX-Principal-Groups` | Comma-separated group names (optional). |
| `X-NHX-Scopes` | Space-separated token scopes (optional). |
...
Configure your gateway to remove NeMo Helix auth headers from incoming external requests. This prevents clients from forging identities.
...
1. **Headers are set correctly** — Make a request through the gateway and check that `X-NHX-Principal-Id` and `X-NHX-Scopes` are present on the service side when the token contains scopes.
2. **Headers are stripped from external requests** — Try sending `X-NHX-Principal-Id` or `X-NHX-Scopes` from outside; verify they are stripped by the gateway.
