...
When authorization is enabled, a platform administrator can be configured. Setting **`admin_email`** tells platform seed which identity should receive the **HelixAdmin** role. Use it to create the first workspaces and grant roles to other users. After bootstrap, manage access via workspaces and members as described in [Managing Access](/documentation/access-control/authorization/managing-access).
...
The binding is created by `platform-seed`, not by auth service startup alone. Helm deployments run the platform-seed Job by default. For source installs, set `platform.seed_on_startup: true`, export `NHX_SEED_ON_STARTUP=true` before `nemo services run`, or run `uv run python -m nhx.platform_seed` after services are healthy. If platform seed has not run, `admin_email` users can authenticate but still receive `403 Forbidden` because their HelixAdmin binding does not exist.
...
Configuration can be overridden with environment variables using the **`NHX_AUTH_`** prefix. Names are derived from the config keys in UPPER_SNAKE_CASE.

Examples:

```bash
NHX_AUTH_ENABLED=true
NHX_AUTH_POLICY_DECISION_POINT_BASE_URL=http://auth:8000
NHX_AUTH_POLICY_DECISION_POINT_PROVIDER=embedded
NHX_AUTH_ADMIN_EMAIL=admin@example.com
NHX_AUTH_EMBEDDED_PDP_AUTO_BUILD_WASM=true
NHX_AUTH_TOKEN_SIGNING__PRIVATE_KEY_FILE=/etc/nhx/workload-token/private-key.pem
```

Nested auth keys use a double underscore after `NHX_AUTH_`: for example,
`NHX_AUTH_OIDC__ISSUER`, `NHX_AUTH_OIDC__CLIENT_ID`, and
`NHX_AUTH_ACCESS_KEYS__ENABLED`.
...
They are not a standalone NeMo Helix plugin.
...
auth:
 enabled: true
 token_signing:
   issuer: "https://nhx.company.com/apis/auth"
...
```bash
NHX_AUTH_TOKEN_SIGNING__ISSUER=https://nhx.company.com/apis/auth
NHX_AUTH_TOKEN_SIGNING__KEY_ID=nemo-helix-signing
NHX_AUTH_TOKEN_SIGNING__PRIVATE_KEY_FILE=/etc/nhx/workload-token/private-key.pem
NHX_AUTH_ACCESS_KEYS__ENABLED=true
NHX_AUTH_ACCESS_KEYS__ISSUE_FORMAT=jwt
NHX_AUTH_ACCESS_KEYS__ACCEPTED_FORMATS=jwt
NHX_AUTH_ACCESS_KEYS__AUDIENCE=nemo-helix-access-key
NHX_AUTH_ACCESS_KEYS__DEFAULT_EXPIRES_IN_SECONDS=2592000
NHX_AUTH_ACCESS_KEYS__MAX_EXPIRES_IN_SECONDS=2592000
NHX_AUTH_ACCESS_KEYS__ROTATION_GRACE_PERIOD_SECONDS=172800
```

List-valued env vars for accepted Scoped Access Key formats are written as
comma-separated values, for example `NHX_AUTH_ACCESS_KEYS__ACCEPTED_FORMATS=jwt`.
...
Workload token exchange is a platform-wide mode. When
`auth.oidc.workload_token_exchange_enabled=true`, managed workloads that need
creator-scoped platform access use exchanged bearer tokens, and trusted
`X-NHX-*` identity headers are rejected on protected service routes. When it is
disabled but `auth.enabled=true`, the platform uses trusted-header mode.
...
When `NHX_WORKLOAD_IDENTITY_TOKEN_FILE` is present, the SDK reads the subject
...
`auth.oidc.workload_scope` is a blanket allow-list for scopes minted by the
current workload token exchange endpoint. It is not a per-service or
per-workload grant model. Use it conservatively; per-service and per-workload
scope grants should be modeled with stable service identities and role bindings
when those grants are available.
...
context receive a NeMo Helix opaque workload proof token at
`NHX_WORKLOAD_IDENTITY_TOKEN_FILE`.
