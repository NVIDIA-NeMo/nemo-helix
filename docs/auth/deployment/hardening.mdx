...
description: ""
---
Security checklist for deploying NeMo Helix to production. Work through each section and verify your deployment meets these requirements.

For the security architecture, see [Security Model](/documentation/access-control/security-model). For configuration details, see [Auth Configuration](/documentation/access-control/deployment).
...
- [ ] **Disable password grant in production**: Password grant bypasses MFA. If your IdP supports it, disable the resource owner password grant for the NeMo Helix application registration. Restrict it to dedicated service accounts if CI/CD requires it.
- [ ] **Set `admin_email` to a real platform admin**: Use a specific person's email, not a shared mailbox. The HelixAdmin role bypasses all authorization checks.
...
- [ ] **Review additional issuers**: If `additional_issuers` is configured, verify all listed issuers are trusted. Each issuer can produce tokens that NeMo Helix will accept.
...
- [ ] **Restrict HelixAdmin**: Only one email should have HelixAdmin. This role bypasses all authorization — treat it like a root account.
...
- [ ] **Strip auth headers from external requests**: Configure your ingress/gateway to remove `X-NHX-Principal-Id`, `X-NHX-Principal-Email`, `X-NHX-Principal-Groups`, `X-NHX-Principal-On-Behalf-Of`, and `X-NHX-Scopes` from all incoming external traffic. See [Gateway Integration](/documentation/access-control/deployment/gateway-integration).
...
- [ ] **Consider gateway bearer-auth callout**: To enforce bearer-token validation before requests reach Helix services, configure Envoy `ext_authz` to call `/apis/auth/ext-authz`. See [Gateway Integration](/documentation/access-control/deployment/gateway-integration).
...
- [ ] **Verify CLI config file permissions**: Token storage at `~/.config/nhx/config.yaml` should have permissions `0600` (owner read/write only). Avoid storing this file in cloud-synced or shared directories. See [Using Authentication — Config File](/documentation/access-control/authentication/using-authentication#config-file) for full guidance.
...
- [Auth Configuration](/documentation/access-control/deployment) — Full configuration reference.
