...
Cluster             https://nhx.company.com
Context             default
Config File         /home/alice/.config/nhx/config.yaml
Auth Type           oauth
...
For CI pipelines, use the password grant to obtain a token without a browser: `nemo auth login --username <user> --password <pass>` (or set `NHX_OIDC_USERNAME` / `NHX_OIDC_PASSWORD` environment variables). If your CI system can obtain tokens directly (e.g., workload identity federation), pass the token via `access_token` as shown in [Make API Calls](#make-api-calls) below.
...
curl -H "Authorization: Bearer $TOKEN" \
 https://nhx.company.com/apis/entities/v2/workspaces
```
...
command and is not a separate NeMo Helix plugin.
...
```bash
curl -H "Authorization: Bearer $NHX_SCOPED_ACCESS_KEY" \
 https://nhx.company.com/apis/entities/v2/workspaces
```
...
`--service-account` requires the HelixAdmin role; omit it to create a
personal, user-bound key instead:

```bash
nemo auth access-keys create --name "intake-worker" --service-account service-id \
...
applies even to a HelixAdmin's own scoped key: the HelixAdmin bypass only
grants access within the key's own scope, so a HelixAdmin who scopes down their
...
- **SDK**: Refreshes lazily before each API call when the token is within 60 seconds of expiry. No background threads or timers — the cost is paid only when a refresh is actually needed (typically once per hour). Multiple `NeMoHelix()` clients in the same Python process share a single token, so only one refresh happens even with many clients.
...
Tokens are stored in `~/.config/nhx/config.yaml`:
...
- **File permissions**: Ensure `0600` (owner read/write only). The CLI sets this by default — verify after manual edits: `chmod 600 ~/.config/nhx/config.yaml`.
