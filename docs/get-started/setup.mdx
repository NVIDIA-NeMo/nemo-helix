...
Install NeMo Helix and start working with AI agents.
...
- For the PyPI install: [uv](https://docs.astral.sh/uv/) `>=0.10.10`, or `pip` in a Python environment
...
Use the [PyPI package](https://pypi.org/project/nemo-helix/) for the quickest local install. The `all` extra includes the SDK, CLI, platform services, and default first-party plugins; drop it for the SDK and CLI alone.
...
Use the source checkout when you are contributing to NeMo Helix, editing local plugins, building Studio assets from source, or running development workflows:

<Note>
Install [Flox](https://flox.dev/docs/install-flox/install) before running `make bootstrap` for the smoothest setup. It supplies the repository-pinned uv, Node.js, and pnpm toolchain; you do not need to activate it first.
</Note>
...
Without Flox, install the system toolchain versions printed by `make toolchain-versions`, plus a C compiler, then run `make TOOLCHAIN=system bootstrap` followed by `source .venv/bin/activate`.

If Studio asset bootstrap fails, the API can still run but Studio is unavailable until the web bundle is built. Ensure Flox is installed, or provide the required system Node.js and pnpm versions with `TOOLCHAIN=system`, then rerun `make bootstrap-studio` from the repository root.
...
5. **Select agent models** — pick a default model for quality-critical work and a fast model for latency-sensitive work; reuse the default if one model is enough
...
```bash
export NHX_SEED_ON_STARTUP=true
nemo services run
```
...
`uv run nemo-helix run task --task nhx.platform_seed` is equivalent. The seed task is idempotent. It creates the **HelixAdmin** binding in the `system` workspace for `auth.admin_email`, the wildcard **Editor** binding for the default workspace, the wildcard **Viewer** binding for the `system` workspace, and the wildcard **WorkspaceCreator** binding for the `system` workspace. Without this step, OIDC login can succeed but admin operations may still return `403 Forbidden` because the RBAC bindings are missing.

When setup completes, try chatting with the default model you selected:

```bash
nemo chat
```

Use `--fast` for the fast model, or `--model <model-name>` for any other model from `nemo models list`.
...
Override the default and fast models with `NEMO_DEFAULT_MODEL` and
`NEMO_FAST_MODEL`. If the fast override is omitted, it reuses the default.
...
|------|-----------------|
| Chat with the default model | `nemo chat` |
| Chat with another model | `nemo chat --model <model-name>` |
...
The `nemo-helix` package includes a Python SDK. After running `nemo setup`, the simplest initialization uses your CLI context:
...
Check `~/.local/state/nhx/instances/<scope>/services.log` in the directory where you ran `nemo setup`. The most common cause is port 8080 already in use.

### Studio returns 404 or unavailable

Studio requires the FastAPI web assets built by `make bootstrap-studio`. If `make bootstrap` warned that Studio asset bootstrap did not complete, install Flox or provide the versions printed by `make toolchain-versions` with `TOOLCHAIN=system`, then rerun `make bootstrap-studio` and `nemo services restart` from the repository root.
...
cat ~/.local/state/nhx/instances/<scope>/services.log   # view service output
nemo services stop            # stop the running instance for this scope
```

### Stopped instance directories cluttering `ls --all`

After stopping or crashing local services, stopped scope directories can remain under `~/.local/state/nhx/instances/<scope>/`. Empty lock-only ghosts are cleaned automatically; directories with service logs stay until you remove them:
