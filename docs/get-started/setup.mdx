...
Use the [PyPI package](https://pypi.org/project/nemo-helix/) for the quickest local install. The `all` extra includes the SDK, CLI, Helix services, and default first-party plugins; drop it for the SDK and CLI alone.
...
| `--start-services` / `--no-start-services` | prompt (interactive) or skip (auto) | Start local Helix services |
