...
Stage 1 mining (`enable_mining: true`) compiles to **`nhx-automodel-training`** as `python -m nhx.automodel.tasks.retrieval_mine` (torchrun of the Nemotron miner plus unroll/JSONL). All retrieval steps use one container-backed profile (`data_designer.job_executor_profile`, or `--profile`) so generation, conversion, model staging, and mining share job storage. Convert stays on `nhx-cpu-tasks`; mining uses the GPU image.

`model` is a platform model entity with an attached fileset. Before the GPU step,
`nhx-customizer-tasks` downloads that fileset into the shared job storage. The miner and
