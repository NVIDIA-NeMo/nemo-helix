...
The Python SDK is checked in to this repository (historically generated by the Stainless SaaS from the OpenAPI spec and a Stainless config). The Stainless cloud generation step has been removed; the SDK is now maintained in-repo (regenerate the OpenAPI spec locally, then hand-maintain the SDK under `sdk/python/nemo-platform`).
