...
- The agent runs from the platform's own ``nhx-api`` image, which already ships
...
- The image ref is composed from ``NHX_E2E_IMAGE_REGISTRY`` /
  ...
  ``needs_nhx_api_image`` marker skips the test unless both are set; the Kind
...
def agent_deployment_image() -> str:
    ...
    ``NHX_E2E_IMAGE_TAG``) as ``{registry}/nhx-api:{tag}``. In the Kind e2e job
    this exact ref is pre-pulled into the cluster nodes, so the pod resolves it
    node-locally under ``IfNotPresent``. The ``needs_nhx_api_image`` marker
