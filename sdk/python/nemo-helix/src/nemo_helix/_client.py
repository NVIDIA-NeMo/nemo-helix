...
from nemo_helix._base_client import DefaultAsyncHttpxClient, DefaultHttpxClient
...
from nemo_helix_plugin.secrets.compat import AsyncSecretsResource, SecretsResource
...
class NeMoHelix(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        ``~/.config/nhx/config.yaml`` and wires up transparent OIDC token refresh.
        ...
        .. code-block:: python
            ...
            from nemo_helix import NeMoHelix
            client = NeMoHelix(
                base_url=os.environ["NHX_BASE_URL"],
                access_token=os.environ["NHX_ACCESS_TOKEN"],
                workspace="default",
            )
        ...
        Args:
            ...
            base_url: Base URL of the NeMo Helix API. If omitted, read from
                the active context in the nhx config file. Passing this parameter
                without ``context_name`` or ``access_token`` activates direct mode
                (no config file is read, no auth headers are injected).
            config_path: Path to the nhx config file. Defaults to
                ``~/.config/nhx/config.yaml``. Override in containers or CI where
        ...
        if should_bootstrap:
            try:
                ...
                raise RuntimeError(f"NeMoHelix client initialization failed: {e}") from e

        if base_url is None:
            base_url = bootstrap_base_url

        if base_url is None:
            raise RuntimeError("NeMoHelix client initialization failed: base_url is required")
    ...
    def with_raw_response(self) -> NeMoHelixWithRawResponse:
        return NeMoHelixWithRawResponse(self)

    @cached_property
    def with_streaming_response(self) -> NeMoHelixWithStreamedResponse:
        return NeMoHelixWithStreamedResponse(self)
    ...
    def _get_workspace_path_param(self) -> str:
        ...
        raise ValueError(
            "Missing workspace argument; Please provide it at the client level, e.g. NeMoHelix(workspace='abcd') or per method."
        )
    ...
    def __getattr__(self, name: str) -> Any:
        from nemo_helix_plugin.discovery import discover_sdk
...
class AsyncNeMoHelix(AsyncAPIClient):
    ...
    @cached_property
    def with_streaming_response(self) -> AsyncNeMoHelixWithStreamedResponse:
        return AsyncNeMoHelixWithStreamedResponse(self)
    ...
    def _get_workspace_path_param(self) -> str:
        ...
        raise ValueError(
            "Missing workspace argument; Please provide it at the client level, e.g. AsyncNeMoHelix(workspace='abcd') or per method."
        )
...
class NeMoHelixWithRawResponse:
    _client: NeMoHelix
