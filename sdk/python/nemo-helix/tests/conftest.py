...
from nemo_helix import NeMoHelix, AsyncNeMoHelix, DefaultAioHttpClient
from nemo_helix._utils import is_dict
...
logging.getLogger("nemo_helix").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[NeMoHelix]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with NeMoHelix(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncNeMoHelix]:
