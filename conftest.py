...
def pytest_runtest_setup(item):
    ...
    if "needs_nhx_api_image" in [marker.name for marker in item.iter_markers()]:
        if not (os.environ.get("NHX_E2E_IMAGE_REGISTRY") and os.environ.get("NHX_E2E_IMAGE_TAG")):
            skip_test("Skipping nhx-api-image test (set NHX_E2E_IMAGE_REGISTRY and NHX_E2E_IMAGE_TAG)")
