...
The CLI is implemented with Typer in `packages/nemo_helix_ext/src/nemo_helix_ext/cli/`. Every
command talks to the platform through the typed clients in `packages/nemo_helix_plugin`
(`nemo_helix_plugin.<area>.client`). The CLI has **no dependency on the generated `nemo_helix`
(Stainless) SDK**; `packages/nemo_helix_ext/tests/cli/test_stainless_boundary.py` enforces this
statically and by running the CLI with `nemo_helix` un-importable. Never add a `from nemo_helix`
import under `cli/`.
...
from nemo_helix_plugin.secrets.types import ListSecretsQueryParams, HelixSecretCreateRequest
...
from nemo_helix_ext.cli.core.types import EntityOutputFormatOption, ListOutputFormatOption
...
- Request bodies are the typed request models from `nemo_helix_plugin.<area>.types`, built only from
...
- Errors: let `nemo_helix_plugin.client.errors` propagate; `@handle_errors` maps them (404 → exit 3,
...
mirroring the server's FastAPI route exactly, with tests under `packages/nemo_helix_plugin/tests/<area>/`.
...
uv run --frozen pytest packages/nemo_helix_plugin/tests/<area> -q
```
