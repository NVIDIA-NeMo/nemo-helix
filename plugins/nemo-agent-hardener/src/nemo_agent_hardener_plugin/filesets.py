...
scanned by ``agent-hardener inspect`` and later run inside the OpenShell sandbox).
"""
...
def download_fileset(sdk: NeMoHelix, ref: str, dest: Path) -> Path:
...
def upload_file_to_fileset(sdk: NeMoHelix, local_path: Path, *, workspace: str, prefix: str = "hitlog") -> str:
...
def delete_fileset(sdk: NeMoHelix, ref: str) -> None:
...
def upload_project_dir(sdk: NeMoHelix, project_dir: Path, *, workspace: str) -> str:
...
def download_and_extract_project(sdk: NeMoHelix, ref: str, workdir: Path) -> Path:
