...
agent-hardener.yaml  only for running standalone; `init --project-dir` derives this
```
...
nemo agent-hardener synth-benign --manifest-id langgraph-victim --yes
nemo agent-hardener run --manifest-id langgraph-victim
```
