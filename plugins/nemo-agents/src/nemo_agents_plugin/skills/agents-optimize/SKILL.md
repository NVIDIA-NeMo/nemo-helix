...
- `nemo agents optimize --strategy nat --agent <name> --config-fileset <workspace/name> --config <config-path-in-fileset> --output-agent <new-name>`
...
# The optimize command's --config value must be relative to the staged fileset root.
