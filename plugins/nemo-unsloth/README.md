...
2. `UnslothJob.compile` produces a 4-step `HelixJobSpec` (see `nhx.unsloth.images` for the exact container commands):
...
- `nhx.unsloth.compile.platform_job_config_compiler` — invoked from `UnslothJob.compile()` to build the 4-step `HelixJobSpec`.
