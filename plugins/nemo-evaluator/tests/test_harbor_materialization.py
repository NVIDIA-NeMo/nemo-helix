...
def snapshot(root, folder="wrapped", entity="stored"):
    """Package a task tree and return its stored definition alongside the archive bytes used by fake downloads."""
    buffer = io.BytesIO()
...
@pytest.fixture
def tree(tmp_path):
    """Create a native task with hidden files, an empty directory, and an executable verifier."""
...
def run(members, objects, destination, transport):
    """Materialize stored tasks through a fake sync or async Files client backed by the supplied archive bytes."""
    client = Mock()

    def download(*, workspace, name, path):
        """Serve the requested in-memory object through the streaming interface for the selected client type."""
...
def test_complete_tree_round_trip(tree, tmp_path, transport):
    """Check archive contents and modes survive extraction and repeated materialization gets a fresh directory."""
...
def test_failure_is_atomic(tree, tmp_path, transport, failure):
    """Inject archive or member failures and verify cleanup preserves only the caller's existing files."""
    member, objects = snapshot(tree)
    members = [member]
...
def test_native_invalid_tree_never_published(tree, tmp_path, transport, failure):
    """Corrupt required native task artifacts and verify no extracted dataset remains after validation fails."""
...
def test_native_config_controls_required_artifacts(tree, tmp_path, transport, layout):
    """Verify artifact requirements follow multistep and separate-verifier configuration rather than a fixed layout."""
...
async def test_async_concurrency_is_bounded_and_drained(tree, tmp_path, fail):
    """Track download lifetimes to verify serial streaming and cleanup after either success or interruption."""
    for i in range(20):
    ...
    async def download(*, workspace, name, path):
        """Return a tracked archive stream that can fail while the materializer consumes it."""

        @asynccontextmanager
        async def stream(**kwargs):
            """Count active streams and release the count even when archive consumption raises."""
...
def test_many_small_files_suite(tree, tmp_path, transport):
    """Materialize many small files across distinct tasks, checking task order and reporting preparation time."""
    import shutil
    import time
