...
def service(existing_tasks: set[tuple[str, str]], entity_store) -> TasksetService:
    return TasksetService(entity_store, _FakeTaskService(existing_tasks), authorize_task_read=AsyncMock())
...
async def test_a_tag_pinned_member_stores_the_tagged_revision(
    ...
    service = TasksetService(entity_store, task_service, authorize_task_read=AsyncMock())
...
async def test_create_resolves_bare_ref_against_taskset_workspace(
    ...
    service = TasksetService(entity_store, _FakeTaskService(existing_tasks), authorize_task_read=AsyncMock())
...
async def test_create_allows_same_name_in_different_workspaces(service: TasksetService) -> None:
    ...
    first, _ = await service.create_taskset("ts-1", TasksetInput(tasks=[]), workspace="default")
    second, _ = await service.create_taskset("ts-1", TasksetInput(tasks=[]), workspace="other")
...
async def test_member_naming_an_unknown_revision_is_rejected(
    ...
    service = TasksetService(entity_store, _NoSuchRevision(existing_tasks), authorize_task_read=AsyncMock())
...
async def test_members_resolve_concurrently(existing_tasks: set[tuple[str, str]], entity_store) -> None:
    ...
    service = TasksetService(entity_store, _Tracking(members), authorize_task_read=AsyncMock())
...
async def test_membership_is_stored_in_canonical_order(existing_tasks: set[tuple[str, str]], entity_store) -> None:
    ...
    """
    members = {("default", f"task-{i}") for i in range(5)}
    service = TasksetService(entity_store, _FakeTaskService(members), authorize_task_read=AsyncMock())
...
async def test_a_failed_member_cancels_its_siblings(existing_tasks: set[tuple[str, str]], entity_store) -> None:
    ...
    members = {("default", f"task-{i}") for i in range(5)}
    service = TasksetService(entity_store, _SlowExceptOne(members), authorize_task_read=AsyncMock())
...
async def test_id_membership_uses_task_record_and_authorizes(entity_store):
    from unittest.mock import AsyncMock

    from nemo_evaluator.api.schemas import EvaluatorTaskDefinition, TaskInput
    from nemo_evaluator.api.service.task_service import TaskService

    task_service = TaskService(entity_store, AsyncMock(), AsyncMock())
    task, _ = await task_service.create_task(
        "by-id", TaskInput(spec=EvaluatorTaskDefinition(kind="evaluator", intent="test")), workspace="default"
    )
    authorize = AsyncMock()
    service = TasksetService(entity_store, task_service, authorize_task_read=authorize)
    suite, _ = await service.create_taskset("ids", TasksetInput(task_ids=[task.id]), workspace="default")
    assert suite.tasks[0].root.startswith("default/by-id#")
    authorize.assert_awaited_once_with("default", "by-id")


@pytest.mark.parametrize("by_id", [False, True])
async def test_member_denial_prevents_taskset_write(entity_store, by_id):
    from fastapi import HTTPException
    from nemo_evaluator.api.schemas import EvaluatorTaskDefinition, TaskInput
    from nemo_evaluator.api.service.task_service import TaskService

    tasks = TaskService(entity_store, AsyncMock(), AsyncMock())
    task, _ = await tasks.create_task(
        "member", TaskInput(spec=EvaluatorTaskDefinition(kind="evaluator", intent="test")), workspace="private"
    )
    authorize = AsyncMock(side_effect=HTTPException(403, "Denied"))
    service = TasksetService(entity_store, tasks, authorize_task_read=authorize)
    body = TasksetInput(task_ids=[task.id]) if by_id else TasksetInput(tasks=[TaskRef("private/member")])
    before = set(entity_store.entities)
    with pytest.raises(HTTPException) as raised:
        await service.create_taskset("denied", body, workspace="default")
    assert raised.value.status_code == 403
    assert set(entity_store.entities) == before


async def test_deleted_or_wrong_kind_id_is_not_a_member(entity_store):
    from nemo_evaluator.api.service.task_service import TaskService
    from nemo_evaluator.entities import TasksetEntity

    other = await entity_store.create(TasksetEntity(name="other", workspace="default"))
    tasks = TaskService(entity_store, AsyncMock(), AsyncMock())
    service = TasksetService(entity_store, tasks, authorize_task_read=AsyncMock())
    for identity in (other.id, "missing"):
        with pytest.raises(TaskRefNotFoundError):
            await service.create_taskset("invalid", TasksetInput(task_ids=[identity]), workspace="default")


async def test_id_membership_distinguishes_same_name_across_workspaces(entity_store):
    from nemo_evaluator.api.schemas import EvaluatorTaskDefinition, TaskInput
    from nemo_evaluator.api.service.task_service import TaskService
    from nemo_evaluator.entities import TaskRevisionEntity

    tasks = TaskService(entity_store, AsyncMock(), AsyncMock())
    heads = []
    for workspace in ("first", "second"):
        head, _ = await tasks.create_task(
            "member", TaskInput(spec=EvaluatorTaskDefinition(kind="evaluator", intent=workspace)), workspace=workspace
        )
        heads.append(head)
    assert heads[0].id != heads[1].id
    revisions = [entity for entity in entity_store.entities.values() if isinstance(entity, TaskRevisionEntity)]
    assert len({entity.id for entity in revisions}) == 2
    for revision in revisions:
        fetched = await entity_store.get_by_id(TaskRevisionEntity, revision.id)
        assert fetched.parent == revision.parent
    service = TasksetService(entity_store, tasks, authorize_task_read=AsyncMock())
    suite, _ = await service.create_taskset("suite", TasksetInput(task_ids=[heads[1].id]), workspace="default")
    assert suite.tasks[0].root.startswith("second/member#")
