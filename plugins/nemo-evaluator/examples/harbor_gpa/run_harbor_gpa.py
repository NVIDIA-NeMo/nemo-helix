...
"""Run deepagents inside Harbor and score each trajectory on the nine Agent GPA dimensions.

Harbor's verifier says whether the task was done. The GPA metrics in ``gpa_metrics.py`` say how
well the agent got there, each with a written reason, and ``view.gpa`` averages them.
...
from gpa_metrics import Judge, attach_gpa, gpa_metrics
...
parser.add_argument("--judge-model", default="nvidia/nemotron-3.5-lightning-30b-a3b", help="Model that judges.")
...
# 1. The tasks: three Harbor task folders, each with a gpa.json goal. discover_harbor_tasks gives
#    each one the verifier's HarborRewardMetric; attach_gpa adds the nine GPA metrics and the view.
tasks = attach_gpa(
    discover_harbor_tasks(Path(__file__).parent / "gpa_dataset"),
    gpa_metrics(Judge(args.judge_model)),
)

# 2. The agent: Harbor runs NeMo Fabric's deepagents harness inside each task container.
#    fabric_telemetry="relay" records the trajectory the metrics read; the relay extra enables it.
runner = HarborAgentTaskRunner(
    config=HarborRuntimeConfig(
        ...
        agent_env_from_host=["NVIDIA_API_KEY"],  # Harbor persists only the template "${NVIDIA_API_KEY}"
        n_concurrent_trials=3,
        agent_setup_timeout_multiplier=8.0,  # the first trial installs Fabric into the container
...
# 4. Report. A metric that could not measure a trial (no trajectory) counts as unmeasured, not zero.
print("\nAggregates:")
