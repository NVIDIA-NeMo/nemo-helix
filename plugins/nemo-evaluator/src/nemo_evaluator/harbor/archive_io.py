...
class _CheckedWriter:
    ...
    def write(self, chunk: bytes) -> None:
        """Write and hash a downloaded chunk only if the cumulative size stays within the limit."""
        self.size += len(chunk)
...
def download_verified(
    ...
    *,
    ...
    """Download a Fileset object while verifying its size and SHA-256 digest.

    Args:
        client: Authenticated synchronous Files client.
        ref: Fully qualified Fileset object reference.
        output: Writable binary stream that receives verified bytes.
        limit: Maximum number of bytes accepted from the response.
        expected_size: Optional exact byte count.
        expected_digest: Required lowercase SHA-256 digest.

    Raises:
        ValueError: The object exceeds ``limit`` or fails size or digest verification.
    """
...
async def download_verified_async(
    ...
    *,
    ...
    """Asynchronously download a Fileset object while verifying size and digest.

    Args:
        client: Authenticated asynchronous Files client.
        ref: Fully qualified Fileset object reference.
        output: Writable binary stream that receives verified bytes.
        limit: Maximum number of bytes accepted from the response.
        expected_size: Optional exact byte count.
    ...
    async with response.stream(chunk_size=CHUNK_BYTES) as chunks:
        async for chunk in chunks:
            await run_blocking_archive_operation(writer.write, chunk)
    writer.verify(expected_size, expected_digest)
