...
_NATIVE_UNAVAILABLE = (
    ...
    "(switchyard_rust), but they are not installed in this process."
)
...
def _optional_bool(config: dict[str, Any], field: str) -> bool:
    value = config.get(field, False)
    if not isinstance(value, bool):
        raise InferenceMiddlewareError(f"{field} must be a boolean", status_code=400)
    return value


def map_random_routing_config(config: dict[str, Any]) -> tuple[list[float], int | None, dict[str, list[str]]]:
    """Map the existing random_routing JSON onto libsy ``random`` + ``models.any``.
...
def validate_stage_router_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    notes = _require_mapping(config.get("handoff_notes") or {}, "handoff_notes")
...
def validate_llm_classifier_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    session_affinity = _optional_bool(config, "session_affinity")
    message_hash_fallback = _optional_bool(config, "message_hash_fallback")
...
def models_for_native_config(config_type: str, config: dict[str, Any]) -> dict[str, list[str]]:
    if config_type == "random_routing":
        _weights, _seed, models = map_random_routing_config(config)
        return models
