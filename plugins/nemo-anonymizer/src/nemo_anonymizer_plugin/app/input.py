...
async def prepare_anonymizer_input_async(
    data: AnonymizerInputSpec,
    *,
    sdk: AsyncPlatformClient | SyncPlatformClient | None,
    workspace: str,
    allow_local_paths: bool,
    ...
    if source_kind == "fileset":
        if sdk is None:
            raise AnonymizerInvalidConfigError("Fileset input requires a NeMo Platform client.")
        if isinstance(sdk, SyncPlatformClient):
