...
@pytest.fixture
def test_app(mock_client: MagicMock) -> FastAPI:
    app = FastAPI()
    app.include_router(
        artifacts_module.router,
        prefix="/apis/auditor/v2/workspaces/{workspace}",
    )
    app.dependency_overrides[get_nemo_client] = lambda: mock_client
    return app
...
class TestDownloadAuditArtifacts:
    def test_all_results_present_returns_200_with_valid_targz(
        self, client: TestClient, mock_client: MagicMock, tmp_path: Path
    ) -> None:
        ...
        with (
            patch.object(artifacts_module.AsyncJobsClient, "from_client", return_value=mock_jobs),
            patch.object(artifacts_module.AsyncFilesClient, "from_client", return_value=MagicMock()),
    ...
    def test_no_results_at_all_returns_404(self, client: TestClient, mock_client: MagicMock) -> None:
        mock_jobs = AsyncMock()
    ...
    def test_result_names_queried_in_order(self, client: TestClient, mock_client: MagicMock, tmp_path: Path) -> None:
    ...
    def test_workspace_and_job_forwarded_to_jobs_client(
        ...
        with (
            patch.object(artifacts_module.AsyncJobsClient, "from_client", return_value=mock_jobs),
            patch.object(artifacts_module.AsyncFilesClient, "from_client", return_value=mock_files),
