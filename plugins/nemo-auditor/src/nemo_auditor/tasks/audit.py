...
Builds the task clients, then dispatches to :class:`~nemo_auditor.jobs.audit.AuditJob`.
...
def main() -> int:
    ...
    try:
        client = get_task_nemo_client("auditor")
        async_client = get_async_task_nemo_client("auditor")
