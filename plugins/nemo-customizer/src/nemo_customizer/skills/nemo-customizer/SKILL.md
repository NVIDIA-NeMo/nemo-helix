...
- **`--upload-environment` uploads a Gym package, it does not build one** — validate it first with `uv run --package nmp-rl pi-to-gym-conversion --validate-only <dir>` (`references/gym-environments.md`). The flag is rejected on `automodel` and `unsloth`, which have no environment.
