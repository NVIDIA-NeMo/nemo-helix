...
class BuildPlan:
    ...
    #: The two settings with no safe default, proven present. See `_require_buildable`.
    push_secret: str
    signing_key: str
    ...
    def resolve(cls, build_set: BuildSet, *, config: BuilderConfig, workspace: str) -> BuildPlan:
        ...
        push_secret, signing_key = _require_buildable(config)
        job_name = f"{build_set.name}-{build_set.revision}"
        images = tuple(
            _plan_image(index, spec, config=config, workspace=workspace, build_set=build_set, job_name=job_name)
...
def _require_buildable(config: BuilderConfig) -> tuple[str, str]:
    """Reject a deployment that cannot build at all, before looking at what was asked for.

    A deployment missing its credential or its signing key fails the submit with an error the
    caller can act on, rather than producing a job that dies in a pod later.
    """
    ...
    return config.push_secret, config.signing_key


def _plan_image(
    index: int, spec: BuildSpec, *, config: BuilderConfig, workspace: str, build_set: BuildSet, job_name: str
) -> PlannedImage:
    ...
    string produces the moment anything resolves it. The deployment's ``repository_prefix``
    applies only when the spec takes the deployment's registry: a spec naming its own registry
...
def _require_distinct_destinations(images: tuple[PlannedImage, ...]) -> None:
    ...
    Compared after defaults are resolved, because a spec that names the default registry and one
    that omits it are the same destination. A plain ``ValueError`` -- a malformed request (400),
    not a deployment that cannot satisfy a well-formed one (409).
    """
    seen: dict[str, str] = {}
    for image in images:
