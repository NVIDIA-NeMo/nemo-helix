...
def _clients(
    ...
    *,
...
) -> tuple[MagicMock, MagicMock, AsyncCustomizationHelixClients]:
    models = MagicMock()
    ...
    return models, files, AsyncCustomizationHelixClients(files=files, models=models, jobs=MagicMock())
...
def _adapter_clients(existing_base: str | None, *, missing: bool = False) -> AsyncCustomizationHelixClients:
    models = MagicMock()
    if missing:
    ...
    return AsyncCustomizationHelixClients(files=MagicMock(), models=models, jobs=MagicMock())
...
def _in_flight_clients(jobs: _Jobs) -> AsyncCustomizationHelixClients:
    return AsyncCustomizationHelixClients(files=MagicMock(), models=MagicMock(), jobs=cast(Any, jobs))


def _job(name: str, output_name: str, status: HelixJobStatus = HelixJobStatus.ACTIVE) -> SimpleNamespace:
...
async def test_output_name_check_rejects_a_running_job_with_the_same_output() -> None:
    ...
    jobs = _Jobs([_job("automodel-job-1", "my-lora", HelixJobStatus.PENDING)])
...
async def test_output_name_check_fails_closed_when_jobs_cannot_be_listed() -> None:
    ...
    with pytest.raises(HelixJobCompilationError, match="Could not check for running jobs"):
        await validate_output_name_not_in_flight("my-lora", "default", _in_flight_clients(jobs))
