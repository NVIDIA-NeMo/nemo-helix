...
def test_progress_updates_use_a_short_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "nhx.customization_common.training.progress.get_task_nemo_client",
        lambda service_name: object(),
    )
    monkeypatch.setattr(
        "nhx.customization_common.training.progress.JobsClient.from_client",
...
def test_the_task_client_is_not_rebuilt_with_a_custom_client(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("nhx.customization_common.training.progress.get_task_nemo_client", _factory)
    monkeypatch.setattr(
        "nhx.customization_common.training.progress.JobsClient.from_client",
        lambda client: _JobsClient(),
    )
