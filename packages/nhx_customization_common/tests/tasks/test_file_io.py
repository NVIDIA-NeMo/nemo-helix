...
def _make_runner(
    ...
    *,
    ...
    from nhx.customization_common.tasks.file_io.run import FileIORunner
    from nhx.customization_common.tasks.file_io_progress_reporter import NoOpProgressReporter
...
def _raise_runner_conflict() -> None:
    ...
    run_mod = sys.modules["nhx.customization_common.tasks.file_io.run"]
...
class TestCreateFileset:
    ...
    def test_conflict_patches_metadata_on_existing(self) -> None:
        from nemo_helix_plugin.files.types import UpdateFilesetRequest
        from nhx.customization_common.schemas.file_io import FileSetRef
...
class TestUploadFileset:
    def test_upload_item_rejects_directory_path(self) -> None:
        from nhx.customization_common.schemas.file_io import FileSetRef, UploadItem
    ...
    @patch("nhx.customization_common.tasks.file_io.run.FilesetFileSystem")
    def test_uses_files_client_for_transfer(self, mock_fs_cls, tmp_path: Path) -> None:
        from nhx.customization_common.schemas.file_io import FileSetRef
        from nhx.customization_common.tasks.file_io.run import UPLOAD_TIMEOUT
    ...
    def test_upload_failure_propagates_as_file_upload_error(self, mock_fs_cls, tmp_path: Path) -> None:
        from nhx.customization_common.schemas.file_io import FileSetRef, FileUploadError
...
class TestDownloadFileset:
    ...
    def test_uses_files_client_for_transfer(self, mock_fs_cls, tmp_path: Path) -> None:
        from nhx.customization_common.schemas.file_io import FileSetRef
        from nhx.customization_common.tasks.file_io.run import DOWNLOAD_TIMEOUT, LIST_FILES_TIMEOUT
...
class TestRun:
    def test_builds_sync_files_task_client_for_filesystem_transfers(self) -> None:
        ...
        file_io_run = importlib.import_module("nhx.customization_common.tasks.file_io.run")

        sync_client = MagicMock()
        sync_client.base_url = "http://nemo-helix.local"
        files = MagicMock()
        jobs = MagicMock()
