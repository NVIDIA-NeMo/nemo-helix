...
from nhx.customization_common.cli import renderer as renderer_mod
from nhx.customization_common.cli.renderer import (
...
def test_build_url_basic() -> None:
    url = build_studio_customization_url("https://nhx.test", "default", "job-1")
    assert url == "https://nhx.test/studio/workspaces/default/customizations/job-1"
...
def test_build_url_encodes_custom_workspace_and_job() -> None:
    ...
    url = build_studio_customization_url("https://nhx.test", "acme corp/eu", "sft job/1")
    assert url == "https://nhx.test/studio/workspaces/acme%20corp%2Feu/customizations/sft%20job%2F1"
...
_PROBE_REQUEST = httpx.Request("GET", "https://nhx.test/status")
...
def test_studio_available_when_in_ready(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert studio_is_available("https://nhx.test") is True
    assert captured["url"] == "https://nhx.test/status"
...
def test_studio_not_available_when_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert studio_is_available("https://nhx.test") is False
...
def test_renderer_prints_link_when_studio_ready(
    ...
    out, err = _run_complete(_ctx("https://nhx.test", workspace="acme"), {"name": "job-1"}, capsys)
    # Link + guidance go to STDERR, not stdout.
    assert "View in Studio: https://nhx.test/studio/workspaces/acme/customizations/job-1" in err
...
def test_renderer_omits_link_when_studio_not_ready(
    ...
    out, err = _run_complete(_ctx("https://nhx.test"), {"name": "job-1"}, capsys)
...
def test_renderer_omits_link_when_no_job_name(
    ...
    out, err = _run_complete(_ctx("https://nhx.test"), {"id": "internal-uuid"}, capsys)
    assert "View in Studio" not in err
...
def test_renderer_shell_quotes_workspace_with_spaces(
    ...
    monkeypatch.setattr(renderer_mod, "studio_is_available", lambda base_url: True)
    _out, err = _run_complete(_ctx("https://nhx.test", workspace="acme corp"), {"name": "job-1"}, capsys)
