...
description: >-
  End-to-end NeMo Helix recipe for domain embedding and reranking fine-tuning:
...
preconditions:
  ...
  - platform_running
  - workspace_exists
compatibility: NeMo Helix with data-designer [retrieval-sdg], customizer automodel, and evaluator retrieve-eval plugins.
maturity: active
license: Apache-2.0
user-invocable: true
...
Use `nemo`, or `uv run nemo` from the nemo-helix root. If neither works, route
to `setup`. The CLI defaults to `http://localhost:8080`; `NHX_BASE_URL` overrides
