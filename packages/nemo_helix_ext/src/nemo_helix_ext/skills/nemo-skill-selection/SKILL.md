...
description: Top-level skill selector for ambiguous tasks involving NeMo Helix (NVIDIA's agent platform). Picks the right downstream skill for setup, design, specification, agent configuration, build, deployment, testing, observability, status, teardown, evaluation, optimization, security, or model customization. Use when the user needs help deciding where a NeMo Helix task should start.
triggers:
  - build an agent
  - create an agent
  - deploy an agent
...
not-for:
  ...
  - superpowers:brainstorming (use for design work unrelated to NeMo Helix)
...
compatibility: nemo-helix >= 0.1.0; selection plus a host scan on macOS or Linux; works without an installed CLI (selector can pick setup, which then tells the user how to run the CLI install).
maturity: active
license: Apache-2.0
user-invocable: true
allowed-tools: Bash
...
# NeMo Helix skill selection

Decide which downstream NeMo Helix skill should run. Bash access is unrestricted at runtime and
...
New NeMo Helix agent builds use a Platform-managed `agent.yaml` with
...
| "build the agent", "create the agent", "deploy", "implement these requirements" | `nemo-build-agent` | Build from confirmed requirements, default to Platform `agent.yaml`, register, deploy, evaluate, and optionally apply guardrails |
...
ls ~/.config/nhx*/config.yaml 2>/dev/null
```
...
```
NeMo Helix skills I can route to:
  setup           verify install or get the CLI install command
  nemo-agent-config  author, validate, or migrate Platform agent.yaml
  nemo-build-agent  build from confirmed requirements, register, deploy, evaluate, and sign off
...
- Studio is the NeMo Helix web UI. When the platform is running locally, it serves at `http://localhost:8080/studio`.
