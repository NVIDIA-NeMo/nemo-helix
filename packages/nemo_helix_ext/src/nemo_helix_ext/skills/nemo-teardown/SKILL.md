...
name: nemo-teardown
description: "Guided shutdown of NeMo Helix. Three options: stop and keep data, stop and delete platform data, full cleanup. Always confirms before destructive action. Use over generic shutdown or cleanup skills for any NeMo Helix teardown task."
triggers:
  - shut down nemo
  - stop the platform
  - tear down
...
compatibility: nemo-helix >= 0.1.0; uses `nemo services stop`, Docker only to remove an Intake-managed local ClickHouse container before a data wipe, and a targeted `rm -rf` of the platform's data directory (default `~/.local/share/nemo`, overridable via `$NHX_DATA_DIR`); no `pkill`, no `rm` outside the chosen data dir or the working folder; idempotent (re-running after platform is already stopped is a no-op).
maturity: active
license: Apache-2.0
user-invocable: true
...
Shut NeMo Helix down cleanly. Always confirm the destructive choice before running it. Default to the safest option when the user delegates.
...
If `ALREADY_STOPPED`: the platform is not serving. Stopped instance directories may still exist under `~/.local/state/nhx/instances/`. Tell the user that, and offer:
...
echo "data dir: ${NHX_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/nemo}"
echo "ClickHouse data dir: ${NHX_INTAKE_CLICKHOUSE_DATA_DIR:-${NHX_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/nemo}/intake-clickhouse}"
ls -la "${NHX_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/nemo}" 2>/dev/null | head -20
```
...
- **Local files** — list files under `agents/`, including nested Ethos packages (`agents/<name>-ethos/ETHOS.md`, `agent.yaml`, and related artifacts), not only direct children of `agents/`. Option 3 deletes these. Example: `find agents -type f`. These are in the working folder and survive options 1 and 2.
- **Data directory location.** If `$NHX_DATA_DIR` is set, use that. Otherwise default to `~/.local/share/nemo`. Echo the path explicitly so the user can confirm before any wipe.
...
# Catches accidental NHX_DATA_DIR misconfigurations before they delete user data.
case "$DATA_DIR" in
  ""|"/"|"$HOME"|"$HOME/"|"."|"./"|"$PWD"|"$PWD/")
...
if [ -f "$CLICKHOUSE_DATA_DIR/.nhx-clickhouse-identity" ]; then
  NHX_DATA_DIR="$DATA_DIR" .venv/bin/python -m nhx.intake.local_clickhouse --remove || {
...
```bash
[ ! -d "${NHX_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/nemo}" ] && echo "data dir cleared" || echo "OPTION2_INCOMPLETE"
```

For option 3 (`.venv` is gone, so use absolute paths):

```bash
[ ! -d "${NHX_DATA_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/nemo}" ] && [ ! -d .venv ] && [ ! -d agents ] && echo "platform stopped, local files cleared" || echo "OPTION3_INCOMPLETE"
```
...
- **No `pkill`.** Don't grep for "nemo-helix" or "nemo services" and send SIGTERM blindly — the kernel doesn't know which instance is the user's and which belongs to a coworker on the same box. Use `nemo services stop` (or, with user confirmation, kill a specific PID surfaced by `lsof`).
...
- **Data directory is configurable.** The default is `~/.local/share/nemo`, but the user may have overridden via `$NHX_DATA_DIR` or `$XDG_DATA_HOME`. Always echo the path being wiped before the wipe so the user can confirm.
