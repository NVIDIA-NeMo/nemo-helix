...
preconditions:
  ...
  - workspace_exists
  - clickhouse_ready
compatibility: nemo-helix >= 0.1.0; needs a reachable local or remote intake service (with auth and entities) backed by ClickHouse for rollups/results; talks HTTP to /apis/intake/v2 (curl only, no Docker); Experiments viewing in Studio is behind the VITE_FF_EXPERIMENT feature flag.
maturity: beta
license: Apache-2.0
user-invocable: true
...
Everything below uses `${NHX_BASE_URL}` (default `http://localhost:8080`) and a `${WORKSPACE}`
(default `default`). Point `NHX_BASE_URL` at the local platform or a remote HTTPS origin. Reject
...
nhx_authority=${nhx_authority%%/*}
case "${nhx_authority}" in
  *@*) echo "NHX_BASE_URL must not contain userinfo" >&2; exit 1 ;;
esac
case "${NHX_BASE_URL}" in
  https://*) ;;
  http://*)
    case "${nhx_authority}" in
      localhost|127.0.0.1) ;;
      localhost:*|127.0.0.1:*)
        nhx_port=${nhx_authority#*:}
        case "${nhx_port}" in
          ""|*[!0-9]*) echo "loopback NHX_BASE_URL has an invalid port" >&2; exit 1 ;;
        esac
        ;;
      *) echo "HTTP NHX_BASE_URL must use exactly localhost or 127.0.0.1" >&2; exit 1 ;;
    esac
    ;;
  *) echo "remote NHX_BASE_URL must use https://" >&2; exit 1 ;;
esac
if curl -sf "${NHX_BASE_URL}/health/ready" >/dev/null; then
  echo "platform ready"
else
...
: "${NHX_BASE_URL:=http://localhost:8080}"
: "${WORKSPACE:=default}"
experiments="${NHX_BASE_URL}/apis/intake/v2/workspaces/${WORKSPACE}/experiments"
...
```bash
curl -sf -X POST \
  "${NHX_BASE_URL}/apis/intake/v2/workspaces/${WORKSPACE}/evaluations" \
  -H 'Content-Type: application/json' \
...
```bash
curl -sf "${NHX_BASE_URL}/apis/intake/v2/workspaces/${WORKSPACE}/evaluations/my-eval-baseline" \
  | python3 -m json.tool
```
...
```bash
curl -sf "${NHX_BASE_URL}/apis/intake/v2/workspaces/${WORKSPACE}/evaluations/my-eval-baseline/sessions" \
  | python3 -m json.tool
```
