...
Do not assume the user knows NeMo Helix, Fabric, Ethos, MCP or Agent Skills.
...
3. Explain that the supported build path uses the external LangChain Deep
   Agents runtime. NeMo Helix does not vendor the customer's agent. The
   ...
   for deployment, testing, observation and optimization in NeMo Helix.
