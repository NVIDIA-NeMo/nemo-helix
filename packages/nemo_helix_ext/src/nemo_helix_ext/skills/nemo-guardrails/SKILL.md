...
- **CLI paths**: `/app/.venv/bin/nemo` and `/app/.venv/bin/nhx` (same binary,
  two names)
- **API server**: `http://localhost:8080` (default, no auth needed for local services)
- **Default workspace**: `default`
- **Workspace override**: `--workspace <name>` flag, or `NHX_WORKSPACE` env var
...
[packages/nemo_helix_plugin/src/nemo_helix_plugin/inference_middleware.py](packages/nemo_helix_plugin/src/nemo_helix_plugin/inference_middleware.py).
...
[services/core/inference-gateway/src/nhx/core/inference_gateway/api/middleware_registry.py](services/core/inference-gateway/src/nhx/core/inference_gateway/api/middleware_registry.py)).
