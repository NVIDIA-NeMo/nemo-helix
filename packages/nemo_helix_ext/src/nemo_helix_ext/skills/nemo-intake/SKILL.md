...
local or remote. An agent does not need to run on NeMo Helix; it only needs network access and
...
- A reachable local or remote NeMo Helix `intake` service with its `auth` and `entities`
...
For a remote deployment, set `NHX_BASE_URL` to its HTTPS origin and skip local startup. Use the
...
automatically provisions local ClickHouse unless `NHX_INTAKE_CLICKHOUSE_URL` is set:
...
```bash
curl -i "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/spans?page=1&page_size=1"
```
...
`$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE`.
...
4. For supported OTLP instrumentation, point the exporter at Intake:
   ...
   ```bash
   export OTEL_EXPORTER_OTLP_TRACES_ENDPOINT="$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/ingest/otlp/v1/traces"
...
```bash
curl -X POST "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/evaluator-results" \
...
```bash
curl -g "$NHX_BASE_URL/apis/intake/v2/workspaces/$WORKSPACE/spans?filter[session_id]=<session-id>&page=1&page_size=100"
```
