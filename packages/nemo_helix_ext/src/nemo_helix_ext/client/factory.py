...
"""Factory for creating NeMoHelix SDK clients from nhx config.

Layers the generated ``NeMoHelix`` constructors on top of the shared,
SDK-independent bootstrap in :mod:`nemo_helix_ext.client.bootstrap`, which
resolves the nhx config file, discovers OIDC settings, and builds the token
...
def build_client_init_kwargs(
    *,
    ...
    """Build constructor kwargs for a **sync** NeMoHelix client.
...
def build_async_client_init_kwargs(
    *,
    ...
    """Build constructor kwargs for an **async** AsyncNeMoHelix client.
...
def create_client(
    *,
    ...
    """Create a NeMoHelix client from the nhx config.
