...
from nemo_helix_ext.cli.core.types import ListOutputFormat as OutputFormat
from nemo_helix_ext.cli.core.types import TimestampFormat

if typing.TYPE_CHECKING:
    from nemo_helix import AsyncNeMoHelix, NeMoHelix

    from nemo_helix_ext.config.config import ConfigParams, Context
...
logger = logging.getLogger("nemo_helix_ext.cli")
...
class CLIContext:
    ...
    # Lazy-created client
    _client: NeMoHelix | None = field(default=None, repr=False)

    # Lazy-created async client
    _async_client: AsyncNeMoHelix | None = field(default=None, repr=False)
    ...
    def get_client(self, timeout: float = 60.0) -> NeMoHelix:
        """
        ...
        if self._client is None:
            ...
            logger.debug(
                f"Creating NeMoHelix client with base_url={base_url}, workspace={ctx.workspace}, timeout={timeout}"
            )

            auth_config = self._client_auth_config(ctx)
            self._client = NeMoHelix(
                base_url=base_url,
                timeout=timeout,
    ...
    def get_async_client(self, timeout: float = 60.0) -> AsyncNeMoHelix:
        """
        ...
        Returns:
            Initialized AsyncNeMoHelix client
        """
        ...
        if self._async_client is None:
            ...
            base_url = str(ctx.cluster.base_url)
            logger.debug(
                f"Creating AsyncNeMoHelix client with base_url={base_url}, workspace={ctx.workspace}, timeout={timeout}"
            )

            auth_config = self._client_auth_config(ctx)
            self._async_client = AsyncNeMoHelix(
                base_url=base_url,
                timeout=timeout,
                workspace=ctx.workspace,
                **auth_config,
    ...
    def get_job_telemetry_custom_fields(self) -> dict[str, object]:
        ...
        try:
            ...
            from nemo_helix_ext.cli.telemetry.emit import telemetry_opted_in
            from nemo_helix_ext.cli.telemetry.session import get_session_id
