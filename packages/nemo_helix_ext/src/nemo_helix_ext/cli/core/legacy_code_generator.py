...
"""Code generation for ``--output-format code`` on the generated (Stainless SDK) commands.

Kept only while ``cli/commands/api/`` exists; hand-written commands use
:mod:`nemo_helix_ext.cli.core.code_generator`, which dispatches here when
handed a resource path instead of a typed client class.
"""
...
def generate_python_code(
    ...
    if _lifecycle_uses_status_error_handling(lifecycle_type):
        lines.append(
            "from nemo_helix import APIConnectionError, APIStatusError, APITimeoutError, NeMoHelix, NotFoundError"
        )
    else:
        lines.append("from nemo_helix import NeMoHelix")
        if lifecycle_type == _PLATFORM_JOB_LIFECYCLE:
            lines.append("from nemo_helix_plugin.client.adapter import client_from_platform")
            lines.append("from nemo_helix_plugin.jobs.client import JobsClient")
            if lifecycle_mode == "wait":
                lines.append("from nemo_helix_plugin.jobs.watch_types import JobStatusEvent, JobWatchTimeoutError")
    lines.append("")

    if base_url:
        lines.append(f"client = NeMoHelix(base_url={_format_python_literal(base_url)})")
    else:
        lines.append("client = NeMoHelix()")
    lines.append("")
    if lifecycle_type == _PLATFORM_JOB_LIFECYCLE:
