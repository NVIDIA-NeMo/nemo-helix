...
"""
NeMo Helix help formatter for Typer CLI.
...
class NhxErrorHandlingMixin:
    ...
    def main(
        ...
        **extra: Any,
        ...
        code or output.
        """
        from nemo_helix_ext.cli.core.errors import handle_exception
        from nemo_helix_ext.cli.telemetry import runtime
        ...
        is_telemetry_root = bool(getattr(self, "_nhx_emit_command_invoked", False))
...
class NhxHelpFormatter(click.HelpFormatter):
...
class NhxArgument(TyperArgument):
...
class NhxContext(click.Context):
    """Context that uses NeMo Helix formatter."""
    ...
    def make_formatter(self) -> NhxHelpFormatter:
        """Create NeMo Helix-style formatter."""
        return NhxHelpFormatter(
            width=self.terminal_width,
            max_width=self.max_content_width,
        )


class NhxCommand(NhxErrorHandlingMixin, TyperCommand):
    """Command with NeMo Helix-style help formatting."""

    context_class = NhxContext

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initialize and patch options and arguments."""
        super().__init__(*args, **kwargs)
        for param in self.params:
            if isinstance(param, TyperOption) and type(param) is not NhxOption:
                param.__class__ = NhxOption
            elif isinstance(param, TyperArgument) and type(param) is not NhxArgument:
                param.__class__ = NhxArgument
    ...
    def format_options(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        for param in self.get_params(ctx):
            ...
            if isinstance(param, click.Argument):
                ...
                if not isinstance(param, NhxOption) and rv:
...
class NhxGroup(NhxErrorHandlingMixin, TyperGroup):
    """Group with NeMo Helix-style help formatting."""
