...
def get_models(
    ...
    if handle_code_generation(["inference", "models"], "get", kwargs, output_format, state):
        return

    client = state.get_client()
    result = client.inference.models.get(name, **kwargs)
...
def list_models(
    ...
    if handle_code_generation(["inference", "models"], "list", kwargs, output_format, state):
        return

    client = state.get_client()
    path_args = ()
    items = client.inference.models.list(*path_args, **kwargs)
