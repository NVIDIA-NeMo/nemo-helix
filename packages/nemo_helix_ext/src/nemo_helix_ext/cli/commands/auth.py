...
"""Authentication commands for NeMo Helix CLI.

Scope normalization, JWT decode, and scope validation logic live in
nemo_helix_ext.auth.helpers.
"""
...
import httpx
import typer
from nemo_helix_plugin.auth.access_keys.client import AccessKeyIssuerClient, AccessKeysClient
...
access_keys_app = create_typer_app(
    name="access-keys",
    help="Manage NeMo Helix Scoped Access Keys.",
)
...
def ensure_valid_token(context: Context, refresh_buffer_seconds: int = 300) -> bool:
    ...
    if not nhx_config.client_id or not nhx_config.token_endpoint:
        return exp_dt > now
    ...
    try:
        provider = OIDCTokenProvider(
            ...
            client_id=nhx_config.client_id,
...
def auth_callback(ctx: typer.Context) -> None:
    """Manage authentication for NeMo Helix."""
...
def login(
    ...
    password: Annotated[
        str | None,
        typer.Option("--password", help="Password for password grant (prefer env NHX_OIDC_PASSWORD)"),
    ] = None,
    ...
    nemo auth login --context staging --base-url https://nhx.staging.example.com
...
def logout(ctx: typer.Context) -> None:
    ...
    from nemo_helix_ext.config.config import Config
    from nemo_helix_ext.config.models import NoAuthUser
