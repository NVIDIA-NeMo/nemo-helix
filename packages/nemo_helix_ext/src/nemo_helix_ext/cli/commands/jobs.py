...
from nemo_helix_plugin.client.response import NemoPaginatedResponse
from nemo_helix_plugin.jobs.client import JobsClient
from nemo_helix_plugin.jobs.schemas import PlatformJobResultCreateRequest
...
def _jobs_client_from_state(state: CLIContext) -> tuple[NeMoHelix, JobsClient]:
...
def _generate_jobs_python_code(
    *,
    ...
    lines.extend(
        [
            "",
            f"platform_client = NeMoHelix(base_url={_format_python_literal(base_url)})"
            if base_url
            else "platform_client = NeMoHelix()",
