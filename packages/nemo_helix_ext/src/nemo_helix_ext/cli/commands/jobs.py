...
from nemo_helix_plugin.jobs.schemas import HelixJobResultCreateRequest
...
from nemo_helix_ext.cli.core.code_generator import format_code_output
...
from nemo_helix_ext.cli.core.job_log_renderer import render_job_logs
...
from nemo_helix_ext.cli.core.stdin_utils import read_data_input_with_flags, read_payload, validate_required_fields
...
from nemo_helix_ext.cli.core.waiters import wait_for_platform_job
    ...
    *,
    ...
    lines = [
        ...
        (
            ...
            "JobStatusDetailsUpdate, HelixJobStatusUpdateRequest, HelixJobTaskUpdate"
        ),
    ]
    if lifecycle_mode == "wait":
        lines.append("from nemo_helix_plugin.jobs.watch_types import JobStatusEvent, JobWatchTimeoutError")
    lines.extend(
        [
            "",
...
def _render_source_jobs_call(resource_path: list[str], method: str) -> list[str]:
    ...
    if operation == (("jobs",), "create"):
        return [
            ...
            "    workspace=workspace,",
            "    body=CreateHelixJobRequest.model_validate(request),",
            ")",
            "print(response.data())",
        ]
    ...
    if operation == (("jobs", "results"), "create"):
        return [
            ...
            "    job=args['job'],",
            "    name=args['name'],",
            "    body=HelixJobResultCreateRequest.model_validate(request),",
            ")",
            "print(response.data())",
        ]
    ...
    if operation == (("jobs", "steps"), "update_status"):
        return [
            ...
            "    job=args['job'],",
            "    name=args['name'],",
            "    body=HelixJobStatusUpdateRequest.model_validate(request),",
            ")",
            "print(response.data())",
        ]
    if operation == (("jobs", "tasks"), "create_or_update"):
        return [
            ...
            "    step=args['step'],",
            "    name=args['name'],",
            "    body=HelixJobTaskUpdate.model_validate(request),",
            ")",
            "print(response.data())",
        ]
...
def create_jobs(
    ...
    result = jobs_client.create_job(
        ...
        body=CreateHelixJobRequest.model_validate(request_payload),
    ).data()
    format_output(
        result,
        is_list=False,
...
def create_results(
    ...
    result = jobs_client.create_job_result(
        ...
        name=name,
        body=HelixJobResultCreateRequest.model_validate(
...
def update_status_steps(
    ...
    result = jobs_client.update_job_step_status(
        ...
        job=cast(str, input_payload["job"]),
        name=name,
        body=HelixJobStatusUpdateRequest.model_validate(
            _without_keys(input_payload, {"workspace", "job"}),
        ),
    ).data()
...
def create_or_update_tasks(
    ...
    result = jobs_client.update_job_step_task(
        ...
        name=name,
        body=HelixJobTaskUpdate.model_validate(
