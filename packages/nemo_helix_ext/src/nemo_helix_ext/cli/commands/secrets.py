...
from nemo_helix_plugin.client.types import OffsetPagination, OffsetPaginationMetadata
...
from nemo_helix_ext.cli.core.pagination import PaginationType, warn_if_more_pages
from nemo_helix_ext.cli.core.stdin_utils import resolve_secret_value, validate_required_fields
...
class _PaginationMetadata:
    ...
    @classmethod
    def for_all_items(cls, items: list[HelixSecretResponse], page_size: int | None) -> _PaginationMetadata:
        return cls(
            page=1,
...
def _update_body(*, description: str | None, secret_data: str | None) -> HelixSecretUpdateRequest:
    if description is not None and secret_data is not None:
        return HelixSecretUpdateRequest(description=description, value=SecretStr(secret_data))
    if description is not None:
        return HelixSecretUpdateRequest(description=description)
    if secret_data is not None:
        return HelixSecretUpdateRequest(value=SecretStr(secret_data))
...
def _source_secret_page(
    response: NemoPaginatedResponse[HelixSecretResponse, OffsetPagination],
    *,
    all_pages: bool,
    page_size: int | None,
...
def _emit_secrets_output(
    ...
    *,
    ...
    from nemo_helix_ext.cli.core.table_config import resolve_and_validate_columns, validate_output_columns
