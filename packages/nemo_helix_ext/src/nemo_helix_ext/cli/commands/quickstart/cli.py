...
"""Quickstart CLI commands for managing the NeMo Helix container."""
...
from nemo_helix_ext.quickstart.prompts import RegistryCredentials
...
def _should_prompt_for_pull_credentials(error: Exception, config: QuickstartConfig) -> bool:
    ...
    from nemo_helix_ext.quickstart.prompts import detect_registry_auth_type
...
def _configure_registry_auth(
    *,
    ...
    from nemo_helix_ext.quickstart import prompt_for_registry_credentials
    from nemo_helix_ext.quickstart.prompts import _registry_host_from_image
...
def _write_quickstart_config(port: int = 8080, *, admin_email: str | None = None) -> None:
    """Write quickstart cluster and context to the NeMo Helix config file.
...
quickstart_app = typer.Typer(help="Quickstart commands for managing the NeMo Helix container.")
...
def status(ctx: typer.Context) -> None:
    ...
    from nemo_helix_ext.quickstart import QuickstartCluster
...
def configure(
    ...
    from nemo_helix_ext.quickstart import (
        QuickstartConfig,
    ...
    if auto:
        ...
        console.print("  ╔══════════════════════════════════════════════════════════════╗")
        console.print("  ║                                                              ║")
        console.print("  ║      NeMo Helix Quickstart Auto-Configuration             ║")
        console.print("  ║                                                              ║")
        console.print("  ╚══════════════════════════════════════════════════════════════╝\n")
...
def up(
    ...
    console.print("\n[bold cyan]Starting NeMo Helix Quickstart[/bold cyan]\n")
    ...
    if not skip_preflight:
        ...
        if cluster._preflight_checker.is_already_running():
            ...
            if force or image:
                ...
                console.print("\n[green]✓[/green] [green]NeMo Helix Quickstart is already running![/green]")
...
def destroy(
    ...
    from nemo_helix_ext.quickstart import QuickstartCluster, QuickstartConfig, prompt_confirm
...
def _run_job_diagnostic(port: int, registry: str, tag: str, *, admin_email: str | None = None) -> bool:
    ...
    Args:
        port: The port the NeMo Helix service is running on.
...
def doctor(ctx: typer.Context) -> None:
    ...
    from nemo_helix_ext.quickstart import QuickstartCluster, QuickstartConfig

    console.print("\n[bold cyan]NeMo Helix Quickstart Doctor[/bold cyan]\n")
        ...
        try:
            ...
            exit_code, output = container.exec_run(
                [
                    "sh",
                    "-c",
                    "echo NHX_IMAGE_REGISTRY=$NHX_IMAGE_REGISTRY && echo NHX_IMAGE_TAG=$NHX_IMAGE_TAG",
                ]
            )
            if exit_code == 0:
