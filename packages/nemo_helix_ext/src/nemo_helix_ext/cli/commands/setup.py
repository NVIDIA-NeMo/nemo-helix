...
from nemo_helix import APIConnectionError, APIStatusError, APITimeoutError, NeMoHelix
...
from nhx.platform_runner.config import DEFAULT_LOCAL_SERVICES_BIND_HOST, PlatformAppConfig
...
from nemo_helix_ext.cli.commands.skills.registry import get_installer, load_skills
...
def _provider_exists(client: NeMoHelix, name: str, workspace: str) -> bool:
...
def _secret_exists(client: NeMoHelix, name: str, workspace: str) -> bool:
...
def _create_secret(client: NeMoHelix, name: str, value: str, workspace: str) -> None:
    secrets = client_from_platform(client, SecretsClient)
    secrets.create_secret(body=PlatformSecretCreateRequest(name=name, value=SecretStr(value)), workspace=workspace)


def _update_secret(client: NeMoHelix, name: str, value: str, workspace: str) -> None:
    secrets = client_from_platform(client, SecretsClient)
...
def _probe_model_entity(client: NeMoHelix, workspace: str, entity_id: str) -> bool:
...
def _select_usable_model_pair(client: NeMoHelix, workspace: str, entity_ids: list[str]) -> ModelPair | None:
...
def _auto_setup(client: NeMoHelix, workspace: str) -> str | None:
