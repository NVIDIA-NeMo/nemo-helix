...
# NeMo Helix SDK extensions

This package provides extensions to the generated `nemo-helix` SDK. It includes things that are built on top
of the SDK and is added to the SDK package (this way we only need to maintain a single package).

These extensions include:
- The CLI module (`nemo_helix_ext/src/nemo_helix_ext/cli`) - provides a `nhx` command that provides a way to interact with the platform.
...
This package is a member of the NeMo Helix workspace, so `uv sync` from the root of the workspace makes it available as `nhx` (or `uv run nhx` without venv activated).
...
```bash
# Quick setup
nemo config set --base-url https://nhx.example.com && nemo auth login
...
The CLI reads configuration from `~/.config/nhx/config.yaml`:
...
```bash
nemo config set --context production --base-url https://nhx.example.com --activate
```
...
2. **Environment variables** (`NHX_BASE_URL`)
3. **Config file** (`~/.config/nhx/config.yaml`)
4. **Defaults**
...
```bash
NHX_CONFIG_FILE=/path/to/config.yaml   # Override config file path
NHX_CURRENT_CONTEXT=local              # Which context to use
NHX_WORKSPACE=my-workspace             # Default workspace
NHX_BASE_URL=https://api.example.com   # API endpoint
NHX_API_KEY=your-key                   # Authentication
NHX_OUTPUT_FORMAT=json                 # Output format
NHX_TIMESTAMP_FORMAT=relative          # Timestamp display
NHX_PAGE_SIZE=50                       # Results per page
```
...
Run NeMo Helix locally with the recommended setup flow:
...
Built with [Typer](https://typer.tiangolo.com/), [Rich](https://rich.readthedocs.io/), and [nemo-helix](https://github.com/stainless-sdks/nemo-helix-v1-python).

See [docs/overview.md](docs/overview.md) for architecture details.

### Auto-Generated Code

**IMPORTANT:** Files in `src/nemo_helix_ext/cli/commands/api/` are auto-generated.
- Do NOT manually edit these files
- Do NOT include in code reviews
- Generated from templates in `tools/nemo-helix-sdk-tools/src/nemo_helix_sdk_tools/sdk/cli_generator/templates/`
