...
from nemo_helix_ext.cli.app import app, cli
...
from nemo_helix_ext.cli.version import client_version
...
def test_root_help_includes_lazy_api_commands():
    runner = CliRunner()
    sys.modules.pop("nemo_helix_ext.cli.commands.entities", None)
    sys.modules.pop("nhx.intake.cli", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.files", None)
    ...
    assert result.exit_code == 0
    # intake and experiments are nhx-intake plugin CLIs (nemo.cli entry points).
    ...
    assert "nemo_helix_ext.cli.commands.entities" not in sys.modules
    assert "nhx.intake.cli" not in sys.modules
    assert "nemo_helix_ext.cli.commands.files" not in sys.modules
...
def test_lazy_api_group_help_loads_on_demand():
    ...
    assert "nemo_helix_ext.cli.commands.files" in sys.modules
...
def test_build_top_level_lazy_entries_prefers_plugin_over_api_name_collision():
    ...
    module_entries = (
        TopLevelEntry(
            import_path="nemo_helix_ext.cli.commands.custom_plugin:app",
            name="custom-plugin",
    ...
    with (
        patch("nemo_helix_ext.cli.app.TOP_LEVEL_ENTRIES", module_entries),
        patch(
...
def test_cli_entry_point_discards_the_command_return_value(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    job so a wrapper can follow it, and ``NhxErrorHandlingMixin.main`` passes
