...
from nemo_helix_ext.cli.commands.api import API_TOP_LEVEL_ENTRIES
from nemo_helix_ext.cli.commands.manifest_registry import TOP_LEVEL_ENTRIES
from nemo_helix_ext.cli.core.lazy_load import (
...
from nemo_helix_ext.cli.manifest import TopLevelEntry, build_top_level_entries
...
def test_version_flag(flag):
    ...
    assert result.stdout.strip() == f"nemo version {nemo_helix.__version__}"
...
def test_help_includes_getting_started():
    ...
    assert "Set up NeMo Helix" in result.stdout
...
def test_generated_list_help_includes_stream_option():
    ...
    with patch("nemo_helix_ext.quickstart.QuickstartConfig.load", return_value=qs_config):
...
def test_generated_list_validates_stream_output_before_client_setup():
    ...
    with (
        ...
        patch("nemo_helix_ext.cli.core.context.CLIContext.get_client") as mock_get_client,
    ):
...
"""
    ...
    monkeypatch.setenv("NHX_CONFIG_FILE", str(config_path))
    monkeypatch.setenv("NHX_CURRENT_CONTEXT", "staging")
...
def test_root_no_arg_help_skips_active_context_when_config_unavailable(tmp_path, monkeypatch):
    monkeypatch.setenv("NHX_CONFIG_FILE", str(tmp_path / "missing.yaml"))
...
def test_root_help_includes_lazy_api_commands():
    runner = CliRunner()
    sys.modules.pop("nemo_helix_ext.cli.commands.api.entities", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.api.experiments", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.api.files", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.api.intake", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.auth", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.use_cases.chat", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.config", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.skills.cli", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.quickstart.cli", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.services.cli", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.use_cases.wait", None)
    result = runner.invoke(app, ["--help"])
    ...
    assert "nemo_helix_ext.cli.commands.api.entities" not in sys.modules
    assert "nemo_helix_ext.cli.commands.api.experiments" not in sys.modules
    assert "nemo_helix_ext.cli.commands.api.files" not in sys.modules
    assert "nemo_helix_ext.cli.commands.api.intake" not in sys.modules
    assert "nemo_helix_ext.cli.commands.auth" not in sys.modules
    assert "nemo_helix_ext.cli.commands.use_cases.chat" not in sys.modules
    assert "nemo_helix_ext.cli.commands.config" not in sys.modules
    assert "nemo_helix_ext.cli.commands.skills.cli" not in sys.modules
    assert "nemo_helix_ext.cli.commands.quickstart.cli" not in sys.modules
    assert "nemo_helix_ext.cli.commands.services.cli" not in sys.modules
    assert "nemo_helix_ext.cli.commands.use_cases.wait" not in sys.modules
...
def test_members_api_command_is_nested_under_workspaces():
    runner = CliRunner()
    sys.modules.pop("nemo_helix_ext.cli.commands.members", None)
    sys.modules.pop("nemo_helix_ext.cli.commands.workspaces", None)

    result = runner.invoke(app, ["workspaces", "members", "--help"])

    assert result.exit_code == 0
    assert "Manage members" in result.stdout
    assert "nemo_helix_ext.cli.commands.workspaces" in sys.modules
    assert "nemo_helix_ext.cli.commands.members" not in sys.modules
...
def test_auth_command_and_hidden_context_option_remain_invokable():
    ...
    assert result.exit_code == 0
    assert "Manage authentication for NeMo Helix" in result.stdout
...
def test_lazy_api_group_help_loads_on_demand():
    ...
    assert "nemo_helix_ext.cli.commands.api.files" in sys.modules


@pytest.mark.parametrize(
    ("argv", "module_name", "expected_text"),
    [
        (["auth", "--help"], "nemo_helix_ext.cli.commands.auth", "Manage authentication for NeMo Helix"),
        (["config", "--help"], "nemo_helix_ext.cli.commands.config", "Manage NeMo Helix CLI configuration"),
        (["skills", "--help"], "nemo_helix_ext.cli.commands.skills.cli", "Install AI agent skill files for Nemo"),
        (
            ["quickstart", "--help"],
        ...
        (
            ["wait", "--help"],
            "nemo_helix_ext.cli.commands.use_cases.wait",
...
def test_lazy_top_level_command_help_loads_on_demand():
    ...
    assert "nemo_helix_ext.cli.commands.use_cases.chat" in sys.modules
...
def test_lazy_top_level_command_preserves_loaded_hidden_flag():
    ...
    group = ManifestBackedNhxGroup(name="nemo", callback=callback)
    loaded_command = click.Command("hidden-command", hidden=True)

    with patch("nemo_helix_ext.cli.core.lazy_load.build_lazy_loader", return_value=lambda: loaded_command):
...
def test_lazy_group_help_uses_loaded_group_help():
    ...
    assert result.exit_code == 0
    assert "Read NeMo Helix documentation." in result.stdout
    assert "Read NeMo Helix documentation from the CLI." not in result.stdout
...
def test_build_top_level_lazy_entries_prefers_plugin_over_api_name_collision():
    ...
    api_entries = (
        TopLevelEntry(
            import_path="nemo_helix_ext.cli.commands.api.custom_plugin:app",
            name="custom-plugin",
        ...
        TopLevelEntry(
            import_path="nemo_helix_ext.cli.commands.api.files:app",
            name="files",
            help="Manage files.",
            panel="Core plugins",
            kind="group",
    ...
    with (
        patch("nemo_helix_ext.cli.app.TOP_LEVEL_ENTRIES", ()),
        patch("nemo_helix_ext.cli.app.API_TOP_LEVEL_ENTRIES", api_entries),
        patch(
            "nemo_helix_ext.cli.app._installed_plugin_command_entry_points",
...
def test_plugin_loader_registers_unavailable_command_for_broken_jobs():
    ...
    with (
        ...
        patch(
            "nemo_helix_ext.cli.app._discover_plugin_job_entry_points",
...
def test_plugin_loader_surfaces_customization_contributor_discovery_error():
    ...
    with patch("nemo_helix_ext.cli.core.lazy_load.resolve_name", return_value=_BrokenCustomizationCLI):
...
def test_token_refresh_runs_when_quickstart_auth_enabled():
    ...
    with (
        patch("nemo_helix_ext.quickstart.QuickstartConfig.load", return_value=qs_config),
        patch("nemo_helix_ext.cli.commands.auth.ensure_valid_token", return_value=True) as mock_ensure,
        patch("nemo_helix_ext.cli.core.context.CLIContext.get_sdk_context", return_value=MagicMock()),
        patch("nemo_helix_ext.cli.core.context.CLIContext.reset_sdk_context"),
    ):
        runner.invoke(app, ["workspaces", "--help"])
