...
"""Guard: the ``nemo`` CLI depends only on the typed clients in ``nemo_helix_plugin``.
...
3. The same subprocess technique covers the plugin seam: the
   ``nemo_helix_plugin`` discovery/CLI/scheduler modules import cleanly and
...
from nemo_helix_ext.cli.app import app  # noqa: E402

runner = CliRunner()
results = {}
for args in json.loads(sys.argv[1]):
    result = runner.invoke(app, args, env={"NHX_BASE_URL": "http://127.0.0.1:1"})
    results[" ".join(args)] = {
...
# here: both import the local-services runner (``nhx.platform_runner.config``,
# ``nemo_helix_ext.local.process``), which pulls in server-side packages
# (``nhx.common.service``) that still depend on the generated SDK.
...
def _run_probe(commands: list[list[str]], tmp_path: Path) -> dict[str, dict[str, object]]:
    env = {**os.environ, "NHX_CONFIG_FILE": str(tmp_path / "config.yaml")}
    for key in list(env):
        if key.startswith("NHX_") and key != "NHX_CONFIG_FILE":
            env.pop(key)
...
def test_code_output_never_references_the_generated_sdk(tmp_path: Path) -> None:
    # Exiting 0 is not enough: a command can run without nemo_helix and still
    ...
    offenders = {
        ...
        if "nemo_helix import" in str(info["output"]) or "NeMoHelix(" in str(info["output"])
    }
    assert not offenders, f"-f code snippets still built on the generated SDK: {json.dumps(offenders, indent=2)}"
    for cmd, info in results.items():
        assert "from nemo_helix_plugin." in str(info["output"]), f"{cmd} emitted no typed-client import"


# Plugin CLI groups discovered through the ``nemo.cli`` entry-point group. The
# discovery path (``nemo_helix_plugin.discovery`` and the modules it pulls in)
...
@pytest.mark.parametrize("module", ["nemo_helix_ext.cli.app", "nemo_helix_ext.cli.core.context"])
...
PLUGIN_CONTRACT_MODULES: tuple[str, ...] = (
    "nemo_helix_plugin.discovery",
    "nemo_helix_plugin.cli",
    "nemo_helix_plugin.commands",
    "nemo_helix_plugin.scheduler",
)
...
def test_plugin_contract_import_graph_is_free_of_generated_sdk() -> None:
    probe = (
        ...
        + "loaded = sorted(name for name in sys.modules if name == 'nemo_helix' or name.startswith('nemo_helix.'))\n"
