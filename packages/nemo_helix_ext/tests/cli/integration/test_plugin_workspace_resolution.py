...
The unit coverage in ``packages/nemo_helix_plugin/tests/test_commands.py``
...
def config_file(isolated_config, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    fixture has cleared ``NHX_*`` env vars and pointed ``NHX_CONFIG_FILE`` at
...
def _app(register) -> typer.Typer:
    ...
    Mirrors what ``nemo_helix_ext.cli.app.main`` does for plugin commands:
