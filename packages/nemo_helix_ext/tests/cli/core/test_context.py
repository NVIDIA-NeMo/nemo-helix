...
def test_get_client_uses_config_bootstrap_for_persisted_oauth_context_and_is_cached():
    ...
    with (
        patch("nemo_helix_ext.config.config.get_context", return_value=resolved_context),
        patch("nemo_helix_ext.config.config.Config.load") as mock_config_load,
        patch("nemo_helix_ext.config.config.Config.runtime_access_token_source_label", return_value=None),
        patch("nemo_helix.NeMoHelix", autospec=True) as mock_client_cls,
    ):
...
def test_get_async_client_uses_config_bootstrap_for_persisted_oauth_context_and_is_cached():
    ...
    with (
        ...
        patch("nemo_helix_ext.config.config.Config.load") as mock_config_load,
        patch("nemo_helix_ext.config.config.Config.runtime_access_token_source_label", return_value=None),
        patch("nemo_helix.AsyncNeMoHelix", autospec=True) as mock_client_cls,
    ):
        mock_config_load.return_value.get_config_file.return_value = config_file
...
def test_typed_client_shares_the_platform_clients_transport_and_auth():
    ...
    with patch("nemo_helix_ext.config.config.get_context", return_value=resolved_context):
...
def test_get_workspace_returns_none_when_no_context_resolves():
    ...
    with patch("nemo_helix_ext.config.config.get_context", side_effect=RuntimeError("no config")):
