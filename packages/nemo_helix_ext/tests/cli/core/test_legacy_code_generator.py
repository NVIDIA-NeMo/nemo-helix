...
import pytest
from nemo_helix_ext.cli.core.legacy_code_generator import generate_python_code
...
def test_generate_python_code_simple_list():
    ...
    assert "from nemo_helix import NeMoHelix" in code
    assert "client = NeMoHelix()" in code
...
def test_generate_python_code_with_base_url():
    ...
    assert 'client = NeMoHelix(base_url="http://test.example.com")' in code
...
def test_generate_python_code_with_platform_job_watch():
    ...
    assert "from nemo_helix.jobs.watch import watch_job" not in code
    assert "from nemo_helix_plugin.client.adapter import client_from_platform" in code
    assert "from nemo_helix_plugin.jobs.client import JobsClient" in code
...
def test_generate_python_code_with_platform_job_wait():
    ...
    assert "import time" not in code
    for symbol in ("JobStatusEvent", "JobWatchTimeoutError", "JobsClient", "NeMoHelix"):
        assert symbol in code
    assert "from nemo_helix_plugin.jobs.watch_types import JobStatusEvent, JobWatchTimeoutError" in code
    assert "jobs_client = client_from_platform(client, JobsClient)" in code
...
def test_generate_python_code_with_inference_deployment_wait():
    ...
    for symbol in ("APIConnectionError", "APIStatusError", "APITimeoutError", "NeMoHelix", "NotFoundError"):
        assert symbol in code
