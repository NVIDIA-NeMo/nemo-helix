...
def test_generate_python_code_simple_list():
    ...
    assert "from nemo_helix_plugin.secrets.client import SecretsClient" in code
...
def test_generate_python_code_renders_request_model_and_imports_it():
    body = HelixSecretCreateRequest(name="hf-token", value="s3cret", description="HF token")
    code = generate_python_code(SecretsClient, "create_secret", {"workspace": "default", "body": body})

    assert "from nemo_helix_plugin.secrets.types import HelixSecretCreateRequest" in code
    assert 'body=HelixSecretCreateRequest(name="hf-token", description="HF token", value="***")' in code
    assert "s3cret" not in code
    assert code.index("from nemo_helix_plugin.secrets.client") < code.index("client = SecretsClient")
...
def test_generate_python_code_renders_root_model_body_positionally():
    ...
    assert "from nemo_helix_plugin.inference_gateway.types import JsonBody" in code
...
def test_generate_python_code_inference_deployment_wait_block():
    ...
    assert "from nemo_helix_plugin.client.errors import NemoHTTPError, NemoTransportError, NotFoundError" in code
    assert "from nemo_helix_plugin.inference_gateway.client import InferenceGatewayClient" in code
