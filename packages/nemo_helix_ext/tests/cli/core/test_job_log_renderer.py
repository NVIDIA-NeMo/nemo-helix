...
def test_render_job_logs_prints_logs_in_order() -> None:
    ...
    render_job_logs(
        [
            HelixJobLog(timestamp=timestamp, job="job-a", job_step="step-a", job_task="task-a", message="started"),
            HelixJobLog(timestamp=timestamp, job="job-a", job_step="step-a", job_task="", message="running"),
        ],
        console=console,
    )
