...
from nemo_helix_plugin.client import errors as plugin_errors
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (AuthenticationError, 401, "Authentication error:", "NHX_ACCESS_TOKEN"),
...
def test_handle_usage_error_with_context(capsys):
    mock_parent_ctx = _make_mock_context("nhx", parent=None)
...
def test_not_found_hint_shows_specific_list_cmd(capsys):
    ...
    root_ctx = _make_mock_context("nhx", parent=None)
...
def test_pydantic_validation_error_is_usage_error(capsys):
    ...
    try:
        HelixSecretCreateRequest(name="x", value="v")
