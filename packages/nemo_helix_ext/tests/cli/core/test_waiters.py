...
import httpx
import pytest
from nemo_helix_ext.cli.core import waiters
...
WAITERS_MODULE = "nemo_helix_ext.cli.core.waiters"
WATCH_MODULE = "nemo_helix_plugin.jobs.watch"
...
class _StatusResponse:
    def __init__(self, status: HelixJobStatusResponse) -> None:
        self._status = status

    def data(self) -> HelixJobStatusResponse:
        return self._status
...
def _status_response(status: str | HelixJobStatus) -> _StatusResponse:
...
def test_platform_job_wait_live_display_recomputes_elapsed() -> None:
    display = waiters._HelixJobWaitLiveDisplay(start_time=100.0, timeout=1200, poll_interval=3)
...
def test_wait_for_platform_job_uses_dynamic_live_display_for_unchanged_status_polls() -> None:
    ...
    assert isinstance(live.renderable, waiters._HelixJobWaitLiveDisplay)
