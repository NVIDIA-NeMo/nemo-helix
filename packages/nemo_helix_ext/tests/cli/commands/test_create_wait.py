...
import pytest
import typer
from nemo_helix_ext.cli.commands.inference.deployments import create_deployments
from nemo_helix_ext.cli.commands.jobs import create_jobs, watch_platform_job
from nemo_helix_ext.cli.core.job_watch_renderer import JobWatchRenderResult
from nemo_helix_plugin.models.client import ModelsClient
from nemo_helix_plugin.models.types import CreateModelDeploymentRequest
...
def _patch_jobs_client(jobs_client: MagicMock):
    ...
    return patch("nemo_helix_ext.cli.commands.jobs.JobsClient", jobs_client_cls)
...
def test_jobs_create_watch_uses_sdk_watcher_and_outputs_created_job() -> None:
    ...
    with (
        patch(
            "nemo_helix_ext.cli.commands.jobs.handle_code_generation",
            return_value=False,
        ) as handle_code_generation,
        _patch_jobs_client(jobs_client),
        patch("nemo_helix_ext.cli.commands.jobs.format_output") as format_output,
        patch(
            "nemo_helix_ext.cli.commands.jobs.render_job_watch_events",
...
def test_jobs_create_watch_exits_when_renderer_reports_failure() -> None:
    ...
    with (
        patch("nemo_helix_ext.cli.commands.jobs.handle_code_generation", return_value=False),
        _patch_jobs_client(jobs_client),
        patch(
            "nemo_helix_ext.cli.commands.jobs.render_job_watch_events",
...
def test_jobs_create_wait_uses_quiet_waiter_and_outputs_created_job() -> None:
    ...
    with (
        ...
        _patch_jobs_client(jobs_client),
        patch("nemo_helix_ext.cli.commands.jobs.format_output") as format_output,
        patch("nemo_helix_ext.cli.commands.jobs.wait_for_platform_job", return_value=True) as wait_for_job,
        patch("nemo_helix_ext.cli.commands.jobs.render_job_watch_events") as render_events,
    ):
        create_jobs(
            ctx,
            name="input-job",
...
def test_jobs_create_stamps_telemetry_custom_fields() -> None:
    ...
    with (
        patch("nemo_helix_ext.cli.commands.jobs.handle_code_generation", return_value=False),
        _patch_jobs_client(jobs_client),
        patch("nemo_helix_ext.cli.commands.jobs.format_output"),
    ):
        create_jobs(
            ctx,
            name="input-job",
...
def test_jobs_create_wait_exits_when_waiter_reports_failure() -> None:
    ...
    with (
        patch("nemo_helix_ext.cli.commands.jobs.handle_code_generation", return_value=False),
        _patch_jobs_client(jobs_client),
        patch("nemo_helix_ext.cli.commands.jobs.wait_for_platform_job", return_value=False),
        pytest.raises(typer.Exit) as exc_info,
    ):
        create_jobs(
            _ctx(client),
...
def test_inference_deployment_create_exits_when_wait_fails() -> None:
    ...
    with (
        patch("nemo_helix_ext.cli.commands.inference.deployments.handle_code_generation", return_value=False),
        patch("nemo_helix_ext.cli.commands.inference.deployments.format_output"),
        patch(
            "nemo_helix_ext.cli.commands.inference.deployments.wait_for_inference_deployment",
            return_value=False,
...
def test_inference_deployment_create_watch_uses_waiter() -> None:
    ...
    with (
        patch(
            "nemo_helix_ext.cli.commands.inference.deployments.handle_code_generation",
            return_value=False,
        ) as handle_code_generation,
        patch("nemo_helix_ext.cli.commands.inference.deployments.format_output") as format_output,
        patch(
            "nemo_helix_ext.cli.commands.inference.deployments.wait_for_inference_deployment",
            return_value=True,
        ) as wait_for_inference_deployment,
