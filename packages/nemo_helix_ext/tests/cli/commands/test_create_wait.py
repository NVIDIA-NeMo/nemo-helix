...
import pytest
import typer
from nemo_helix_ext.cli.commands.api.inference.deployments import create_deployments
...
def test_jobs_create_watch_uses_sdk_watcher_and_outputs_created_job() -> None:
    ...
    with (
        ...
        patch("nemo_helix_ext.cli.commands.jobs.client_from_platform", return_value=jobs_client),
...
def test_inference_deployment_create_exits_when_wait_fails() -> None:
    ...
    with (
        patch("nemo_helix_ext.cli.commands.api.inference.deployments.handle_code_generation", return_value=False),
        patch("nemo_helix_ext.cli.commands.api.inference.deployments.format_output"),
        patch(
            "nemo_helix_ext.cli.commands.api.inference.deployments.wait_for_inference_deployment",
            return_value=False,
...
def test_inference_deployment_create_watch_uses_waiter() -> None:
    ...
    with (
        patch(
            "nemo_helix_ext.cli.commands.api.inference.deployments.handle_code_generation",
            return_value=False,
        ) as handle_code_generation,
        patch("nemo_helix_ext.cli.commands.api.inference.deployments.format_output") as format_output,
        patch(
            "nemo_helix_ext.cli.commands.api.inference.deployments.wait_for_inference_deployment",
            return_value=True,
        ) as wait_for_inference_deployment,
