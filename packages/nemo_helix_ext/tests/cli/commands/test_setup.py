...
class TestCheckHelixReachable:
    ...
    def test_env_certificate_authority_overrides_context_certificate_authority(self):
        ...
        mock_resp.status_code = 200
        with (
            patch.dict("os.environ", {"NHX_CLIENT_SSL_CERT_FILE": "/env/ca.pem"}, clear=True),
            patch(f"{SETUP_MOD}.httpx.get", return_value=mock_resp) as mock_get,
        ):
            assert (
                _check_platform_reachable(
    ...
    def test_reachable_via_cluster_info_when_status_missing(self):
        ...
        with (
            ...
            assert _check_platform_reachable("https://nemo-helix-freeplay.dev.aire.nvidia.com") is True
        assert calls == [
            ("https://nemo-helix-freeplay.dev.aire.nvidia.com/status", "/tmp/custom-ca.pem"),
            ("https://nemo-helix-freeplay.dev.aire.nvidia.com/cluster-info", "/tmp/custom-ca.pem"),
        ]
...
class TestCheckHelixReachableWithRetries:
...
class TestCheckOllamaRunning:
    def test_ollama_running(self):
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        with patch("nemo_helix_ext.cli.commands.setup.httpx.get", return_value=mock_resp):
            assert _check_ollama_running("http://localhost:11434/v1") is True

    def test_ollama_not_running(self):
        with patch("nemo_helix_ext.cli.commands.setup.httpx.get", side_effect=Exception("conn refused")):
...
def _bypass_key_validation():
    ...
    with patch(
        "nemo_helix_ext.cli.commands.setup._validate_api_key",
...
class TestMaybeStartServices:
    ...
    def test_exits_early_when_services_extra_missing(self, maybe_start_preflight_mocks, capsys):
        ...
        captured = capsys.readouterr()
        assert "nemo-helix[all]" in captured.err
...
class TestRemoteConnection:
    ...
    def test_reuses_runtime_access_token_override(self, monkeypatch):
        monkeypatch.setenv("NHX_ACCESS_TOKEN", "runtime-token")
        cli_context = MagicMock()
        cli_context.get_sdk_context.return_value = Context(
            context_name="default",
            cluster=Cluster(name="remote", base_url="https://remote.example.com"),
        ...
        with patch("nemo_helix_ext.cli.commands.auth._login_with_oidc") as mock_login:
            setup_commands._ensure_platform_auth(cli_context)
    ...
    def test_clears_stale_credentials_when_remote_auth_is_disabled(self, tmp_path, monkeypatch):
        ...
        with patch("nemo_helix_ext.cli.commands.auth._login_with_oidc", return_value=False):
            setup_commands._ensure_platform_auth(cli_context)
...
class TestLocalDataDirHelpers:
    ...
    def test_start_services_background_forwards_data_dir(self):
        ...
        process lifecycle module unchanged. The actual ``NHX_DATA_DIR``
...
class TestDetectCodingAgents:
    def test_detects_cursor_directory(self, tmp_path):
        ...
        (tmp_path / ".git").mkdir()
        with patch("nemo_helix_ext.cli.commands.setup.Path.cwd", return_value=tmp_path):
...
class TestLoadSkillsWithWarnings:
    def test_captures_plugin_warnings(self):
        ...
        with patch("nemo_helix_ext.cli.commands.setup.load_skills", side_effect=_stub):
    ...
    def test_handler_cleaned_up_on_exception(self):
        ...
        with patch("nemo_helix_ext.cli.commands.setup.load_skills", side_effect=_boom):
...
class TestMaybeInstallSkills:
    ...
    def _patched_install(self, tmp_path, **call_kwargs):
        ...
        # alpha is built-in (source_plugin=None → "nemo-helix"); beta comes
        ...
        with (
            patch(
                "nemo_helix_ext.cli.commands.setup._detect_coding_agents",
                return_value=[("AGENTS.md", "codex"), (".cursor", "cursor"), (".claude", "claude")],
            ),
            patch(
                "nemo_helix_ext.cli.commands.setup._load_skills_with_warnings",
                return_value=(skills, []),
            ),
            patch(
                "nemo_helix_ext.cli.commands.setup._find_project_root",
                return_value=tmp_path,
            ),
            patch(
                "nemo_helix_ext.cli.commands.setup.get_installer",
    ...
    def test_interactive_without_markers_offers_every_agent(self, tmp_path):
        ...
        multiselect = MagicMock(side_effect=[["nemo-helix"], ["claude"]])
        with (
            patch("nemo_helix_ext.cli.commands.setup._detect_coding_agents", return_value=[]),
            patch("nemo_helix_ext.cli.commands.setup._load_skills_with_warnings", return_value=(skills, [])),
            patch("nemo_helix_ext.cli.commands.setup._find_project_root", return_value=tmp_path),
            patch("nemo_helix_ext.cli.commands.setup.is_interactive", return_value=True),
            patch("nemo_helix_ext.cli.commands.setup.prompt_multiselect", multiselect),
            patch("nemo_helix_ext.cli.commands.setup.prompt_choice", return_value=Scope.PROJECT.value),
            patch("nemo_helix_ext.cli.commands.setup.prompt_confirm", return_value=True),
            patch(
                "nemo_helix_ext.cli.commands.setup.get_installer",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"install_skills": None, "skills_from": ["nemo-helix"]},
        ],
        ids=[
    ...
    def test_skills_from_builtin_only(self, tmp_path):
        """--skills-from nemo-helix installs only built-in skills."""
        calls = self._patched_install(
            tmp_path,
            auto=True,
            install_skills=True,
            skills_from=["nemo-helix"],
        )
    ...
    def test_skills_from_multiple_sources_unions(self, tmp_path):
        ...
        calls = self._patched_install(
            ...
            auto=True,
            install_skills=True,
            skills_from=["nemo-helix", "example-plugin"],
        )
        for _name, _scope, sk in calls:
...
class TestFindProjectRoot:
    def test_finds_git_root(self, tmp_path):
        ...
        with patch("nemo_helix_ext.cli.commands.setup.Path.cwd", return_value=subdir):
...
class TestProviderIdempotency:
    ...
    def test_existing_provider_gets_updated_with_secret(self):
        ...
        assert isinstance(create_call.kwargs["body"], HelixSecretCreateRequest)
...
class TestValidateApiKey:
    ...
    def test_rejected_key_does_not_blame_probe_target(self):
        ...
        rejected = _http_response(401, {"detail": "Invalid API key"}, text='{"detail":"Invalid API key"}')
    ...
    def test_gone_probe_includes_upstream_detail_and_does_not_fail(self):
        ...
        catalog = _http_response(200, {"data": [{"id": "nvidia/nemotron-3-nano-30b-a3b"}]})
        gone = _http_response(410, {"title": "Gone", "status": 410, "detail": detail})
...
class TestValidateApiKeyIntegration:
    ...
    def test_auto_gone_probe_still_registers_provider(self):
        """HTTP 410 on the chat probe must not abort provider registration or blame the key."""
        client = _make_mock_client()
        catalog = _http_response(200, {"data": [{"id": "nvidia/nemotron-3-nano-30b-a3b"}]})
        gone = _http_response(
            410,
    ...
    def test_auto_rejected_key_still_points_at_env_var(self):
        client = _make_mock_client()
        catalog = _http_response(200, {"data": [{"id": "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning"}]})
        rejected = _http_response(401, {"detail": "Unauthorized"})
    ...
    def test_auto_warning_escapes_rich_markup_in_upstream_detail(self):
        client = _make_mock_client()
        catalog = _http_response(200, {"data": [{"id": "nvidia/nemotron-3-nano-30b-a3b"}]})
        gone = _http_response(410, {"detail": "invalid value at [/messages/0]"})
        ...
        with (
            ...
            result = _auto_setup(client, "default")
        assert result == "nvidia-build"
        printed = " ".join(str(call.args[0]) for call in print_message.call_args_list if call.args)
        assert r"\[/messages/0]" in printed
...
def spinner_console():
    ...
    with patch("nemo_helix_ext.cli.commands.setup.console") as mock_console:
...
class TestCheckControllerHealth:
    ...
    def test_status_not_published_on_hosted_deployment(self):
        ...
        with patch(f"{SETUP_MOD}.httpx.get", side_effect=_get):
            ok, msg = _check_controller_health("https://nemo-helix-freeplay.dev.aire.nvidia.com")
        assert ok is True
...
class TestVerifyHelixHealth:
    ...
    def test_missing_status_endpoint_returns_true_with_info(self):
        with (
            ...
            result = _verify_platform_health("https://nemo-helix-freeplay.dev.aire.nvidia.com")
        assert result is True
