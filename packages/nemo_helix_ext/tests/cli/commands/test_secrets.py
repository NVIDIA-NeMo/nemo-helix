...
from nemo_helix_plugin.client.types import OffsetPaginationMetadata
...
class _PaginatedSecretsResponse:
    def __init__(self, items: list[HelixSecretResponse], metadata: OffsetPaginationMetadata) -> None:
        self._items = items
        self._metadata = metadata
        self.page_called = False
        self.items_called = False

    def page(self) -> PageResult[HelixSecretResponse, OffsetPaginationMetadata]:
        self.page_called = True
        return PageResult(items=self._items, metadata=self._metadata)

    def items(self) -> Iterator[HelixSecretResponse]:
...
class _FakeSecretsClient:
    def __init__(self) -> None:
        ...
        self.list_response = _PaginatedSecretsResponse(
            [
                HelixSecretResponse(name="hf-token", workspace="default"),
                HelixSecretResponse(name="wandb-key", workspace="default"),
            ],
            {
                "page": 1,
                "page_size": 20,
    ...
    def create_secret(self, *, workspace: str | None, body: HelixSecretCreateRequest) -> _Response[HelixSecretResponse]:
        self.create_calls.append(_CreateSecretCall(workspace=workspace, body=body))
        return _Response(HelixSecretResponse(name=body.name, workspace=workspace or "default"))

    def update_secret(
        self, *, workspace: str | None, name: str, body: HelixSecretUpdateRequest
    ) -> _Response[HelixSecretResponse]:
        self.update_calls.append(_UpdateSecretCall(workspace=workspace, name=name, body=body))
        return _Response(HelixSecretResponse(name=name, workspace=workspace or "default"))
    ...
    def get_secret(self, *, workspace: str | None, name: str) -> _Response[HelixSecretResponse]:
        self.get_calls.append((name, workspace))
        return _Response(HelixSecretResponse(name=name, workspace=workspace or "default"))

    def access_secret(self, *, workspace: str | None, name: str) -> _Response[HelixSecretAccessResponse]:
        self.access_calls.append((name, workspace))
        return _Response(HelixSecretAccessResponse(name=name, workspace=workspace or "default", value="secret"))

    def delete_secret(self, *, workspace: str | None, name: str) -> _Response[None]:
        self.delete_calls.append((name, workspace))
        return _Response(None)

    def rotate_encryption_keys(self) -> _Response[HelixSecretAdminRotationResponse]:
        self.rotate_called = True
        return _Response(HelixSecretAdminRotationResponse(rotated_secrets=0, success=True))
...
def _invoke(secrets: _FakeSecretsClient, args: list[str]) -> Result:
    with (
        patch("nemo_helix_ext.quickstart.QuickstartConfig.load", return_value=_qs_no_auth),
        patch("nemo_helix_ext.cli.core.context.CLIContext.get_client", return_value=_FakeSDK()),
        patch("nemo_helix_ext.cli.commands.secrets.client_from_platform", return_value=secrets),
    ):
...
def test_code_output_uses_typed_secrets_client() -> None:
    ...
    assert result.exit_code == 0, result.output
    assert "from nemo_helix_plugin.secrets.client import SecretsClient" in result.stdout
    assert "nemo_helix import" not in result.stdout
    assert "client.create_secret(" in result.stdout
    assert 'HelixSecretCreateRequest(name="hf-token", value="***")' in result.stdout
    assert "nvapi-real" not in result.stdout
...
def test_update_code_output_preserves_unset_value_for_description_only_update() -> None:
    ...
    assert result.exit_code == 0, result.output
    assert 'body=HelixSecretUpdateRequest(description="metadata only")' in result.stdout
...
def test_update_code_output_preserves_unset_description_for_value_only_update() -> None:
    ...
    assert result.exit_code == 0, result.output
    assert 'body=HelixSecretUpdateRequest(value="***")' in result.stdout
