...
This module exists next to :mod:`nemo_platform_plugin.cli_state` on purpose:
the option and the resolver that consumes it belong together. Commands import
:data:`WorkspaceOption` and :func:`~nemo_platform_plugin.cli_state.resolve_cli_workspace`
...
- :func:`workspace_option` — the underlying ``typer.Option``, for the
  ...
  :mod:`nemo_platform_plugin.commands`, which assemble their signatures
...
#
# Note $NMP_WORKSPACE outranks the context's configured workspace: the SDK
...
# ``nemo_platform_ext.config.config.Config.resolve`` first.
...
def workspace_option(*, help: str = WORKSPACE_HELP, rich_help_panel: str | None = None) -> Any:
    ...
    :func:`~nemo_platform_plugin.cli_state.resolve_cli_workspace` on the first
