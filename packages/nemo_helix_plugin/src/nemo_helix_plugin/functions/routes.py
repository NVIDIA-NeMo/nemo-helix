...
:func:`~nemo_helix_plugin.jobs.routes.add_job_routes` for the function tier.
It returns an :class:`APIRouter` carrying a single ``POST`` route that:

- Validates the request body against
  :attr:`~nemo_helix_plugin.function.NemoFunction.spec_schema`.
- Builds a :class:`~nemo_helix_plugin.function_context.FunctionContext` from
...
injector that emits a :class:`~nemo_helix_plugin.functions.frames.Heartbeat`
    ...
    *,
    ...
    Args:
        ...
        authz: The plugin's :class:`~nemo_helix_plugin.authz.AuthzScope`.
            ...
            :data:`~nemo_helix_plugin.authz.GENERATED_ROUTE_CALLERS`, with
