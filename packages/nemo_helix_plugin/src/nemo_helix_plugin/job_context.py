...
:class:`~nemo_helix_plugin.job.NemoJob` so each job can ship its own payload
shape.
...
from nemo_helix_plugin.job_usage import JobUsageReporter, LocalJobUsageReporter
...
class JobContext:
    """Runtime context for :class:`~nemo_helix_plugin.job.NemoJob.run`.
    ...
    Attributes:
        ...
        results: Sink for publishing results (local directory for
            laptop runs, NeMo Helix fileset on the platform).
