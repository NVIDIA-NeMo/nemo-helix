...
class InvalidPermissionFormatError(ValueError):
    """Raised when a string is not valid NeMo Helix permission syntax (dot-separated segments)."""


class InvalidScopeFormatError(ValueError):
    """Raised when a string is not valid NeMo Helix scope syntax, or a permission was used as a scope."""
...
# NeMo Helix OAuth scopes in static-authz: ``area:verb`` (e.g. ``platform:read``, ``data-designer:write``).
NHX_SCOPE_PATTERN = re.compile(rf"^{_SEGMENT}:{_SEGMENT}$")


def is_valid_permission_id(value: str) -> bool:
    """Return True if *value* matches NeMo Helix permission syntax (dot-separated)."""
    if not isinstance(value, str) or not value:
        return False
    return PERMISSION_ID_PATTERN.fullmatch(value) is not None


def is_valid_nhx_scope_id(value: str) -> bool:
    """Return True if *value* matches NeMo Helix scope syntax used in ``static-authz.yaml`` (colon-separated)."""
    if not isinstance(value, str) or not value:
        return False
    return NHX_SCOPE_PATTERN.fullmatch(value) is not None
...
def validate_permission_strings(permissions: list[str], *, context: str) -> None:
    """Ensure each string is a valid NeMo Helix permission id.
...
def validate_nhx_scope_strings_for_config(scopes: list[str], *, context: str) -> None:
    """Validate scope strings from ``static-authz.yaml`` (strict NeMo Helix ``area:verb`` form).

    Raises:
        InvalidScopeFormatError: If any entry is not valid NeMo Helix scope syntax, or looks like a permission.
    """
    for s in scopes:
        ...
        if not is_valid_nhx_scope_id(s):
            raise InvalidScopeFormatError(
        ...
        for path, methods in endpoints.items():
            ...
            for method_name, op in methods.items():
                ...
                if isinstance(callers_list, list):
                    ...
                    from nemo_helix_plugin.authz import validate_caller_strings
