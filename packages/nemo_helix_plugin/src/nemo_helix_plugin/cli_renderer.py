...
A plugin's :class:`~nemo_helix_plugin.cli.NemoCLI` can return a :class:`CLIRenderer`
subclass from :meth:`~nemo_helix_plugin.cli.NemoCLI.get_function_renderer` /
:meth:`~nemo_helix_plugin.cli.NemoCLI.get_job_renderer`. When supplied, the framework
...
- :meth:`CLIRenderer.on_frame` — called once per frame as it arrives. For
  ...
  by :meth:`~nemo_helix_plugin.function.NemoFunction.run`). For remote ``submit`` the
