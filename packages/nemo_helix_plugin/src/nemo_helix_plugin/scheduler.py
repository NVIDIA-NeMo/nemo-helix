...
:func:`~nemo_helix_plugin.commands.add_job_commands`:

- :meth:`NemoJobScheduler.run_local` — execute the job in-process. Validates
  the spec against :attr:`~nemo_helix_plugin.job.NemoJob.input_spec_schema` (or
  :attr:`~nemo_helix_plugin.job.NemoJob.spec_schema`), invokes
  :meth:`~nemo_helix_plugin.job.NemoJob.to_spec` when an input shape is declared,
  then calls ``job.run(...)``.
...
When :attr:`~nemo_helix_plugin.job.NemoJob.spec_schema` is declared, the scheduler
...
from nemo_helix_plugin.run_dependencies import resolve_run_kwargs
...
class NemoJobScheduler:
    ...
    def run_local(
        ...
        *,
        ...
        Args:
            job_cls: :class:`~nemo_helix_plugin.job.NemoJob` subclass.
            ...
            sdk: Optional :class:`~nemo_helix.NeMoHelix` handle.
            ...
            async_sdk: Optional :class:`~nemo_helix.AsyncNeMoHelix`
        ...
        # ``nemo-helix-plugin-*`` dirs under ``$TMPDIR``; tests that need hermetic
    ...
    def _build_local_context(self, job_cls: type["NemoJob"], *, workspace: str) -> JobContext:
        ...
        """
        root = Path(tempfile.mkdtemp(prefix="nemo-helix-plugin-", suffix=f"-{job_cls.name}"))
