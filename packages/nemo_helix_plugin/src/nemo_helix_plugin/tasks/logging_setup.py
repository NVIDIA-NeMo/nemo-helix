...
dispatched through :mod:`nemo_helix_plugin.tasks.dispatcher`.
...
2. **Entry-point discovery** - scans the ``nemo.logging_provider`` group.
   When ``nhx-common`` is installed (every platform image), its provider is
...
def configure_task_logging() -> None:
    ...
    in-process, the ``nemo-helix run task`` entrypoint, or a test harness.
...
def _resolve_log_config() -> tuple[LogLevel, LogFormat]:
    ...
    """
    try:
        from nemo_helix_plugin.config import CommonServiceConfig, Configuration
...
def _resolve_provider() -> TaskLoggingProvider:
    ...
    # The nemo-helix bundle inherits nhx-common's entry points, so the same
