...
"""Task entrypoint dispatcher for :class:`~nemo_helix_plugin.job.NemoJob` subclasses.

Mirrors :meth:`~nemo_helix_plugin.scheduler.NemoJobScheduler.run_local` for any
...
:class:`~nemo_helix_plugin.job_context.JobContext` from the environment, and
invokes ``job.run(...)`` with the same signature-based DI used locally
(see :func:`~nemo_helix_plugin.run_dependencies.resolve_run_kwargs`).

Because ``run_task`` only runs inside a platform-spawned process, the
default ``ctx.results`` is :class:`~nemo_helix_plugin.job_results.PlatformJobResults`
...
from nemo_helix_plugin.run_dependencies import LocalRunError, resolve_run_kwargs
...
def run_task(
    ...
    *,
    ...
    :class:`~nemo_helix_plugin.run_dependencies.LocalRunError` propagates verbatim.

    The auto-built ``ctx`` wires
    :class:`~nemo_helix_plugin.job_results.PlatformJobResults` as ``ctx.results``,
    so *sdk* is required when *ctx* is not supplied.

    Root logging is bootstrapped via
    :func:`~nemo_helix_plugin.tasks.logging_setup.configure_task_logging`
    before anything else, since a task container configures none of its own.
    A caller that already configured logging keeps it.

    Args:
        job_cls: The :class:`~nemo_helix_plugin.job.NemoJob` subclass to run.
        sdk: :class:`~nemo_helix.NeMoHelix` handle, typically built via
            ``get_task_sdk("<plugin>")`` so ``NHX_PRINCIPAL`` threads through
...
def build_ctx_from_env(sdk: NeMoHelix) -> JobContext:
    ...
    the job class name — :class:`~nemo_helix_plugin.jobs.result_manager.ResultManager`
