...
- **``AuthContext``** is a pure-data mirror of ``nhx.common.auth.AuthContext``
  with the same wire shape and no ``from_principal``/``to_principal`` behavior.
- **``InferenceParams``** is a faithful replica of
  ``nhx.common.inference.InferenceParams`` (pure pydantic, no server imports).
- **``BackendFormat``** is reused from ``nemo_helix_plugin.inference_middleware``.
...
# Inlined constants
#
# Mirror ``nhx.common.entities.constants`` and ``nhx.core.models.constants``.
# Inlined rather than imported so this package stays free of an ``nhx_common``
...
# nhx.core.models.constants -- adapter/model reference rules.
...
# ---------------------------------------------------------------------------
# Inference parameters (replica of nhx.common.inference.InferenceParams)
# ---------------------------------------------------------------------------
