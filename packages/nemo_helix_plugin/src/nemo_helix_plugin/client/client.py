...
"""Typed HTTP client for NeMo Helix.
...
class BaseNemoClient(Generic[HttpClientT]):
    ...
    def access_keys(self) -> NemoClient | AsyncNemoClient:
        from nemo_helix_plugin.auth.access_keys.client import AccessKeysClient, AsyncAccessKeysClient

        return self._resource_client(AccessKeysClient, AsyncAccessKeysClient)

    @property
    def iam(self) -> NemoClient | AsyncNemoClient:
        from nemo_helix_plugin.iam.client import AsyncIAMClient, IAMClient
    ...
    def guardrail(self) -> NemoClient | AsyncNemoClient:
        from nemo_helix_plugin.guardrail.client import AsyncGuardrailClient, GuardrailClient
    ...
    def projects(self) -> NemoClient | AsyncNemoClient:
        from nemo_helix_plugin.projects.client import AsyncProjectsClient, ProjectsClient
...
class NemoClient(BaseNemoClient[httpx.Client]):
    ...
    def from_config(
        ...
        """Create a NemoClient from the user's nhx config file.
...
class AsyncNemoClient(BaseNemoClient[httpx.AsyncClient]):
    """Async HTTP client for NeMo Helix APIs.
    ...
    def __init_subclass__(cls, **kwargs: object) -> None:
        ...
        from nemo_helix_plugin.client.method import EndpointMethod
    ...
    def from_config(
        ...
        """Create an AsyncNemoClient from the user's nhx config file.
