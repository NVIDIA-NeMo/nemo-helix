...
Reads ``~/.config/nhx/config.yaml`` (the user-facing CLI/SDK config) and
...
:mod:`nemo_helix_plugin.config` which reads ``/etc/nhx/config.yaml``.
"""
...
class Config(BaseModel):
    ...
    Manages the user-facing config file (``~/.config/nhx/config.yaml``),
    runtime overrides (env vars, CLI flags, SDK params), and resolution
    of effective configuration.

    Environment variables (prefix ``NHX_``)::

        NHX_CURRENT_CONTEXT, NHX_WORKSPACE, NHX_OUTPUT_FORMAT,
        NHX_TIMESTAMP_FORMAT, NHX_COLOR_OUTPUT, NHX_BASE_URL, NHX_ACCESS_TOKEN
    """
    ...
    def _migrate_legacy_api_key_users(cls, config_data: dict) -> None:
        ...
        for user in users:
            ...
            if "@" in token:
                from nemo_helix_plugin.client.oidc import generate_unsigned_jwt
    ...
    def get_default_config_path(cls) -> Path:
        ...
        if xdg_config_home:
            return Path(xdg_config_home) / "nhx" / "config.yaml"
