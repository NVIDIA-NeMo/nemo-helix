...
:class:`~nemo_helix_plugin.service.NemoService`, :class:`~nemo_helix_plugin.controller.NemoController`,
...
Override for tests::
    ...
    from nemo_helix_plugin.config import set_nemo_config_override, clear_nemo_config_override
...
NHX_PREFIX_BASE = "NHX_"
NHX_CONFIG_FILE_PATH_ENV_VAR = "NHX_CONFIG_FILE_PATH"
NHX_CONFIG_FILE_PATH_DEFAULT = "/etc/nhx/config.yaml"
NHX_CONFIG_WARNINGS_DISABLED_ENV_VAR = "NHX_CONFIG_WARNINGS_DISABLED"

_NHX_DATA_DIR_ENV_VAR = "NHX_DATA_DIR"
_XDG_DATA_HOME_ENV_VAR = "XDG_DATA_HOME"
_DATA_DIR_NAME = "nemo"
_FALLBACK_DATA_DIR = Path(f"~/.local/share/{_DATA_DIR_NAME}")


def nhx_user_data_dir() -> Path:
    """Return the directory for persistent NeMo Helix local-development state.

    Resolution order:

    1. ``$NHX_DATA_DIR`` if set
    ...
    """
    override = os.environ.get(_NHX_DATA_DIR_ENV_VAR)
    if override:
...
if environ.get(NHX_CONFIG_WARNINGS_DISABLED_ENV_VAR, "") != "":
...
def get_service_config_prefix(service_name: str) -> str:
    ...
    if service_name == "platform":
        return NHX_PREFIX_BASE
    safe_name = re.sub(r"\W+", "_", service_name).upper()
    return f"{NHX_PREFIX_BASE}{safe_name}_"
...
_T_platform_config = TypeVar("_T_platform_config", bound="NemoHelixConfig")
...
class Configuration:
    ...
    _platform_config_class: ClassVar[Type["NemoHelixConfig"] | None] = None
        ...
        with open(yaml_file_path, "r") as file:
            try:
                ...
                if not isinstance(global_settings, dict):
                    raise ValueError(
                        f"NeMo Helix configuration file '{yaml_file_path}' does not contain a top-level dictionary."
                    )
                if not global_settings:
                    raise ValueError(
                        "NeMo Helix configuration file is empty. Please check the configuration file path, "
                        + f"or unset the environment variable '{NHX_CONFIG_FILE_PATH_ENV_VAR}' to use default values."
                    )
                return global_settings
            except yaml.YAMLError as e:
                raise ValueError(f"Error parsing NeMo Helix configuration file as YAML: {e}") from e

    @staticmethod
    def get_global_settings_from_env() -> dict:
        yaml_file_path = environ.get(NHX_CONFIG_FILE_PATH_ENV_VAR)
        if not yaml_file_path:
            yaml_file_path = NHX_CONFIG_FILE_PATH_DEFAULT
    ...
    def register_platform_config_class(cls, platform_config_class: Type[_T_platform_config]) -> None:
        ...
        """
        if not issubclass(platform_config_class, NemoHelixConfig):
            raise TypeError("platform_config_class must be a NemoHelixConfig subclass")
    ...
    @classmethod
    def get_platform_config_class(cls) -> Type["NemoHelixConfig"]:
    ...
    @classmethod
    def get_platform_config(cls) -> "NemoHelixConfig":
...
def get_platform_config_class() -> Type[NemoHelixConfig]:
    """Return the currently registered platform config class."""
...
def validate_docker_available() -> bool:
    ...
    Thin wrapper over :func:`nemo_helix_plugin.capabilities.probe_docker`.
    Prefer ``probe_docker`` when callers need the failure detail or a
    ``docker_host`` override.

    Uses ``use_cache=False`` so :meth:`NemoHelixConfig.validate_runtime`
...
class NemoHelixConfig(ServiceConfig):
    ...
    @classmethod
    def get(cls) -> NemoHelixConfig:
        """Return the active platform config.

        ``NemoHelixConfig`` itself is a public handle for the registered
    ...
    image_registry: str = Field(
        default="my-registry",
        description="Docker registry for NeMo Helix images (e.g., 'nvcr.io/nvidia/nemo-helix').",
    )
    image_tag: str = Field(
        default="local",
        description="Default tag for NeMo Helix images.",
    )
    ...
    seed_on_startup: bool = internal_field(
        ...
        description=(
            ...
            "Use for single nhx-api container deployments where a separate Helm Job is not used. "
            "The task waits for entities, auth, and files to be ready, then runs guardrails/evaluator/data-designer seeding. "
            "Set via config file (platform.seed_on_startup) or NHX_SEED_ON_STARTUP env var."
        ),
    )
    ...
    def get_service_url(self, api_name: str) -> str:
        ...
        Subclasses (e.g. in nhx-common) may override to add local-service routing.
        """
    ...
    def handle_docker_env_vars(cls, values: Any) -> Any:
        ...
        if isinstance(docker_config, dict) and "reserved_gpu_device_ids" not in docker_config:
            env_var = environ.get("NHX_DOCKER_RESERVED_GPU_DEVICE_IDS")
            if env_var:
...
def get_nemo_helix_config() -> NemoHelixConfig:
    """Returns the platform configuration singleton.

    Delegates to Configuration.get_platform_config() so that subclass overrides
    (e.g. nhx-common's PlatformConfig with local-service routing) are respected.
    """
    return Configuration.get_platform_config()


# Default implementation; platform packages can register an extended class.
Configuration.register_platform_config_class(NemoHelixConfig)

# Aliases — nhx-common and services use PlatformConfig / get_platform_config
PlatformConfig = NemoHelixConfig
get_platform_config = get_nemo_helix_config
...
class CommonServiceConfig(create_service_config_class("service")):  # ty: ignore[unsupported-base]
    ...
    Reads from env vars with prefix ``NHX_SERVICE_`` and YAML key ``service``.
    """
    ...
    log_level: Literal["DEBUG", "INFO", "WARN", "ERROR"] = Field(
        "INFO", alias="LOG_LEVEL", description="Logging level for the NeMo Helix."
    )
    scheme: str = Field(default="http", description="Scheme for the NeMo Helix service.")
    host: str = Field(default="127.0.0.1", description="Host for the NeMo Helix service.")
    port: int = Field(default=8080, description="Port for the NeMo Helix service.")
...
class NemoConfig(ServiceConfig, metaclass=_NemoConfigMeta):
    ...
    ``ClassVar[str]`` — exactly as they would on :class:`~nemo_helix_plugin.service.NemoService`
    or :class:`~nemo_helix_plugin.controller.NemoController`:
    ...
    .. attribute:: plugin_name
        ...
        the plugin's :attr:`~nemo_helix_plugin.service.NemoService.name` (and entry-point
    ...
    def get(cls) -> Self:
        ...
        Delegates to :func:`~nhx.common.config.base.Configuration.get_service_config`
