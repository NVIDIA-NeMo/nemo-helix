...
``TypedRequest`` from ``nemo_helix_plugin.inference_middleware_types``.
"""
...
class TypedResponse(Protocol):
    ...
    ``nemo_helix_plugin.inference_middleware_types``. The base middleware
...
class VirtualModel(Protocol):
    ...
    ``nemo_helix_plugin.inference_middleware``. The concrete Pydantic entity
...
class ModelSpec(Protocol):
    ...
    Concrete type: ``nhx.core.models.schemas.ModelSpec``.
    """
...
class ModelEntity(Protocol):
    ...
    Concrete type: ``nhx.core.models.schemas.ModelEntity``.
    Defined here as a Protocol so ``nemo_helix_plugin`` does not depend on the
    models service package.

    When the models service is migrated to define ``ModelEntity`` as a
    ``NemoEntity`` subclass in ``nemo_helix_plugin``, this Protocol is replaced
...
class ModelProvider(Protocol):
    ...
    Concrete type: ``nhx.core.models.entities.ModelProvider``.
    Defined here as a Protocol so ``nemo_helix_plugin`` does not depend on the
    models service package.
    """
    ...
    async def get_middleware_config(self, config_type: str, config_id: str) -> Any:
        ...
        Args:
            ...
            config_id: The ``"workspace/name"`` reference from
                :class:`~nemo_helix_plugin.inference_middleware.MiddlewareCall`.
