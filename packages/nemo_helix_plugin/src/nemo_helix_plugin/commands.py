...
- :func:`add_job_commands` — three-verb subgroups
  (``run``/``submit``/``explain``) per :class:`~nemo_helix_plugin.job.NemoJob`,
  routed through :class:`~nemo_helix_plugin.scheduler.NemoJobScheduler`.
...
``run``
    ...
    scheduler validates against :attr:`~nemo_helix_plugin.job.NemoJob.spec_schema`
    / :attr:`~nemo_helix_plugin.job.NemoJob.input_spec_schema` when declared.
...
import httpx
import typer
from nemo_helix_plugin._spec_flags import (
    UNSET,
    SpecLeafField,
...
from nemo_helix_plugin.functions.routes import DEFAULT_FUNCTION_PATH, NDJSON_MEDIA_TYPE
...
from nemo_helix_plugin.jobs.telemetry import merge_job_telemetry_custom_fields
...
def _output_format_is_json(typer_ctx: typer.Context | None) -> bool:
    ...
    the top-level :func:`nemo_helix_ext.cli.app.main` callback. Defensive
...
def add_job_commands(
    ...
    *,
    ...
    registered under :attr:`~nemo_helix_plugin.job.NemoJob.name` in a ``"Jobs"``
    ...
    Args:
        cli_app: The plugin's :class:`typer.Typer` group to inject
            ...
            :meth:`~nemo_helix_plugin.cli.NemoCLI.get_cli`.
        jobs: Mapping of entry-point key → :class:`~nemo_helix_plugin.job.NemoJob`
            subclass, already filtered to the relevant plugin. Typically a
            subset of the dict returned by
            :func:`~nemo_helix_plugin.discovery.discover_jobs`.
        cli: Optional plugin :class:`~nemo_helix_plugin.cli.NemoCLI` instance. When
            supplied, :meth:`~nemo_helix_plugin.cli.NemoCLI.update_job_cli` is
...
def _job_input_schema(job_cls: type[NemoJob]) -> type[BaseModel] | None:
    ...
    :meth:`~nemo_helix_plugin.scheduler.NemoJobScheduler._validate_and_compile`:
    :attr:`~nemo_helix_plugin.job.NemoJob.input_spec_schema` when declared,
    else :attr:`~nemo_helix_plugin.job.NemoJob.spec_schema`. Both ``run`` and
...
def _load_spec(spec_str: str, spec_file: Path | None) -> dict:
    ...
    :func:`~nemo_helix_plugin._spec_flags.deep_merge` assumes a dict and
...
def add_function_commands(
    ...
    *,
    ...
    under :attr:`~nemo_helix_plugin.function.NemoFunction.name` in a
    ...
    Args:
        ...
        functions: Mapping of entry-point key →
            :class:`~nemo_helix_plugin.function.NemoFunction` subclass,
            already filtered to the relevant plugin. Typically a
            subset of the dict returned by
            :func:`~nemo_helix_plugin.discovery.discover_functions`.
        cli: Optional plugin :class:`~nemo_helix_plugin.cli.NemoCLI` instance. When
            supplied, :meth:`~nemo_helix_plugin.cli.NemoCLI.update_function_cli` is
...
async def _invoke_function_locally(
    ...
    *,
    ...
    Mirrors :func:`~nemo_helix_plugin.run_dependencies.resolve_run_kwargs` for the SDK
...
def _api_segment_for_function(fn_cls: type[NemoFunction]) -> str:
    ...
    path. We resolve it via :func:`~nemo_helix_plugin.discovery.discover_functions`
    ...
    dashes. This mirrors :func:`nemo_helix_plugin.scheduler._api_segment_for`
    ...
    to ``my``.
    """
    from nemo_helix_plugin.discovery import discover_functions
