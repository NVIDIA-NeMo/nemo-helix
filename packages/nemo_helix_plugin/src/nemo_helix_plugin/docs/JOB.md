...
> Topics to cover there: `HelixJobSpec` shape, `HelixJobStep`, `ContainerSpec`, `ResourcesSpec`, environment variables, how `profile` and `options` flow into per-step settings, result serializers.

## Container context

When the platform runs a compiled job step in a container, it sets these env vars — read them from inside `run()` using helpers from `nhx.common.jobs.config`:

```python
from nhx.common.jobs.config import get_task_config, get_job_id, get_workspace
...
:func:`~nemo_helix_plugin.cli_state.resolve_cli_workspace`, never to a
