...
# NeMo Helix Plugin Architecture
...
1. `NHX_SERVICES` env var consulted — if set, only listed services start
...
| Header | Value |
|---|---|
| `X-NHX-Principal-Id` | `user@example.com` or `service:my-plugin` |
| `X-NHX-Principal-Email` | `user@example.com` |
| `X-NHX-Principal-Groups` | `group1,group2` |
| `X-NHX-Principal-On-Behalf-Of` | `user@example.com` (when a service acts for a user) |
