...
# Quickstart: Build Your First NeMo Helix Plugin

A plugin with an HTTP endpoint, a CLI command, a scheduled job, and typed configuration.

## Prerequisites

Python 3.12–3.14, `uv`, `export NHX_BASE_URL=http://localhost:8080` (Steps 2–5), running NeMo Helix (Step 6).
...
**No `__init__.py` files** — NeMo Helix plugins use implicit namespace packages. The hatchling config handles this automatically.
...
name = "nemo-my-plugin"
version = "0.1.0"
description = "My NeMo Helix plugin."
requires-python = ">=3.12"
dependencies = ["nemo-helix-plugin", "nemo-helix"]
...
Normal third-party plugins only need `hatchling` in `build-system.requires`. `nhx-build-tools` is a first-party build helper for NeMo Helix packages that use `[tool.bundle-package]` to bundle workspace sources into their published wheels.
...
```bash
NHX_MY_PLUGIN_DEBUG=true
NHX_MY_PLUGIN_GREETING=Howdy
```
...
- **Missing `nemo-helix`:** `get_entity_client` requires `nemo-helix` installed. Without it, entity injection at startup will fail.
