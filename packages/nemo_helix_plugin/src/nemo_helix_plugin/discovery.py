...
"""Plugin discovery — locate installed NeMo Helix plugins via entry points.
...
``nemo.services``              → :func:`discover_services`              — :class:`~nemo_helix_plugin.service.NemoService` subclass  (typed, platform instantiates)
``nemo.cli``                   → :func:`discover_cli`                   — :class:`~nemo_helix_plugin.cli.NemoCLI` subclass  (typed, platform instantiates)
``nemo.cli.agents``            → :func:`discover_agent_cli`             — :class:`~nemo_helix_plugin.cli.NemoCLI` subclass mounted at ``nemo agents <agent>``
``nemo.jobs``                  → :func:`discover_jobs`                  — :class:`~nemo_helix_plugin.job.NemoJob` subclass  (typed, platform instantiates)
``nemo.functions``             → :func:`discover_functions`             — :class:`~nemo_helix_plugin.function.NemoFunction` subclass  (typed, platform instantiates)
``nemo.controllers``           → :func:`discover_controllers`           — :class:`~nemo_helix_plugin.controller.NemoController` subclass  (typed, platform instantiates)
``nemo.sdk``                   → :func:`discover_sdk`                   — :class:`~nemo_helix_plugin.sdk.NemoPluginSDKResources` instance
...
``nemo.sandbox_profiles``      → :func:`discover_sandbox_profiles`      — :class:`~nemo_helix_plugin.sandbox.SandboxImageProfile` instance
``nemo.inference_middleware``  → :func:`discover_inference_middleware`  — :class:`~nemo_helix_plugin.inference_middleware.NemoInferenceMiddleware` subclass  (typed, IGW instantiates)
``nemo.customization.contributors`` → :func:`discover_customization_contributors` — :class:`~nemo_helix_plugin.customization_contributor.CustomizationContributor` instance  (typed, customization router instantiates)
``nemo.seed``                  → :func:`discover_seed_jobs`             — :class:`~nemo_helix_plugin.seed.NemoSeedJob` subclass  (typed, platform instantiates)

Plugin HTTP authorization is **not** a discovery surface: it is derived from
``nemo.services`` (each :class:`~nemo_helix_plugin.service.NemoService`'s
``@path_rule``-decorated routes, plus the optional ``extra_permissions`` hatch) by
:func:`~nemo_helix_plugin.authz_discovery.discover_authz_contributions`.
...
assembles a :class:`~nemo_helix_plugin.interface.PluginManifest` per plugin by
...
def discover_manifests() -> dict[str, PluginManifest]:
    ...
    any surface group becomes one :class:`~nemo_helix_plugin.interface.PluginManifest`.
...
def discover_services() -> dict[str, type[NemoService]]:
    """Typed wrapper: discover ``nemo.services`` → :class:`~nemo_helix_plugin.service.NemoService` subclass.
...
def discover_cli() -> dict[str, type[NemoCLI]]:
    """Typed wrapper: discover ``nemo.cli`` → :class:`~nemo_helix_plugin.cli.NemoCLI` subclass.

    The platform instantiates each class and calls :meth:`~nemo_helix_plugin.cli.NemoCLI.get_cli`
...
def discover_jobs() -> dict[str, type[NemoJob]]:
    """Typed wrapper: discover ``nemo.jobs`` → :class:`~nemo_helix_plugin.job.NemoJob` subclass.

    Entry-point key convention: ``<plugin-name>.<job-name>`` (e.g.
    ``"example.say-hello"``).  The platform instantiates each class and calls
    :meth:`~nemo_helix_plugin.job.NemoJob.run` with the job config dict — programmatic
    callers drive that through
    :meth:`nemo_helix_plugin.scheduler.NemoJobScheduler.run_local`.
...
def discover_functions() -> dict[str, type[NemoFunction]]:
    """Typed wrapper: discover ``nemo.functions`` → :class:`~nemo_helix_plugin.function.NemoFunction` subclass.

    Entry-point key convention: ``<plugin-name>.<function-name>`` (e.g.
    ``"example.greet"``). The platform instantiates each class and
    invokes :meth:`~nemo_helix_plugin.function.NemoFunction.run` per request
...
def discover_controllers() -> dict[str, type[NemoController]]:
    """Typed wrapper: discover ``nemo.controllers`` → :class:`~nemo_helix_plugin.controller.NemoController` subclass.

    The platform instantiates each class and wraps it in a
    ``NemoControllerAdapter`` that bridges the async :meth:`~nemo_helix_plugin.controller.NemoController.reconcile`
...
def discover_seed_jobs() -> dict[str, type[NemoSeedJob]]:
    """Typed wrapper: discover ``nemo.seed`` → :class:`~nemo_helix_plugin.seed.NemoSeedJob` subclass.

    The platform's seed task instantiates each class and calls
    :meth:`~nemo_helix_plugin.seed.NemoSeedJob.run` after injecting the SDK and
    entity client.
...
def discover_sdk() -> dict[str, NemoPluginSDKResources[Any, Any, Any, Any]]:
    """Typed wrapper: discover ``nemo.sdk`` → :class:`~nemo_helix_plugin.sdk.NemoPluginSDKResources`.
...
def discover_sandbox_profiles() -> dict[str, SandboxImageProfile]:
    ...
    :class:`~nemo_helix_plugin.sandbox.SandboxImageProfile` describing what an
...
def discover_customization_contributors() -> dict[str, CustomizationContributor]:
    ...
    :class:`~nemo_helix_plugin.customization_contributor.CustomizationContributor`
    ...
    :class:`~nemo_helix_plugin.customization_contributor.CustomizationContributorDiscoveryError`.
    """
...
def discover_inference_middleware() -> dict[str, type[NemoInferenceMiddleware]]:
    ...
    to the :class:`~nemo_helix_plugin.inference_middleware.NemoInferenceMiddleware`
