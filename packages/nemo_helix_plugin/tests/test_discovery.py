...
"""Unit tests for nemo_helix_plugin.discovery."""
...
class TestDiscoverEntryPoints:
    def test_returns_empty_when_no_entry_points(self) -> None:
        with patch("nemo_helix_plugin.discovery.entry_points", return_value=[]):
    ...
    def test_returns_entry_points_without_loading(self) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", return_value=[ep]):
            result = discover_entry_points("nemo.cli")
        assert result == {"alpha": ep}
        ep.load.assert_not_called()

    def test_passes_group_to_entry_points(self) -> None:
        with patch("nemo_helix_plugin.discovery.entry_points", return_value=[]) as mock_eps:
    ...
    def test_surface_allowlist_filters_entry_points(self, monkeypatch) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", return_value=[alpha, beta]):
            result = discover_entry_points("nemo.cli")
    ...
    def test_empty_surface_allowlist_disables_entry_points(self, monkeypatch) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", return_value=[alpha]):
    ...
    def test_allowlist_filters_dot_scoped_entry_points_by_plugin_name(self, monkeypatch) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", return_value=[alpha_job, beta_job]):
    ...
    def test_orders_entry_points_by_name_regardless_of_discovery_order(self) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", return_value=[unsloth, automodel, rl]):
            result = discover_entry_points("nemo.jobs")
...
class TestDiscover:
    ...
    def test_failing_entry_point_is_skipped(self) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", return_value=[bad, good]):
    ...
    def test_failure_does_not_affect_subsequent_entries(self) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", return_value=eps):
            result = discover("nemo.x")
    ...
    def test_failed_entry_point_logs_warning_without_traceback(self, caplog) -> None:
        ...
        with caplog.at_level("WARNING", logger="nemo_helix_plugin.discovery"):
            with patch("nemo_helix_plugin.discovery.entry_points", return_value=[bad]):
    ...
    def test_failed_entry_point_logs_traceback_at_debug(self, caplog) -> None:
        bad = _make_ep("missing-plugin", None)
        bad.load.side_effect = ModuleNotFoundError("No module named 'missing_plugin'")
        with caplog.at_level("DEBUG", logger="nemo_helix_plugin.discovery"):
            with patch("nemo_helix_plugin.discovery.entry_points", return_value=[bad]):
                discover("nemo.skills")
...
class TestDiscoverManifests:
    def test_returns_empty_when_no_entry_points(self) -> None:
        with patch("nemo_helix_plugin.discovery.entry_points", side_effect=_eps_by_group({})):
            result = discover_manifests()
        assert result == {}

    def test_scans_all_surface_groups(self) -> None:
        with patch("nemo_helix_plugin.discovery.entry_points", side_effect=_eps_by_group({})) as mock_eps:
            discover_manifests()
    ...
    def test_does_not_load_entry_point_values(self) -> None:
        ep = _make_ep("alpha", object())
        with patch("nemo_helix_plugin.discovery.entry_points", side_effect=_eps_by_group({"nemo.services": [ep]})):
            discover_manifests()
        ep.load.assert_not_called()
    ...
    def test_deduplicates_across_groups(self) -> None:
        ...
        with patch(
            "nemo_helix_plugin.discovery.entry_points",
    ...
    def test_discovers_from_any_surface_group(self) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", side_effect=_eps_by_group({"nemo.cli": [ep]})):
            result = discover_manifests()
    ...
    def test_job_only_plugins_use_plugin_name_not_job_name(self) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", side_effect=_eps_by_group({"nemo.jobs": [ep]})):
            result = discover_manifests()
    ...
    def test_function_only_plugins_use_plugin_name_not_function_name(self) -> None:
        ...
        with patch("nemo_helix_plugin.discovery.entry_points", side_effect=_eps_by_group({"nemo.functions": [ep]})):
            result = discover_manifests()
        assert list(result.keys()) == ["example"]
        assert result["example"].version == "1.2.3"
