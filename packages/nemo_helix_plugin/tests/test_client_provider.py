...
"""Tests for :mod:`nemo_helix_plugin.client_provider`.

Covers the env-var default provider and the provider/entry-point resolution
seam.  The rich platform provider (``nhx.common.client_factory``) is tested in
``packages/nhx_common/tests/client_factory``.
"""
...
from nemo_helix_plugin.client_provider import (
...
class TestBuildHeaders:
    def test_internal_flag(self):
        assert _build_headers(internal=True)["X-NHX-Internal"] == "true"

    def test_service_principal(self):
        assert _build_headers(as_service="svc")["X-NHX-Principal-Id"] == "service:svc"
    ...
    def test_principal_from_env_when_no_service(self, monkeypatch):
        ...
        assert headers["X-NHX-Principal-Id"] == "user@ex.com"
        assert headers["X-NHX-Principal-Email"] == "user@ex.com"
        assert headers["X-NHX-Principal-Groups"] == "g1,g2"
        assert headers["X-NHX-Principal-On-Behalf-Of"] == "boss@ex.com"
        assert headers["X-NHX-Principal-On-Behalf-Of-Email"] == "boss@ex.com"
    ...
    def test_service_principal_ignores_env_principal(self, monkeypatch):
        # The as_service branch does not read NHX_PRINCIPAL (matches legacy behavior).
        monkeypatch.setenv("NHX_PRINCIPAL", json.dumps({"id": "user@ex.com", "on_behalf_of": "boss@ex.com"}))
        headers = _build_headers(as_service="svc")
        assert headers["X-NHX-Principal-Id"] == "service:svc"
    ...
    def test_explicit_on_behalf_of_overrides_env_delegation(self, monkeypatch):
        ...
        assert headers["X-NHX-Principal-On-Behalf-Of"] == "override@ex.com"
        assert "X-NHX-Principal-On-Behalf-Of-Email" not in headers
        assert "X-NHX-Principal-On-Behalf-Of-Groups" not in headers
...
class TestDefaultNemoClientProvider:
    ...
    def test_sync_propagates_env_principal_on_behalf_of(self, monkeypatch):
        ...
        client = DefaultNemoClientProvider().get_nemo_client()
        assert client._default_headers["X-NHX-Principal-Id"] == "creator@ex.com"
        assert client._default_headers["X-NHX-Principal-On-Behalf-Of"] == "real@ex.com"
...
class TestForwardingHeaders:
    ...
    def test_returns_copy_of_async_client_default_headers(self) -> None:
        client = AsyncNemoClient(base_url="http://test:8000", default_headers={"X-NHX-Internal": "true"})

        headers = get_forwarding_headers(client)
        headers["X-NHX-Internal"] = "false"

        assert get_forwarding_headers(client) == {"X-NHX-Internal": "true"}
...
class TestProviderResolution:
    ...
    def test_falls_back_to_default_when_no_entry_points(self, monkeypatch):
        ...
        monkeypatch.delenv("NHX_PRINCIPAL", raising=False)
        with patch("nemo_helix_plugin.client_provider.entry_points", return_value=[]):
    ...
    def test_entry_point_provider_is_discovered(self, monkeypatch):
        ...
        with patch("nemo_helix_plugin.client_provider.entry_points", return_value=eps):
            client = get_nemo_client()
    ...
    def test_resolution_retries_after_entry_point_failure(self):
        ...
        with patch("nemo_helix_plugin.client_provider.entry_points", return_value=[ep]):
