...
import httpx
import pytest
from nemo_helix import AsyncNeMoHelix, ConflictError, NeMoHelix
from nemo_helix.resources.secrets import SecretsResource
from nemo_helix.resources.secrets.admin import AdminResource
from nemo_helix.resources.secrets.secrets import SecretsResource as ModuleSecretsResource
from nemo_helix.types.secrets import HelixSecretAccessResponse, HelixSecretResponse, SecretCreateParams
from nemo_helix.types.secrets.platform_secret_response import HelixSecretResponse as ModuleHelixSecretResponse
from nemo_helix.types.secrets.secret_create_params import SecretCreateParams as ModuleSecretCreateParams

BASE = "http://test:8000"


def test_legacy_access_response_exposes_data_alias() -> None:
    response = HelixSecretAccessResponse(name="api-key", workspace="default", value="secret")
...
def test_legacy_sdk_alias_imports_resolve() -> None:
    ...
    assert ModuleHelixSecretResponse is HelixSecretResponse
...
def test_legacy_secrets_resource_preserves_create_retrieve_list_and_delete() -> None:
    ...
    sdk = NeMoHelix(base_url=BASE, workspace="default", http_client=mock_http)
    ...
    assert isinstance(created, HelixSecretResponse)
...
async def test_async_legacy_secrets_resource_preserves_retrieve() -> None:
    ...
    async_sdk = AsyncNeMoHelix(base_url=BASE, workspace="default", http_client=mock_http)
