...
import pytest
import typer
from nemo_helix_plugin._spec_flags import kw
...
class TestWorkspaceHelpText:
    ...
    def test_env_outranks_the_configured_workspace(self) -> None:
        ...
        $NHX_WORKSPACE beats the context's configured workspace, because the
        SDK ``Config`` treats the env var as an override of the configured
        value (see ``Config.resolve``).
        """
        env_at = WORKSPACE_RESOLUTION.index("NHX_WORKSPACE")
