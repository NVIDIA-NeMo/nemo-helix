...
from nemo_helix_plugin.jobs.types import HelixJobListResultResponse
...
def test_evaluate_job_and_download_endpoint_shapes() -> None:
    ...
    assert status.response_type is HelixJobStatusResponse
    ...
    _assert_paginated_model(logs.response_type, HelixJobLog)
    assert logs.query_params == {"tail": 10}
    assert results.path_template.endswith("/evaluate/jobs/{name}/results")
    assert results.response_type is HelixJobListResultResponse
    ...
    assert result.response_type is HelixJobResultResponse
...
def test_retrieve_eval_job_models_match_job_route_contract() -> None:
    ...
    assert job.status is HelixJobStatus.COMPLETED
