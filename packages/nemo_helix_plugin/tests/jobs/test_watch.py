...
import httpx
import pytest
from nemo_helix_plugin.client.errors import NemoHTTPError, NemoResponseValidationError, NemoTransportError
...
from nemo_helix_plugin.jobs.watch import _status_event, watch_job


def test_status_event_preserves_error_details_for_failed_job() -> None:
    timestamp = datetime(2026, 1, 1, tzinfo=timezone.utc)
    status_response = HelixJobStatusResponse(
        id="job-id",
        name="job-name",
...
class _StatusOnlyClient:
    ...
    def __init__(self, results: list[HelixJobStatusResponse | Exception]) -> None:
...
def _completed() -> HelixJobStatusResponse:
    timestamp = datetime(2026, 1, 1, tzinfo=timezone.utc)
