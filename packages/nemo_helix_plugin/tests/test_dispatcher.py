...
from nemo_helix_plugin.tasks.dispatcher import (
    build_ctx_from_env,
    exit_code_for,
    read_step_config,
...
def _setup_env(
    ...
    *,
    ...
    def _fake_platform_job_results(**_kwargs: object) -> MagicMock:
        return MagicMock(name="HelixJobResults")
...
class TestBuildCtxFromEnv:
    ...
    def _patch_results(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
        sentinel = MagicMock(name="HelixJobResults")
    ...
    def test_usage_default_is_platform_reporter(self, tmp_path: Path, monkeypatch) -> None:
        self._patch_results(monkeypatch)
        usage = MagicMock(name="HelixJobUsageReporter")
        capture = MagicMock(return_value=usage)
        monkeypatch.setattr(dispatcher_module, "HelixJobUsageReporter", capture)
        monkeypatch.setenv("NEMO_JOB_WORKSPACE", "ws")
        monkeypatch.setenv("NEMO_JOB_PERSISTENT_JOB_STORAGE_PATH", str(tmp_path / "p"))
        monkeypatch.setenv("NEMO_JOB_EPHEMERAL_TASK_STORAGE_PATH", str(tmp_path / "e"))
        monkeypatch.setenv("NEMO_JOB_ID", "submitted-job-name")
