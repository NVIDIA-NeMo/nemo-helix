...
"""Tests for nemo_helix_plugin.config — NemoConfig base class and re-exports."""
...
class _WidgetConfig(NemoConfig):
    ...
    plugin_name: ClassVar[str] = "test_nemo_helix_plugin_widget"
...
class _GadgetConfig(NemoConfig):
    ...
    plugin_name: ClassVar[str] = "test_nemo_helix_plugin_gadget"
...
def test_env_prefix_derived_from_plugin_name() -> None:
    ...
    assert _WidgetConfig.model_config.get("env_prefix") == "NEMO_TEST_NEMO_HELIX_PLUGIN_WIDGET_"
...
def test_global_settings_key_returns_plugin_name() -> None:
    ...
    assert _WidgetConfig.global_settings_key() == "test_nemo_helix_plugin_widget"
    assert _GadgetConfig.global_settings_key() == "test_nemo_helix_plugin_gadget"
...
def test_platform_config_is_importable() -> None:
    """HelixConfig is importable from nemo_helix_plugin.config."""
    assert HelixConfig is not None
    assert hasattr(HelixConfig, "model_fields")
    assert "base_url" in HelixConfig.model_fields


def test_nemo_helix_config_alias() -> None:
    """NemoHelixConfig is an alias for HelixConfig."""
    assert NemoHelixConfig is HelixConfig


def test_get_platform_config_is_callable() -> None:
    """get_platform_config() is callable and returns a HelixConfig."""
    result = get_platform_config()
    assert isinstance(result, HelixConfig)
...
def test_register_platform_config_class_controls_platform_config_accessors() -> None:
    ...
    class _RegisteredHelixConfig(NemoHelixConfig):
        registered_marker: str = "registered"

    previous_platform_config_class = get_platform_config_class()
    register_platform_config_class(_RegisteredHelixConfig)
    try:
        assert isinstance(Configuration.get_platform_config(), _RegisteredHelixConfig)
        assert isinstance(get_platform_config(), _RegisteredHelixConfig)
        assert isinstance(NemoHelixConfig.get(), _RegisteredHelixConfig)
        assert isinstance(_RegisteredHelixConfig.get(), _RegisteredHelixConfig)
    finally:
        register_platform_config_class(previous_platform_config_class)


def test_get_nemo_helix_config_alias() -> None:
    """get_nemo_helix_config is an alias for get_platform_config."""
...
def test_validate_docker_available_returns_false_on_connection_failures() -> None:
    ...
    from nemo_helix_plugin.config import validate_docker_available
...
def test_default_client_config_fast_model_falls_back_to_env_default_model(
    ...
    from nemo_helix_plugin.client.config.models import ConfigFile
