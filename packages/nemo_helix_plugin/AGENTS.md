...
# nemo-helix-plugin — Agent Instructions

`nemo-helix-plugin` is the **only package plugin authors install** to build a fully-featured NeMo Helix plugin. It re-exports all abstract base classes, configuration, entity, schema, and CLI utilities needed to contribute services, CLI commands, jobs, controllers, and more to the NeMo Helix.

## Key Concepts

- **One install, everything included**: `nemo-helix-plugin` depends on `nhx-common` (the platform library) transitively. Plugin authors also need `nemo-helix` for `get_entity_client` and SDK features.
...
- **Entities for persistent state**: Use `NemoEntity` + `NemoEntitiesClient` to store plugin data in the NeMo Helix entity store. Entity types are global — use plugin-scoped names (`"my_plugin_widget"` not `"widget"`).
