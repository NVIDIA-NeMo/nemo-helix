...
class NhxDynamicVersionSource(VersionSourceInterface):
    PLUGIN_NAME = "nhx-dynamic-versioning"

    def get_version_data(self) -> dict[str, str]:
        config = nhx_dynamic_versioning_config(self.config)
        version, _ = get_version(config)
        return {"version": version}


def nhx_dynamic_versioning_config(overrides: dict[str, Any] | None = None) -> schemas.UvDynamicVersioning:
...
@hookimpl
def hatch_register_version_source() -> type[NhxDynamicVersionSource]:
    return NhxDynamicVersionSource
