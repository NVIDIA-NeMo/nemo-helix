...
from nhx_build_tools.hatch import (
    ...
    _rewrite_metadata,
    nhx_dynamic_versioning_config,
    read_bundle_force_include,
    read_bundle_shared_data,
)


def test_nhx_dynamic_versioning_config_uses_repo_defaults() -> None:
    config = nhx_dynamic_versioning_config()
...
def test_nhx_dynamic_versioning_config_allows_hatch_version_overrides() -> None:
    config = nhx_dynamic_versioning_config(
        {
            "source": "nhx-dynamic-versioning",
...
def test_nhx_dynamic_version_source_honors_bypass(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("UV_DYNAMIC_VERSIONING_BYPASS", "1.2.3")

    version_source = NhxDynamicVersionSource(
        str(tmp_path),
        {"source": "nhx-dynamic-versioning"},
    )
...
"""
    ...
    assert 'Requires-Dist: nemo-helix[data-designer-nemo] ; extra == "nemo-data-designer-plugin"' in rewritten
    assert 'Requires-Dist: nemo-helix[switchyard] ; extra == "nemo-switchyard"' in rewritten
