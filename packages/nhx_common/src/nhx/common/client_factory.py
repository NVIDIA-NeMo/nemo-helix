...
- per-service URL routing via :class:`~nhx.common.platform_endpoint.PlatformEndpoint`;
...
def _platform_headers(
    ...
    for name, value in get_otel_headers().items():
        ...
        if normalized_name == "x-nhx-internal" or normalized_name.startswith("x-nhx-principal-"):
            continue
        headers[name] = value
    return headers
