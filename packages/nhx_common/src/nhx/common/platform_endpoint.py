...
from nhx.common.immutable_http_client import (
...
UDS_BASE_URL = "http://nemo-helix.local"
...
def _empty_service_endpoints() -> Mapping[str, "HelixEndpoint"]:
...
class HelixEndpoint:
    ...
    service_endpoints: Mapping[str, "HelixEndpoint"] = field(
    ...
    def route_request_url(self, url: str | httpx.URL) -> "RoutedHelixEndpointRequest":
        ...
        return RoutedHelixEndpointRequest(url=routed_url, endpoint=endpoint)


@dataclass(frozen=True)
class RoutedHelixEndpointRequest:
    url: httpx.URL
...
def resolve_service_endpoint(service_name: str, platform_config: HelixConfig | None = None) -> HelixEndpoint:
    """Resolve a service endpoint using ``NHX_<SERVICE>_URL`` before ``NHX_BASE_URL``."""
...
def parse_platform_endpoint(endpoint: str) -> HelixEndpoint:
    ...
    if endpoint.startswith(("http://", "https://")):
        ...
        return HelixEndpoint(connect_base_url=endpoint.rstrip("/"), socket_path=None, transport="tcp")
    if endpoint.startswith("unix://"):
        socket_path = _parse_unix_socket_path(endpoint)
        return HelixEndpoint(connect_base_url=UDS_BASE_URL, socket_path=socket_path, transport="uds")
...
def _service_route_names(platform_config: HelixConfig, env: Mapping[str, str]) -> set[str]:
    names: set[str] = set()
...
def _service_route_names_from_env(env: Mapping[str, str]) -> set[str]:
    ...
    for env_name, endpoint in env.items():
        if not env_name.startswith("NHX_") or not env_name.endswith("_URL"):
            continue
        raw_service_name = env_name.removeprefix("NHX_").removesuffix("_URL")
...
def _service_url_env_var_name(service_name: str) -> str:
    return f"NHX_{_normalize_service_name(service_name).upper().replace('-', '_')}_URL"
...
def _parse_optional_platform_endpoint(endpoint: str | None) -> HelixEndpoint | None:
    if not endpoint:
        return None
    try:
        return parse_platform_endpoint(endpoint)
...
def _url_for_endpoint(url: httpx.URL, endpoint: HelixEndpoint) -> httpx.URL:
    if endpoint.transport == "uds":
...
def _uds_socket_paths(endpoint: HelixEndpoint) -> frozenset[Path]:
    paths: set[Path] = set()
...
class _SyncHelixEndpointRoutingTransport(httpx.BaseTransport):
    ...
    def _transport_for_endpoint(self, endpoint: HelixEndpoint) -> httpx.HTTPTransport:
...
class _SyncExplicitClientRoutingTransport(httpx.BaseTransport):
    def __init__(self, *, endpoint: HelixEndpoint, http_client: httpx.Client) -> None:
        self._endpoint = endpoint
...
class _AsyncHelixEndpointRoutingTransport(httpx.AsyncBaseTransport):
    ...
    def _transport_for_endpoint(self, endpoint: HelixEndpoint) -> httpx.AsyncHTTPTransport:
        if endpoint.transport == "tcp":
            return self._tcp_transport
        if endpoint.socket_path is None:
            raise ValueError("UDS endpoint is missing a socket path")
...
class _AsyncExplicitClientRoutingTransport(httpx.AsyncBaseTransport):
    def __init__(self, *, endpoint: HelixEndpoint, http_client: httpx.AsyncClient) -> None:
        self._endpoint = endpoint
        self._http_client = http_client
