...
from nhx.common.platform_endpoint import PlatformEndpoint, resolve_platform_endpoint
...
def get_sdk_on_behalf_of(
    ...
    if isinstance(on_behalf_of, Principal):
        merged_headers["X-NHX-Principal-On-Behalf-Of"] = on_behalf_of.effective_principal.id
        if on_behalf_of.effective_principal.email:
            merged_headers["X-NHX-Principal-On-Behalf-Of-Email"] = on_behalf_of.effective_principal.email
        if on_behalf_of.effective_principal.groups:
            merged_headers["X-NHX-Principal-On-Behalf-Of-Groups"] = ",".join(on_behalf_of.effective_principal.groups)
    else:
