...
from nhx.common.config import AuthConfig, get_auth_config
...
def _embedded_pdp_base_url_hint(config: AuthConfig) -> str:
    ...
    return (
        ...
        "this process serves /apis/auth (same as platform base_url / NHX_BASE_URL; HTTP(S) or unix://). "
...
def _trusted_header_identity_descriptor(principal: Principal) -> dict[str, Any] | None:
    ...
    return {
        ...
        "subject_claim": "x-nhx-principal-id",
        "account_type": "user",
...
class AuthorizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        # X-NHX-Principal-* headers (includes service principals — evaluated by PDP)
        if headers_dict.get("x-nhx-principal-id"):
    ...
    async def _handle_hf_compatible_request(
        ...
        if bearer_token is not None and is_service_principal(bearer_token):
            headers_dict["x-nhx-principal-id"] = bearer_token
            return await self._handle_principal_headers_request(request, call_next, headers_dict)
    ...
    async def _handle_service_principal_request(
        ...
        logger.debug(
            ...
            request.method,
            request.url.path,
            headers_dict.get("x-nhx-principal-id"),
        )
    ...
    async def _handle_principal_headers_request(
        ...
        """Handle requests that include X-NHX-Principal-* headers.
    ...
    async def _handle_auth_check(
        ...
        scopes_header = headers_dict.get("x-nhx-scopes", "")
