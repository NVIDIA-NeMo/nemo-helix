...
import pytest
from nhx.common.auth.exceptions import InvalidPrincipalHeader
from nhx.common.auth.models import (
    MAX_EMAIL_LENGTH,
    MAX_GROUP_LENGTH,
    MAX_GROUPS_COUNT,
...
class TestPrincipalFromHeaders:
    ...
    def test_from_nhx_headers_basic(self):
        """Test creating Principal from basic NeMo Helix headers."""
        headers = {
    ...
    def test_from_nhx_headers_full(self):
        """Test creating Principal from full NeMo Helix headers."""
        headers = {
            "x-nhx-principal-id": "user@example.com",
            "x-nhx-principal-email": "user@example.com",
            "x-nhx-principal-groups": "admin, users, developers",
            "x-nhx-principal-on-behalf-of": "other@example.com",
        }
    ...
    def test_from_headers_account_context_roundtrip(self):
        headers = {
            "x-nhx-principal-id": "service:worker",
            "x-nhx-actor-account-id": "account-service",
            "x-nhx-actor-aliases": "service:worker,service:worker,legacy-worker",
            "x-nhx-principal-on-behalf-of": "user@example.com",
            "x-nhx-subject-account-id": "account-user",
            "x-nhx-subject-aliases": "legacy-user,user@example.com",
        }
    ...
    def test_from_headers_ignores_caller_kind_header(self):
        headers = {
            "x-nhx-principal-id": "user@example.com",
            "x-nhx-caller-kind": "service_principal",
        }
        ...
        assert "X-NHX-Caller-Kind" not in principal.get_headers()
    ...
    def test_from_headers_rejects_malformed_service_principal_id(self, principal_id: str):
        with pytest.raises(InvalidPrincipalHeader):
            Principal.from_headers({"x-nhx-principal-id": principal_id})
    ...
    def test_from_headers_rejects_empty_service_account_principal_id(self):
        with pytest.raises(InvalidPrincipalHeader, match="malformed"):
            Principal.from_headers({"x-nhx-principal-id": "service-account:"})
    ...
    def test_from_headers_rejects_malformed_service_principal_group(self):
        with pytest.raises(InvalidPrincipalHeader, match="malformed"):
            Principal.from_headers(
                {
                    "x-nhx-principal-id": "user@example.com",
                    "x-nhx-principal-groups": "service:",
                }
            )
    ...
    def test_from_nhx_headers_with_on_behalf_of_groups_and_email(self):
        """On-behalf-of groups and email are parsed when the on-behalf-of header is present."""
        headers = {
            "x-nhx-principal-id": "service:evaluator",
            "x-nhx-principal-on-behalf-of": "user@example.com",
            "x-nhx-principal-on-behalf-of-groups": "team-a, team-b",
            "x-nhx-principal-on-behalf-of-email": "user@example.com",
        }
        principal = Principal.from_headers(headers)
        assert principal is not None
    ...
    def test_from_nhx_headers_empty_groups(self):
    ...
    def test_from_nhx_headers_strips_whitespace(self):
...
class TestPrincipalGetHeaders:
    ...
    def test_get_headers_full(self):
        ...
        assert headers == {
            "X-NHX-Principal-Id": "user@example.com",
            "X-NHX-Principal-Email": "user@example.com",
            "X-NHX-Principal-Groups": "admin,users",
            "X-NHX-Principal-On-Behalf-Of": "other@example.com",
        }
    ...
    def test_get_headers_includes_account_context(self):
        ...
        assert headers["X-NHX-Actor-Account-Id"] == "account-123"
        assert headers["X-NHX-Actor-Aliases"] == "service:worker"
        assert headers["X-NHX-Subject-Account-Id"] == "account-456"
        assert headers["X-NHX-Subject-Aliases"] == "legacy-user"
    ...
    def test_get_headers_empty_groups(self):
        ...
        assert "X-NHX-Principal-Groups" not in headers
...
class TestPrincipalFromEnvVar:
    ...
    def test_from_env_var_success(self):
        ...
        os.environ[NHX_PRINCIPAL_ENVVAR] = '{"id": "user@example.com", "email": "user@example.com"}'
    ...
    def test_from_env_var_invalid_json(self):
        ...
        os.environ[NHX_PRINCIPAL_ENVVAR] = "not valid json"
    ...
    def test_from_env_var_empty_id(self):
        ...
        os.environ[NHX_PRINCIPAL_ENVVAR] = '{"id": "", "email": "user@example.com"}'
    ...
    def test_from_env_var_includes_on_behalf_of_claims(self):
        ...
        os.environ[NHX_PRINCIPAL_ENVVAR] = json.dumps(payload)
        try:
            principal = Principal.from_env_var()
            assert principal is not None
...
class TestPrincipalGetOtlpHeadersValue:
    ...
    def test_otlp_headers_basic(self):
        ...
        assert value == "X-NHX-Principal-Id=user%40example.com"
    ...
    def test_otlp_headers_full(self):
        ...
        assert "X-NHX-Principal-Id=user%40example.com" in value
        assert "X-NHX-Principal-Email=user%40example.com" in value
        assert "X-NHX-Principal-Groups=admin%2Cusers" in value
        assert "X-NHX-Principal-On-Behalf-Of=other%40example.com" in value
    ...
    def test_otlp_headers_includes_on_behalf_of_groups_and_email(self):
        ...
        value = principal.get_otlp_headers_value()
        assert "X-NHX-Principal-On-Behalf-Of-Groups=a%2Cb" in value
        assert "X-NHX-Principal-On-Behalf-Of-Email=user%40example.com" in value
...
class TestPrincipalHeaderValidation:
    ...
    def test_valid_service_principal_id(self):
        headers = {"x-nhx-principal-id": "service:my-service"}
        principal = Principal.from_headers(headers)
        assert principal is not None
        assert principal.id == "service:my-service"

    def test_valid_oidc_subject_principal_id(self):
        headers = {"x-nhx-principal-id": "auth0/abc123-def456"}
        principal = Principal.from_headers(headers)
        assert principal is not None
    ...
    def test_principal_id_at_max_length(self):
        ...
        headers = {"x-nhx-principal-id": value}
        principal = Principal.from_headers(headers)
        assert principal is not None
    ...
    def test_principal_id_spaces(self):
        headers = {"x-nhx-principal-id": "user name"}
        with pytest.raises(InvalidPrincipalHeader, match="invalid characters"):
            Principal.from_headers(headers)

    def test_principal_id_sql_injection(self):
        headers = {"x-nhx-principal-id": "'; DROP TABLE users;--"}
        with pytest.raises(InvalidPrincipalHeader, match="invalid characters"):
            Principal.from_headers(headers)

    def test_principal_id_unicode(self):
        headers = {"x-nhx-principal-id": "user\u00e9@example.com"}
        with pytest.raises(InvalidPrincipalHeader, match="invalid characters"):
            Principal.from_headers(headers)
    ...
    def test_email_plus_addressing(self):
        headers = {
            "x-nhx-principal-id": "user@example.com",
            "x-nhx-principal-email": "user+tag@example.com",
        }
        principal = Principal.from_headers(headers)
        assert principal is not None
    ...
    def test_email_exceeds_max_length(self):
        ...
        headers = {
            "x-nhx-principal-id": "user@example.com",
            "x-nhx-principal-email": long_email,
        }
        with pytest.raises(InvalidPrincipalHeader, match="exceeds maximum length"):
            Principal.from_headers(headers)
    ...
    def test_email_missing_at(self):
        headers = {
            "x-nhx-principal-id": "user@example.com",
            "x-nhx-principal-email": "not-an-email",
        }
    ...
    def test_email_empty_local_part(self):
        headers = {
            "x-nhx-principal-id": "user@example.com",
            "x-nhx-principal-email": "@example.com",
        }
        with pytest.raises(InvalidPrincipalHeader, match="not a valid email"):
            Principal.from_headers(headers)
    ...
    def test_group_exceeds_max_length(self):
        ...
        headers = {
            "x-nhx-principal-id": "user@example.com",
            "x-nhx-principal-groups": long_group,
        }
    ...
    def test_group_with_invalid_characters(self):
        headers = {
            "x-nhx-principal-id": "user@example.com",
            "x-nhx-principal-groups": "valid-group,bad group!",
        }
        with pytest.raises(InvalidPrincipalHeader, match="invalid characters"):
            Principal.from_headers(headers)
    ...
    def test_group_with_control_characters(self):
        headers = {
            "x-nhx-principal-id": "user@example.com",
            "x-nhx-principal-groups": "admin,group\x00name",
        }
        with pytest.raises(InvalidPrincipalHeader, match="invalid characters"):
            Principal.from_headers(headers)
    ...
    def test_on_behalf_of_exceeds_max_length(self):
        ...
        headers = {
            "x-nhx-principal-id": "admin@example.com",
            "x-nhx-principal-on-behalf-of": value,
        }
        with pytest.raises(InvalidPrincipalHeader, match="exceeds maximum length"):
            Principal.from_headers(headers)
    ...
    def test_on_behalf_of_invalid_characters(self):
        headers = {
            "x-nhx-principal-id": "admin@example.com",
            "x-nhx-principal-on-behalf-of": "user; DROP TABLE",
        }
        with pytest.raises(InvalidPrincipalHeader, match="invalid characters"):
            Principal.from_headers(headers)
    ...
    def test_all_headers_valid(self):
        headers = {
            ...
            "x-nhx-principal-email": "user@example.com",
            "x-nhx-principal-groups": "admin,users",
            "x-nhx-principal-on-behalf-of": "service:backend",
        }
        principal = Principal.from_headers(headers)
        assert principal is not None
        assert principal.id == "user@example.com"
    ...
    def test_invalid_principal_id_short_circuits(self):
        ...
        headers = {
            "x-nhx-principal-id": "bad\x00id",
            "x-nhx-principal-email": "also-invalid",
        }
        with pytest.raises(InvalidPrincipalHeader, match="X-NHX-Principal-Id"):
            Principal.from_headers(headers)
