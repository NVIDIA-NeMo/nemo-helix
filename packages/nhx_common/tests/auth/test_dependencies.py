...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv(NHX_PRINCIPAL_ENVVAR, raising=False)
...
def test_principal_from_env_parses_json(clean_env, monkeypatch):
    ...
    assert headers["X-NHX-Principal-Id"] == "job-creator@example.com"
    assert headers["X-NHX-Principal-Email"] == "job-creator@example.com"
    assert headers["X-NHX-Principal-Groups"] == "engineering,platform-team"
...
def test_principal_from_env_handles_id_only(clean_env, monkeypatch):
    ...
    assert headers["X-NHX-Principal-Id"] == "service-account"
    assert "X-NHX-Principal-Email" not in headers
...
def test_principal_from_env_does_not_modify_context(clean_env, monkeypatch):
    ...
    assert principal is not None
    headers = principal.get_headers()
    assert headers["X-NHX-Principal-Id"] == "test-user"
...
def test_principal_from_env_handles_empty_groups(clean_env, monkeypatch):
    ...
    assert headers["X-NHX-Principal-Id"] == "user"
    assert "X-NHX-Principal-Groups" not in headers


def test_principal_from_env_invalid_json_raises(clean_env, monkeypatch):
    """Test that invalid JSON raises ValueError."""
    monkeypatch.setenv(NHX_PRINCIPAL_ENVVAR, "not-valid-json")
...
class TestPrincipalOtlpHeaders:
    ...
    def test_otlp_headers_with_all_fields(self):
        ...
        assert "X-NHX-Principal-Id=user%40example.com" in headers
        assert "X-NHX-Principal-Email=user%40example.com" in headers
        assert "X-NHX-Principal-Groups=team-a%2Cteam-b" in headers
...
class TestBuildServicePrincipalHeadersDelegation:
    ...
    def test_propagates_effective_user_claims(self):
        ...
        try:
            ...
            assert h["X-NHX-Principal-Id"] == "service:guardrails"
            assert h["X-NHX-Actor-Aliases"] == "service:guardrails"
            assert h["X-NHX-Principal-On-Behalf-Of"] == "user@example.com"
            assert h["X-NHX-Principal-On-Behalf-Of-Email"] == "user@example.com"
            assert h["X-NHX-Principal-On-Behalf-Of-Groups"] == "team-a,team-b"
        finally:
            auth_client_context.reset(token)
    ...
    def test_no_on_behalf_headers_when_only_service_context(self):
        ...
        try:
            ...
            assert h == {
                ...
                "X-NHX-Actor-Aliases": "service:jobs",
            }
        finally:
            auth_client_context.reset(token)
    ...
    def test_id_only_when_no_auth_context(self):
        ...
        assert build_service_principal_headers("x") == {
            "X-NHX-Principal-Id": "service:x",
            "X-NHX-Actor-Aliases": "service:x",
        }
    ...
    def test_propagates_effective_account_context(self):
        ...
        try:
            h = build_service_principal_headers("guardrails")
            assert h["X-NHX-Principal-On-Behalf-Of"] == "user@example.com"
            assert h["X-NHX-Subject-Account-Id"] == "account-user"
            assert h["X-NHX-Subject-Aliases"] == "user@example.com,legacy-user"
        finally:
            auth_client_context.reset(token)
...
def test_principal_from_env_serializes_on_behalf_of_headers(clean_env, monkeypatch):
    ...
    from nhx.common.auth import principal_from_env

    principal = principal_from_env()
    assert principal is not None
    headers = principal.get_headers()
    assert headers["X-NHX-Principal-On-Behalf-Of-Groups"] == "ml-team"
    assert headers["X-NHX-Principal-On-Behalf-Of-Email"] == "creator@example.com"
