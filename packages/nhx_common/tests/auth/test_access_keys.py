...
import httpx
import jwt
import nhx.common.auth.access_keys as access_keys_mod
...
from nemo_helix_plugin.auth.access_keys.types import AccessKeyCreateRequest
from nhx.common import http_clients
...
def test_token_signing_defaults_are_shared_and_access_keys_are_disabled():
    ...
    assert config.token_signing.key_id == "nemo-helix-signing"
    ...
    assert config.access_keys.audience == "nemo-helix-access-key"
...
def test_access_key_candidate_requires_token_type_and_canonical_jti(
    ...
    payload = {"nhx_token_type": token_type}
    if jti is not None:
        payload["jti"] = jti
...
def test_token_signing_private_key_file_uses_auth_service_env_override(monkeypatch):
    monkeypatch.setenv(
        "NHX_AUTH_TOKEN_SIGNING__PRIVATE_KEY_FILE",
        "/var/run/secrets/nemo-helix/token-signing/private.pem",
    )

    config = AuthConfig()

    assert config.token_signing.private_key_file == "/var/run/secrets/nemo-helix/token-signing/private.pem"
...
def test_nested_auth_env_vars_use_double_underscore_delimiter(monkeypatch):
    monkeypatch.setenv("NHX_AUTH_TOKEN_SIGNING__KEY_ID", "custom-signing-key")
    monkeypatch.setenv("NHX_AUTH_ACCESS_KEYS__ENABLED", "true")
...
def test_access_key_accepted_formats_uses_auth_service_env_override(monkeypatch, env_value, expected):
    monkeypatch.setenv("NHX_AUTH_ACCESS_KEYS__ACCEPTED_FORMATS", env_value)
...
def test_access_key_max_expires_in_seconds_uses_auth_service_env_override(monkeypatch, env_value, expected):
    monkeypatch.setenv("NHX_AUTH_ACCESS_KEYS__MAX_EXPIRES_IN_SECONDS", env_value)
    if expected is not None and expected < 30 * 24 * 60 * 60:
        monkeypatch.setenv("NHX_AUTH_ACCESS_KEYS__DEFAULT_EXPIRES_IN_SECONDS", env_value)
...
def test_access_key_default_expires_in_seconds_uses_auth_service_env_override(monkeypatch, env_value, expected):
    monkeypatch.setenv("NHX_AUTH_ACCESS_KEYS__DEFAULT_EXPIRES_IN_SECONDS", env_value)
    if expected is None:
        monkeypatch.setenv("NHX_AUTH_ACCESS_KEYS__MAX_EXPIRES_IN_SECONDS", "none")
...
def test_access_key_max_expires_in_seconds_env_override_rejects_zero(monkeypatch):
    monkeypatch.setenv("NHX_AUTH_ACCESS_KEYS__MAX_EXPIRES_IN_SECONDS", "0")
    ...
    *,
    ...
    return AuthConfig(
        ...
        token_signing=TokenSigningConfig(
            issuer="https://nhx.example.test/apis/auth",
...
def test_access_key_issuer_service_stamps_current_principal(tmp_path):
    ...
    assert unverified["aud"] == "nemo-helix-access-key"
    assert unverified["nhx_token_type"] == ACCESS_KEY_TOKEN_TYPE
    assert unverified["nhx_access_key"] == {
        "version": 2,
        "name": "ci-intake",
    }
...
def test_access_key_issuer_service_allows_unnamed_tokens(tmp_path):
    ...
    assert unverified["jti"] == created.jti
    assert unverified["nhx_access_key"] == {"version": 2}
    assert unverified["exp"] == 1785280600
...
async def test_service_account_cannot_create_a_service_bound_access_key_even_with_allow_flag(tmp_path):
    ...
    # was somehow granted HelixAdmin; the identity check must still block self/peer renewal.
    with pytest.raises(AccessKeyValidationError, match="service-account principals"):
...
async def test_service_account_access_key_requires_explicit_admin_authorization(tmp_path):
    ...
    with pytest.raises(AccessKeyValidationError, match="HelixAdmin"):
...
async def test_validate_access_key_token_rejects_service_entity_type_for_user_subject(tmp_path):
    ...
    claims = jwt.decode(created.token, options={"verify_signature": False})
    claims["nhx_access_key"]["entity_type"] = "SERVICE_ACCOUNT"
    signing_key = await access_keys_mod._access_key_signing_key_async(config)
    token = jwt.encode(
        claims,
        signing_key.private_key,
...
async def test_validate_access_key_token_rejects_service_subject_without_service_entity_type(tmp_path):
    ...
    claims = jwt.decode(created.token, options={"verify_signature": False})
    claims["nhx_access_key"].pop("entity_type")
    signing_key = await access_keys_mod._access_key_signing_key_async(config)
    token = jwt.encode(
        claims,
        signing_key.private_key,
