...
import httpx
import pytest
from nhx.common.config import Configuration, HelixConfig, get_common_service_config
...
def clear_platform_url_env_vars(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    for key in tuple(os.environ):
        if key.startswith("NHX_") and key.endswith("_URL"):
...
def test_parse_uds_endpoint() -> None:
    endpoint = parse_platform_endpoint("unix:///tmp/nemo-helix.sock")

    assert endpoint.transport == "uds"
    assert endpoint.connect_base_url == UDS_BASE_URL
    assert endpoint.socket_path == Path("/tmp/nemo-helix.sock")


def test_parse_rejects_raw_socket_path() -> None:
    with pytest.raises(ValueError, match="use unix:///tmp/nemo-helix.sock"):
        parse_platform_endpoint("/tmp/nemo-helix.sock")
...
def test_resolve_service_endpoint_falls_back_to_base_url(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("NHX_SECRETS_URL", raising=False)
    config = HelixConfig(base_url="http://platform:8080")
...
def test_resolve_service_endpoint_skips_invalid_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    config = HelixConfig(base_url="http://platform:8080")
    monkeypatch.setenv("NHX_SECRETS_URL", "not-a-url")
...
def test_endpoint_env_family_is_not_part_of_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("NHX_SECRETS_ENDPOINT", "unix:///tmp/secrets.sock")
    monkeypatch.delenv("NHX_SECRETS_URL", raising=False)
    config = HelixConfig(base_url="http://platform:8080")
...
@pytest.fixture
def platform_config_with_service_discovery() -> HelixConfig:
...
def test_platform_endpoint_uses_env_override(
    ...
    monkeypatch.setenv("NHX_ENTITIES_URL", "http://entities-env:9090")
    endpoint = resolve_platform_endpoint(platform_config_with_service_discovery)
...
def test_platform_endpoint_skips_invalid_env_only_service_url(
    ...
    monkeypatch.setenv("NHX_FILES_URL", service_url)
    config = HelixConfig(base_url="http://platform:8080")
...
def test_platform_endpoint_skips_invalid_unrelated_service_url_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("NHX_NOT_A_SERVICE_URL", "not-a-url")
    config = HelixConfig(base_url="http://platform:8080")
...
def test_platform_endpoint_keeps_configured_local_service_on_local_url(monkeypatch: pytest.MonkeyPatch) -> None:
    config = HelixConfig(base_url="http://platform:8080", services="hello-world")  # type: ignore[abstract]
    monkeypatch.setenv("NHX_HELLO_WORLD_URL", "http://hello-world-service:8080")
    endpoint = resolve_platform_endpoint(config)
...
def test_platform_endpoint_keeps_local_service_with_underscore_configured_name(
    ...
    config = HelixConfig(base_url="http://platform:8080", services="my_service")
...
def test_platform_endpoint_keeps_local_service_when_env_override_exists(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("NHX_SERVICE_HOST", "127.0.0.1")
    monkeypatch.setenv("NHX_SERVICE_PORT", "8080")
    monkeypatch.setenv("NHX_HELLO_WORLD_URL", "http://hello-world-env:9090")
...
def test_platform_endpoint_ignores_unrelated_service_url_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
    config = HelixConfig(base_url="http://platform:8080")  # type: ignore[abstract]
    monkeypatch.setenv("NHX_NOT_A_SERVICE_URL", "http://not-a-service:8080")
...
def test_platform_endpoint_routes_uds_service() -> None:
    ...
    assert str(routed.url) == "http://nemo-helix.local/apis/entities/v2/workspaces"
...
def test_platform_endpoint_logs_path_without_raw_url(
    ...
    caplog.set_level(logging.DEBUG, logger="nhx.common.platform_endpoint")
    endpoint = resolve_platform_endpoint(platform_config_with_service_discovery)
...
def test_uds_sync_sdk_http_client_keeps_transport_and_omits_unset_timeout() -> None:
    ...
    with (
        patch("nhx.common.platform_endpoint.ImmutableDefaultHttpxClient") as sdk_client,
        patch("nhx.common.platform_endpoint.ImmutableHttpxClient") as client,
    ):
        endpoint.sync_sdk_http_client()
...
def test_uds_sync_sdk_http_client_can_disable_redirects() -> None:
    ...
    with patch("nhx.common.platform_endpoint.ImmutableHttpxClient") as client:
        endpoint.sync_sdk_http_client(follow_redirects=False)
...
def test_sync_routing_transport_prebuilds_uds_service_transports() -> None:
    ...
    with patch("nhx.common.platform_endpoint.httpx.HTTPTransport", FakeHTTPTransport):
        routing_transport = _SyncHelixEndpointRoutingTransport(
...
def test_sync_routing_transport_routes_api_path_to_service_url(
    ...
    endpoint = resolve_platform_endpoint(platform_config_with_service_discovery)
    with patch("nhx.common.platform_endpoint.httpx.HTTPTransport", FakeHTTPTransport):
        routing_transport = _SyncHelixEndpointRoutingTransport(endpoint=endpoint)
...
async def test_async_routing_transport_routes_api_path_to_service_url(
    ...
    endpoint = resolve_platform_endpoint(platform_config_with_service_discovery)
    with patch("nhx.common.platform_endpoint.httpx.AsyncHTTPTransport", FakeAsyncHTTPTransport):
        routing_transport = _AsyncHelixEndpointRoutingTransport(endpoint=endpoint)
...
def test_routing_transports_pass_custom_ca_to_tcp_transport(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    endpoint = resolve_platform_endpoint(
        HelixConfig(  # type: ignore[abstract]
...
def test_uds_async_sdk_http_client_keeps_transport_and_omits_unset_timeout() -> None:
    ...
    with (
        patch("nhx.common.platform_endpoint.ImmutableDefaultAsyncHttpxClient") as sdk_client,
        patch("nhx.common.platform_endpoint.ImmutableAsyncHttpxClient") as client,
    ):
        endpoint.async_sdk_http_client()
...
def test_uds_async_sdk_http_client_can_disable_redirects() -> None:
    ...
    with patch("nhx.common.platform_endpoint.ImmutableAsyncHttpxClient") as client:
        endpoint.async_sdk_http_client(follow_redirects=False)
