...
class MockService(Service):
    ...
    def __init__(self, dependency_provider: DependencyProvider | None = None):
        super().__init__(name="test-service", module_name="nhx.test", dependency_provider=dependency_provider)
...
class NestedRouterService(Service):
    ...
    def __init__(self):
        super().__init__(name="nested-router", module_name="nhx.test")
...
class TestServiceBase:
    ...
    def test_service_init(self):
        ...
        assert service.module_name == "nhx.test"
    ...
    def test_service_openapi_schema_is_generated_lazily(self, monkeypatch: pytest.MonkeyPatch):
        import nhx.common.service.base as service_base
...
class TestDependencyProvider:
    ...
    def test_request_scoped_nemo_clients_are_distinct_and_share_transport(self):
        ...
        assert first._default_headers["X-NHX-Principal-Id"] == "user-one@example.com"
        assert first._default_headers["X-NHX-Principal-On-Behalf-Of"] == "delegate-one@example.com"
        assert first._default_headers["traceparent"] == "00-trace-one-span-one-01"
        assert second._default_headers["X-NHX-Principal-Id"] == "user-two@example.com"
