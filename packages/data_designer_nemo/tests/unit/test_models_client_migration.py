...
Driving a real ``NeMoHelix`` -> ``client_from_platform`` -> ``ModelsClient``
...
def test_get_nhx_provider_hits_the_provider_endpoint() -> None:
    ...
    sdk = NeMoHelix(base_url=BASE, workspace="default", http_client=httpx.Client(transport=transport))
...
async def test_get_nhx_provider_async_hits_the_provider_endpoint() -> None:
    transport, seen = _recording_transport(_provider_json())
    sdk = AsyncNeMoHelix(base_url=BASE, workspace="default", http_client=httpx.AsyncClient(transport=transport))
