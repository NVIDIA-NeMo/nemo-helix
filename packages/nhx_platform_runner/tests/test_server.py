...
from nhx.common.controller import ControllerManager
from nhx.common.platform_endpoint import _SyncHelixEndpointRoutingTransport
from nhx.common.service import RouterConfig, Service
from nhx.platform_runner import config as runner_config
from nhx.platform_runner import server
from nhx.platform_runner.config import DEFAULT_UVICORN_KEEP_ALIVE_TIMEOUT_SECONDS
from nhx.platform_runner.health import ReadinessCheck, create_platform_health_router
...
_RUN_ENV_KEYS = (
    ...
    "NHX_SERVICE_PORT",
    "NHX_BASE_URL",
    "NHX_AUTH_POLICY_DECISION_POINT_BASE_URL",
    "NHX_SERVICES",
    "NHX_CONTROLLERS",
    "NHX_SIDECARS",
)
...
def test_build_platform_app_returns_app_without_running_uvicorn(monkeypatch):
    ...
    app = server.build_platform_app(
        runner_config.HelixAppConfig(services=["agents"], controllers=[]),
        access_key_lifecycle_http_client=lifecycle_http_client,
        env={},
    )
...
def test_build_platform_app_accepts_platform_app_config(monkeypatch):
    ...
    app = server.build_platform_app(
        config=runner_config.HelixAppConfig(services=("agents",), controllers=()),
        env={},
    )
...
def test_embedded_auth_preflight_invokes_policy_wasm_helper(monkeypatch):
    ...
    from nhx.core.auth.app.embedded_pdp import policy_wasm
...
def test_run_server_runs_embedded_auth_preflight():
    ...
    with (
        patch("nhx.platform_runner.server.get_auth_config", return_value=auth_cfg),
        patch(
            "nhx.platform_runner.server.preflight_embedded_auth_policy_wasm", side_effect=lambda cfg: calls.append(cfg)
        ),
        patch("nhx.platform_runner.server.create_app", return_value=FastAPI()) as create_app,
        patch("nhx.platform_runner.server.setup_fastapi_instrumentations"),
        patch("nhx.platform_runner.server.uvicorn.run") as uvicorn_run,
    ):
...
def test_run_server_can_bind_tcp_and_unix_domain_socket():
    ...
    with (
        patch("nhx.platform_runner.server.get_auth_config", return_value=auth_cfg),
        patch("nhx.platform_runner.server.preflight_embedded_auth_policy_wasm"),
        patch("nhx.platform_runner.server.create_app", return_value=FastAPI()),
        patch("nhx.platform_runner.server.setup_fastapi_instrumentations"),
        patch("nhx.platform_runner.server._run_server_on_bound_sockets") as run_bound_sockets,
    ):
        server.run_server(services=[], host="127.0.0.1", port=9999, socket_path="/tmp/nemo-helix.sock")
    ...
    assert run_bound_sockets.call_args.kwargs == {
        ...
        "port": 9999,
        "socket_path": "/tmp/nemo-helix.sock",
    }
...
def test_run_server_on_bound_sockets_sets_keep_alive_timeout():
    ...
    with (
        patch("nhx.platform_runner.server.uvicorn.Config", side_effect=[tcp_config, uds_config]) as config_cls,
        patch("nhx.platform_runner.server.uvicorn.Server") as server_cls,
        patch("nhx.platform_runner.server.asyncio.run") as asyncio_run,
    ):
...
def test_run_server_with_reload_sets_keep_alive_timeout():
    ...
    with (
        ...
        server.run_server_with_reload(
            "nhx.platform_runner.server:create_default_app",
            host="127.0.0.1",
            port=9999,
            keep_alive_timeout_seconds=12,
        )
...
def test_create_default_app_raises_for_unknown_service_from_env(monkeypatch):
    monkeypatch.setattr(server, "_obs_initialized", True)
    monkeypatch.setenv("NHX_SERVICES", "missing-service")
    monkeypatch.setattr(server, "get_available_services", lambda: {})
    monkeypatch.setattr(server, "get_available_controllers", lambda: {})

    with pytest.raises(
        ValueError, match="Unknown service 'missing-service' requested via NHX_SERVICES='missing-service'"
    ):
        server.create_default_app()
...
def test_create_default_app_raises_for_unknown_controller_from_env(monkeypatch):
    monkeypatch.setattr(server, "_obs_initialized", True)
    monkeypatch.delenv("NHX_SERVICES", raising=False)
    monkeypatch.setenv("NHX_CONTROLLERS", "missing-controller")
    monkeypatch.setattr(server, "get_available_services", lambda: {})
    monkeypatch.setattr(server, "get_available_controllers", lambda: {})

    with pytest.raises(
        ValueError,
        match="Unknown controller 'missing-controller' requested via NHX_CONTROLLERS='missing-controller'",
    ):
        server.create_default_app()
...
def test_create_app_reports_controller_that_crashes_before_registration(monkeypatch, caplog):
    ...
    manager.clear()
    try:
        with caplog.at_level(logging.ERROR, logger="nhx.platform_runner.controller_threads"):
            with TestClient(
...
def test_create_app_with_seed_on_startup_blocks_readiness_until_seed_completes(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "nhx.platform_seed", SimpleNamespace(run_platform_seed_from_startup=fake_seed))
...
def test_root_redirects_to_studio(auth_enabled, method):
    ...
    with (
        patch("nhx.platform_runner.server.get_platform_config", return_value=_make_platform_config_mock()),
        patch("nhx.platform_runner.server.get_auth_config", return_value=auth_cfg),
        patch("nhx.common.auth.middleware.get_auth_config", return_value=auth_cfg),
    ):
...
def test_root_returns_ok_when_redirect_disabled():
    ...
    with (
        patch(
            "nhx.platform_runner.server.get_platform_config",
...
def test_non_get_root_still_requires_auth():
    ...
    with (
        ...
        patch("nhx.platform_runner.server.get_auth_config", return_value=auth_cfg),
        patch("nhx.common.auth.middleware.get_auth_config", return_value=auth_cfg),
        patch("nhx.common.auth.client.AuthClient.authorize_request", return_value=MagicMock(allowed=False)),
    ):
        app = server.create_app(services=[])
